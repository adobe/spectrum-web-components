...
- **Invalid-gated error message** — following React Spectrum's [TextField](https://react-spectrum.adobe.com/TextField) strategy, the in-shadow error-text element is folded into that same `ariaDescribedByElements` list (after the description, before any external elements) only while the host reads as `invalid` (read structurally, so a host that doesn't declare `invalid` at all simply never surfaces an error message). It is not exposed via `aria-errormessage`, since AT support for that attribute is still inconsistent.
...
The error-text element is folded into `ariaDescribedByElements` only while the host is `invalid`; the description remains associated regardless.
...
- **Error message folded into `aria-describedby`** — following React Spectrum's [TextField](https://react-spectrum.adobe.com/TextField) strategy, the error message is never the sole description source; it supplements the combined `ariaDescribedByElements` set while `invalid` is `true`, rather than being pointed at separately via `aria-errormessage`.
