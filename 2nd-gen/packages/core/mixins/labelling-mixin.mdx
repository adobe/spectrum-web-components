...
- **Dev-mode warnings** — warns when no accessible-name source resolves to anything, and separately warns when `accessibleLabel` is set alongside a visible label (slotted or via a resolved `accessibleLabelledby`), since a differently-worded `accessibleLabel` would silently take over the accessible name.
...
Setting `accessible-label` alongside a visible label (a slotted `label`, or a resolved `accessible-labelledby`) triggers a dev-mode console warning. A visible label is itself the accessible name; a differently-worded `accessible-label` would silently override that computation, which is exactly the [Label in Name (2.5.3)](https://www.w3.org/WAI/WCAG22/Understanding/label-in-name.html) mismatch this mixin's precedence order otherwise avoids. The warning fires regardless of which source actually wins, since setting both is a likely authoring mistake either way. Open the browser console to see the warning for the example below.
...
- **Dev-mode warnings** — a field with no accessible-name source at all triggers a console warning; so does a field that sets `accessible-label` while a visible label is also present.
