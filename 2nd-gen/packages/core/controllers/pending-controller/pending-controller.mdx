...
- **`PendingController`** (`@adobe/spectrum-wc-core/controllers/pending-controller`) — state (delayed activation, inline-size freeze, derived accessible name) and `renderPendingState()`, which renders the spinner so the host needs no separate directive import.
- **`renderPendingSpinner`** (`@adobe/spectrum-wc-core/directives/pending-spinner`) — the underlying render-only directive. The controller owns and exposes it; use it directly only for a stateless spinner without a controller.
- **`PendingMixin`** (`@adobe/spectrum-wc-core/mixins`) — wires the controller, the `pending` / `pending-label` properties, and click suppression onto a host in one step, and re-exposes `renderPendingState()`.
...
} from '@adobe/spectrum-wc-core/controllers/pending-controller';
...
| ---------------------- | ---------------------------------------------------- | ------------------------------------------------------------------------------- |
| `renderPendingSpinner` | `@adobe/spectrum-wc-core/directives/pending-spinner` | Underlying spinner render; exposed via the controller's `renderPendingState()`. |
| `PendingMixin`         | `@adobe/spectrum-wc-core/mixins`                     | Wire the controller + properties + click suppression; re-exposes the render.    |
