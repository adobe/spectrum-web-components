...
- <kbd>Home</kbd> / <kbd>End</kbd> jump to the first or last item (row-major
  order for `grid`).
- <kbd>Ctrl</kbd> + <kbd>Home</kbd> / <kbd>Ctrl</kbd> + <kbd>End</kbd> (`grid`
  only) jump to the first cell of the first row or the last cell of the last
  row.
- <kbd>Page Up</kbd> / <kbd>Page Down</kbd> move `pageStep` items (linear modes)
  or rows (`grid`) when `pageStep` is set.
...
- <kbd>Home</kbd> / <kbd>End</kbd> use visual row-major order (first and last
  item in that flattened sequence).
- <kbd>Ctrl</kbd> + <kbd>Home</kbd> / <kbd>Ctrl</kbd> + <kbd>End</kbd> jump to
  the first cell of the top row or the last cell of the bottom row, which
  matches rectangular grids and differs from plain <kbd>End</kbd> only when the
  last row has fewer cells than earlier rows.
- <kbd>Page Up</kbd> / <kbd>Page Down</kbd> move `pageStep` rows at a time (in
  this demo, `pageStep: 2`); the focused column index is clamped when a row has
  fewer cells (same rule as <kbd>ArrowUp</kbd> / <kbd>ArrowDown</kbd>).
