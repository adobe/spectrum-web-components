...
1. **Box**: white card container with shadow, a taller multiline card by default or collapsed to a single-line row via the `collapsed` attribute.
...
- **`collapsed`**: single-line row; the send/stop button sits inline with the textarea and no upload button is shown.

Toggling `collapsed` animates between the two layouts.

The textarea grows with content up to `--swc-prompt-field-max-block-size` (default `40vh`). Override the custom property to raise or lower this ceiling.

To cap growth relative to a surrounding panel instead of the viewport, wrap the prompt field in an ancestor with `container-type: size`:

```html
<div style="container-type: size; block-size: 400px;">
  <swc-prompt-field></swc-prompt-field>
</div>
```

With a sized ancestor present, the default becomes `40cqb` (40% of that ancestor's block size) instead of `40vh`. The prompt field cannot use its own box as the container: the textarea's height would then depend on a container whose height depends on the textarea, and browsers ignore that circular reference, so the sizing must come from something the prompt field is inside of.
