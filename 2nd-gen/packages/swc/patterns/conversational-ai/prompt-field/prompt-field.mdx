...
Prefer a container-relative cap where the prompt field sits inside a sized panel. Give an ancestor `container-type: size` and set the cap to `40cqb` (40% of that ancestor's block size):

```html
<div style="container-type: size; block-size: 400px;">
  <swc-prompt-field
    style="--swc-prompt-field-max-block-size: 40cqb;"
  ></swc-prompt-field>
</div>
```

The prompt field cannot be its own container: the textarea's height would then depend on a container whose height depends on the textarea, a circular reference browsers ignore, so the cap must resolve against something the prompt field is inside of.
