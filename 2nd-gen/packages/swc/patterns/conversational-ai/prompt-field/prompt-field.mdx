...
1. **Box**: white card container with shadow, collapsed to a single-line row by default or expanded into a taller card via the `expanded` attribute.
2. **Input area**: visible label (expanded only) + textarea.
3. **Action bar**: upload trigger (left) and send/stop action (right); shown on its own row only when `expanded`, otherwise the send/stop action sits inline with the textarea.
...
The `expanded` attribute controls the field's collapsed/expanded layout:

- **Collapsed (default)**: single-line row; the send/stop button sits inline with the textarea and no upload button is shown.
- **`expanded`**: taller card with a separate action bar below the textarea, showing the upload button alongside send/stop.

Toggling `expanded` animates between the two layouts (disabled under `prefers-reduced-motion: reduce`).

The textarea grows with content up to `max-rows` and, on small or embedded viewports, up to `--swc-prompt-field-max-block-size` (default `40vh`) if that's smaller — override the custom property to raise or lower this ceiling.
