...
Import the side-effectful registration of `<swc-prompt-field>`:

```ts
import '@adobe/spectrum-wc/patterns/conversational-ai/prompt-field/swc-prompt-field.js';
```

To reference the `PromptField` type, use a type-only import:

```ts
import type { PromptField } from '@adobe/spectrum-wc/patterns/conversational-ai/prompt-field';
```
