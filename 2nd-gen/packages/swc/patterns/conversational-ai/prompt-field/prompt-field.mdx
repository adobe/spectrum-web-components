...
The textarea grows with content up to `--swc-prompt-field-max-block-size` (default `40vh`, or `40cqb` when a sized ancestor sets `container-type`) — override the custom property to raise or lower this ceiling. `min-rows`/`max-rows` add an optional row-count cap on top of this; unset, only `--swc-prompt-field-max-block-size` applies.
