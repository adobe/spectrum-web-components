...
A pixel loader sits at the start of the input row. It rests on a settled frame while idle and animates while **`generating`**. Two attributes configure its artwork, both passed straight through to `swc-pixel-loader`:

- **`icon`** (default `aiLogo`): the single icon the loader builds.
- **`preset`**: a themed icon sequence cycled one per loop (`cc`, `dc`, `exp`, `analyze`, `mega`); overrides `icon` while set.
