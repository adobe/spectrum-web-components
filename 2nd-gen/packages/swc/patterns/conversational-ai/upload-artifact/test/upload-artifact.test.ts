...
export const DismissSizeOverrideTest: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('consumer dismiss-size override is preserved', async () => {
      const dismissButton = el.shadowRoot?.querySelector<HTMLButtonElement>(
        '.swc-UploadArtifact-dismiss'
      );
      ...
      // The dismiss button is an swc-action-button sized from a single
      // --swc-upload-artifact-dismiss-visual-size, which drives its height.
