...
An end-to-end conversation with streaming generation, file upload, feedback, and follow-up suggestions. Try sending a prompt, attaching a file, or selecting a suggestion.
...
<Canvas of={PromptFieldStories.Overview} />

#### `swc-upload-artifact`

Renders an attached file. Used both inside `swc-prompt-field` (before send) and inside a user turn (after send). Supports `card` and `media` types and an optional `dismissible` affordance.

<Canvas of={UploadArtifactStories.Overview} />
...
<Canvas of={ConversationThreadStories.Overview} />
...
<Canvas of={ConversationTurnStories.Overview} />
...
<Canvas of={SystemMessageStories.Overview} />
...
<Canvas of={MessageFeedbackStories.Overview} />

#### `swc-message-sources`

Collapsible numbered list of the sources used to generate the response.

<Canvas of={MessageSourcesStories.Overview} />
...
<Canvas of={SuggestionGroupStories.Overview} />
...
<Canvas of={SuggestionItemStories.Overview} />
