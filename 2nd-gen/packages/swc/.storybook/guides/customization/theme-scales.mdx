...
There are 2 scale contexts available: `medium` (aka desktop) and `large` (aka mobile). Scales apply to properties such as `font-size` and spacing-related values.
...
You can change your application - or a section or element - to an alternate scale by including both the `swc-theme` class and a scale class.
