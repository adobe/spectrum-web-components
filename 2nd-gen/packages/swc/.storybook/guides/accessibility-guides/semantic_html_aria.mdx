...
If you must use a non-interactive wrapper for layout, keep **real** controls, like `button`s, `a`s, or other form elements inside it.

### How semantics help assistive technologies

Screen readers and other assistive technologies expose an **accessibility tree** derived from the DOM. Semantic elements map to familiar roles and names. For example, a `button` is announced as a button, and a labelled text field exposes its label association. Headings define an **outline** to aid in page navigation. Landmark regions, like `main`, `nav`, or `header`, can help users jump to the right part of the page. See [Understanding success criterion 1.3.1: info and relationships](https://www.w3.org/WAI/WCAG22/Understanding/info-and-relationships.html) in WCAG 2.2 for why programmatic relationships and structure matter. Practical guidance is found in our guide about [Headings and landmarks](/docs/guides-accessibility-guides-headings-and-landmarks--docs) (outlines, `aria-labelledby`, and avoiding **too many** generic regions), as well as how to perform in-browser **verification** with [WAVE](/docs/guides-accessibility-guides-wave-toolbar-testing--docs#structure-tab).
...
- **`aria-live`** regions: for status messages that update without moving focus, when no native element conveys the update
- **`aria-invalid`** and **`aria-describedby`**: tying validation messages to fields when relationships from native labelling are unclear
