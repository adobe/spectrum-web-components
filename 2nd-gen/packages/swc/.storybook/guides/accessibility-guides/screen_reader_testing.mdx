...
For how **landmarks** and **headings** should be structured in your markup, see [Accessible landmarks](./accessible_landmarks.mdx). You can **inspect** that structure in the browser with WAVE; see [WAVE toolbar testing](./wave_toolbar_testing.mdx#structure-tab).
...
[Accessible landmarks](./accessible_landmarks.mdx) explains which native **landmark** regions to use, when too many `region` landmarks add noise, and how landmarks work with a heading outline.
...
3. **Navigate to the component**: Use heading or landmark navigation (shape that structure in your app per [Accessible landmarks](./accessible_landmarks.mdx))
...
For quick, on-page **outline** feedback before or during screen reader tests, use [WAVE toolbar testing](./wave_toolbar_testing.mdx). For more testing resources and tools, see [Accessibility resources](./accessibility_resources.mdx).
