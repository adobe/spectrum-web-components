...
export const keyboardTestingGuideImageStyle = {
  border: '1px solid var(--spectrum-gray-300, #d0d0d0)',
  boxSizing: 'border-box',
};

<Meta title="Accessibility/Keyboard testing" />

# Keyboard testing

This guide covers testing keyboard accessibility in Spectrum Web Components, for contributors testing a component implementation and consumers validating an application, so that all users can navigate and interact with an interface using only a keyboard.

> **Did you know?**
>
> Approximately **[15% of all digital accessibility issues](https://www.deque.com/blog/how-keyboard-testing-improves-digital-accessibility/)** relate to keyboard support gaps, and manual keyboard testing catches issues that automated tools cannot detect.
...
- **Invisible focus**: no visible indication of where focus is on the page
- **Keyboard traps**: getting stuck in a section with no way to navigate away
- **Illogical tab order**: focus that jumps randomly instead of following visual flow
- **Missing skip links**: having to tab through repetitive content on every page

## Platform setup instructions

macOS and Safari need manual setup before keyboard testing works as expected. Other browsers are usually ready by default.

### macOS keyboard navigation

By default, macOS does not enable full keyboard navigation. Enable it before testing.

> ⚠️ **Important:** This is the most commonly overlooked setup step on Mac.
...
3. Enable **Keyboard navigation**
   - On older macOS versions, go to the **Shortcuts** tab and select **All controls** under **Full Keyboard Access**
...
<figure>
  <img
    src={macOsSettings}
    alt="macOS Keyboard settings"
    style={keyboardTestingGuideImageStyle}
  />
  <figcaption>
    macOS **System Settings**, **Keyboard**: the **Keyboard navigation** option
    (or, on older macOS, **All controls** under **Full Keyboard Access**) so{' '}
    <kbd>Tab</kbd> reaches all focusable controls.
  </figcaption>
</figure>

### Safari browser settings

Even with macOS keyboard navigation enabled, Safari needs its own setting for in-page <kbd>Tab</kbd> focus.
...
<figure>
  <img
    src={safariSettings}
    alt="Safari Advanced settings"
    style={keyboardTestingGuideImageStyle}
  />
  <figcaption>
    Safari **Settings**, **Advanced** tab: under **Accessibility**, **Press Tab
    to highlight each item on a webpage** is enabled so in-page focus moves to
...
- **Firefox**: Full keyboard navigation is enabled by default. For additional options, open **Settings** → **General** and review **Browsing**.
...
> 
> While buttons use both
<kbd>Enter</kbd> and <kbd>Space</kbd> to activate, **links only use
<kbd>Enter</kbd> to activate.**
...
>
> To experience how keyboard
> interaction should work, try out this [demonstration
> site](https://rha11y-tests.netlify.app/keyboard.html).
...
- **Disabled elements are handled correctly**: Are disabled elements either skipped by <kbd>Tab</kbd> or clearly indicating their disabled state?
...
- [WAVE toolbar testing](/docs/guides-accessibility-guides-wave-toolbar-testing--docs): **order** tab: compare **navigable** order and the drawn path to your visual layout (complements **Tab** testing here)
- [Headings and landmarks](/docs/guides-accessibility-guides-headings-and-landmarks--docs): **skip** links and a sensible **main** / **nav** structure make keyboard traversal predictable
- [Screen reader testing](/docs/guides-accessibility-guides-screen-reader-testing--docs): many screen reader users move focus with the same keys you use for quick navigation in browse mode
