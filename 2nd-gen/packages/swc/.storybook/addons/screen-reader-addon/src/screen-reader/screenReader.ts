    ...
    if (roleKey.name === tagName) {
      ...
      if (roleKey.attributes) {
        const matches = roleKey.attributes.every((attr) => {
          ...
          return element.hasAttribute(attr.name) === (attr.value !== undefined);
        });
...
export default class ScreenReader {
  ...
  private pendingAnnouncement: { element: Element; text: string } | null = null;
   ...
   */
  buildRoleAnnouncement(element: Element, role: string, name: string): string {
    const announcements: Record<string, RoleAnnouncementFn> = {
      ...
      slider: () => {
        const inputElement = element as HTMLInputElement;
        const value =
          inputElement.value || element.getAttribute('aria-valuenow') || '';
        const min =
          inputElement.min || element.getAttribute('aria-valuemin') || '0';
        const max =
          inputElement.max || element.getAttribute('aria-valuemax') || '100';
      ...
      heading: () => {
        const level =
          ...
          (element.tagName ? element.tagName.match(/h(\d)/i)?.[1] : null) ||
          '2';
      ...
      table: () => {
        ...
        const cols =
          element.querySelector('tr, [role="row"]')?.children.length || 0;
      ...
      switch (attr) {
        case 'aria-checked':
          if (value === 'mixed') {
            ...
            const isSwitch = target.getAttribute('role') === 'switch';
            if (isSwitch) {
              announcement = value === 'true' ? 'on' : 'off';
            } else {
              announcement = value === 'true' ? 'checked' : 'not checked';
            }
          }
          break;
  ...
  private onLiveRegionMutation(mutations: MutationRecord[]): void {
    ...
    mutations.forEach((mutation) => {
      ...
      )?.closest('[aria-live], [role="alert"], [role="status"], [role="log"]');
      if (!liveRegion) {
        return;
      }
