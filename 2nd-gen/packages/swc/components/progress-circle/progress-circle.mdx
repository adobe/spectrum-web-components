...
- **Track**: background ring showing the full progress range
...
Use `static-color` when the progress circle needs to sit on top of a photo or colored background. It pins the progress circle's color to the chosen value regardless of the active color theme:
...
By default, when `progress` is not set, the component displays an animated loading indicator. The `aria-valuenow` attribute is omitted, signaling to assistive technologies that the operation duration is unknown.
...
- Progress is shown visually through the fill amount, not relying solely on color ([WCAG 1.4.1](https://www.w3.org/WAI/WCAG22/Understanding/use-of-color.html))
...
- At `progress="0"`, a small amount of fill is still rendered intentionally. A fully empty circle would fail [WCAG 1.4.11](https://www.w3.org/WAI/WCAG22/Understanding/non-text-contrast.html) (non-text contrast) because the track alone may not meet the required 3:1 contrast ratio against the background. The `aria-valuenow` attribute still reflects 0.
...
- Ensure sufficient color contrast between the progress circle and its background ([WCAG 1.4.11](https://www.w3.org/WAI/WCAG22/Understanding/non-text-contrast.html))
