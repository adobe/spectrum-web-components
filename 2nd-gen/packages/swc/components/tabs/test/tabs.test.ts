...
/**
 * Per Spectrum cursor guidelines, tabs use the default (arrow) cursor,
 * not cursor: pointer. Pointer is reserved for links only.
 */
export const CursorNotPointerTest: Story = {
  render: () => html`
    <swc-tabs selected="1" accessible-label="Cursor test">
      <swc-tab tab-id="1">Enabled</swc-tab>
      <swc-tab tab-id="2" disabled>Disabled</swc-tab>
      <swc-tab-panel tab-id="1"><p>Panel 1</p></swc-tab-panel>
      <swc-tab-panel tab-id="2"><p>Panel 2</p></swc-tab-panel>
  ...
  `,
  play: async ({ canvasElement, step }) => {
    const tabs = await getComponents<Tab>(canvasElement, 'swc-tab');
    const enabledTab = tabs[0];
    const disabledTab = tabs[1];

    await step('enabled tab does not use cursor: pointer', async () => {
      const wrapper = enabledTab.shadowRoot?.querySelector(
        '.swc-Tab'
      ) as HTMLElement;
      expect(wrapper, 'tab wrapper exists').toBeTruthy();
      const cursor = getComputedStyle(wrapper).cursor;
      expect(cursor, 'cursor is default, not pointer').not.toBe('pointer');
    });

    await step('disabled tab does not use cursor: pointer', async () => {
      const wrapper = disabledTab.shadowRoot?.querySelector(
        '.swc-Tab'
      ) as HTMLElement;
      expect(wrapper, 'disabled tab wrapper exists').toBeTruthy();
      const cursor = getComputedStyle(wrapper).cursor;
      expect(cursor, 'disabled cursor is default').not.toBe('pointer');
    });
  },
};
