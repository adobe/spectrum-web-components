...
<!-- Standalone icon in a button — set label on the icon or the button -->
...
- Without `label`, the component sets `aria-hidden="true"` on the host, hiding it from assistive technology. Use this for decorative icons that appear alongside visible text.
- When `label` is set, the component removes `aria-hidden` and applies `aria-label` and `role="img"` to the slotted SVG so screen readers announce the icon with a meaningful name.
