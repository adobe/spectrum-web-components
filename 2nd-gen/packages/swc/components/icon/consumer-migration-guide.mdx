...
> `@adobe/spectrum-wc` is a monolithic package. Importing via the per-component side-effect path (e.g. `@adobe/spectrum-wc/components/icon/swc-icon.js`) registers and loads only that element.
