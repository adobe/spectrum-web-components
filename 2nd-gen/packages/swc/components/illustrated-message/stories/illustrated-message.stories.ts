...
const defaultSlotArgs = {
  'default-slot': cloudIcon,
  'heading-slot': 'Illustrated message title',
  'description-slot':
...
export const IllustrationAccessibility: Story = {
  render: (args) => html`
    ${template({
      ...
      'heading-slot': 'Illustrated message title',
      'description-slot':
        'The icon above uses <code>aria-hidden="true"</code> — screen readers skip the illustration entirely and move on to the heading and description.',
    })}
    ${template({
      ...args,
      'default-slot': cloudIcon,
      'heading-slot': 'Illustrated message title',
      'description-slot':
        'The icon above uses <code>role="img"</code> and <code>aria-label</code> — screen readers announce its meaning before reading the heading and description.',
    })}
  `,
  args: {
    ...defaultSlotArgs,
