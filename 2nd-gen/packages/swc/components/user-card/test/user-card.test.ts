...
// The size the fixed AVATAR_SIZE_BY_CARD_SIZE scale (UserCard.ts) maps each
// card size to; kept here (not imported) so a test failure clearly shows
// which side (the scale or the propagation) regressed.
const EXPECTED_AVATAR_SIZE_BY_CARD_SIZE = {
  xs: '100',
  s: '300',
  m: '500',
  l: '700',
...
export const ValidVariantNoWarningTest: Story = {
  render: () => html`
    <swc-user-card variant="secondary">${avatarGlyph()}</swc-user-card>
  `,
  play: async ({ canvasElement, step }) => {
    const card = await getComponent<UserCard>(canvasElement, 'swc-user-card');
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      ...
      async () => {
        ...
        ).toBe(EXPECTED_AVATAR_SIZE_BY_CARD_SIZE.l);
      }
    );
    ...
    await step(
      ...
      async () => {
        ...
        ).toBe(EXPECTED_AVATAR_SIZE_BY_CARD_SIZE.xl);
      }
    );

    await step(
      'maps every card size to its documented avatar size',
      async () => {
        for (const size of CARD_VALID_SIZES) {
          ...
          expect(
            avatar.getAttribute('size'),
            `avatar size matches the card size="${size}" mapping`
          ).toBe(EXPECTED_AVATAR_SIZE_BY_CARD_SIZE[size]);
        }
      }
    );
    ...
    await step(
      ...
      async () => {
        ...
        ).toBe(EXPECTED_AVATAR_SIZE_BY_CARD_SIZE.m);
      }
    );
    ...
    await step(
      ...
      async () => {
        ...
        ).toBe(EXPECTED_AVATAR_SIZE_BY_CARD_SIZE.s);
...
export const AvatarOutlinePropagationTest: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('forces the avatar outline width to 2px', () => {
      const image = avatar.shadowRoot?.querySelector('.swc-Avatar-image');
      expect(image, "the avatar's image element exists").toBeTruthy();
      expect(
        getComputedStyle(image as Element).outlineWidth,
        'the outline width resolves to 2px, matching border-width-200'
      ).toBe('2px');
    });
...
export const AvatarFlushWithoutPreviewTest: Story = {
  render: () => html`
    <swc-user-card>${avatarGlyph()}</swc-user-card>
  `,
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      'sits flush (no negative pull-up margin) when there is no preview',
      () => {
        const marginBlockStart = getComputedStyle(avatar).marginBlockStart;
        expect(
          marginBlockStart.startsWith('-'),
          'avatar margin-block-start is not negative without a preview'
        ).toBe(false);
        expect(
          parseFloat(marginBlockStart),
          'avatar has a positive inset from the top of the card'
        ).toBeGreaterThan(0);
      }
    );
  },
...
export const AvatarOverlapCenteredAcrossSizesTest: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      'centers the avatar on the preview/content seam at every size',
      () => {
  ...
  play: async ({ canvasElement, step }) => {
    await step('warns when the avatar slot has no assigned content', () =>
      withWarningSpy(async (warnCalls) => {
        ...
        expect(
          warnCalls.length,
          'at least one warning is emitted for the missing avatar'
        ).toBeGreaterThan(0);
        expect(
          String(warnCalls[0]?.[1] || ''),
