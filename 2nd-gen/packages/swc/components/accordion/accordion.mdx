...
Density controls the vertical padding applied to each item header. Three values are available: `regular` (the default), `compact`, and `spacious`. The component emits a development warning when `density` is omitted.
