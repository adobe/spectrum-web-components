...
- If the wrapper stacks vertical groups and you want them to share a width, give the wrapper `inline-size: fit-content`. Every group then shrinks to fit the longest label across all of them, while still narrowing further if the surrounding layout is tighter than that. Give the wrapper an explicit `inline-size` value instead for a fixed toolbar width, and each `swc-action-group` fills it automatically.
