...
How you disable a child depends on scope:

- **Whole strip unavailable:** use the group's `disabled` attribute (above).
- **One action unavailable but keyboard-discoverable:** set `aria-disabled="true"` on that `swc-action-button`. The child stays in the roving <kbd>Tab</kbd> and arrow-key sequence; activation is suppressed.
- **One action removed from focus:** set `disabled` on that `swc-action-button`. Native `disabled` applies; the child cannot receive focus and is never the roving tab stop. Arrow keys cannot move focus onto it; pressing an arrow toward that child leaves focus on the current item.
...
- When the whole strip is unavailable, use the group's `disabled` attribute so users can still <kbd>Tab</kbd> into the group and discover those actions. When one action is unavailable but should stay keyboard-discoverable, set `aria-disabled="true"` on that child. Use an individual child's `disabled` attribute only when that control should be removed from focus entirely.
