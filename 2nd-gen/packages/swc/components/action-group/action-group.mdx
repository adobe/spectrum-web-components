...
- If the wrapper stacks vertical groups and you want them to share a width, set the wrapper's `display` to `inline-flex` with `flex-direction: column` and no explicit `inline-size`. Every group then shrinks to fit the longest label across all of them. A block-level `display: flex` wrapper stretches every group to the wrapper's own width instead. Give the wrapper an explicit `inline-size` for a fixed toolbar width, and each `swc-action-group` fills it automatically.
