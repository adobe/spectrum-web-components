...
- **Popover coexistence:** `<swc-tooltip>` uses `popover="manual"`, so opening a tooltip does not dismiss an open `<swc-popover>`, menu, or picker, and opening one of those does not dismiss the tooltip. This preserves the Spectrum 1 `type="hint"` isolation behavior, which left menus and pickers open.
