...
| `disabled` boolean attribute                | Prevents the tooltip from opening via hover, focus, or a programmatic `open = true`. Takes priority over `open` (setting `open` while disabled is a no-op; disabling an open tooltip closes it), including in `manual` mode. Trigger remains interactive. |
...
The tooltip opens after a warm-up delay (default is 1500 ms; set `delay="0"` to open immediately) and closes when the pointer leaves or focus moves away. To permanently opt out of automatic wiring and retain full programmatic control, add the `manual` attribute:
...
- **Popover coexistence:** `<swc-tooltip>` uses `popover="manual"`, so opening a tooltip does not dismiss an open `<swc-popover>`, menu, or picker, and opening one of those does not dismiss the tooltip. This preserves the Spectrum 1 `type="hint"` isolation behavior, which left menus and pickers open. The tooltip joins the shared dismissible stack, so when it is open on top of a popover, Escape closes the tooltip first and leaves the popover open; a second Escape closes the popover.
