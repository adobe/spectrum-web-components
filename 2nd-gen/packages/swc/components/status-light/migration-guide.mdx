...
<Meta title="Status light/Migration guide" />

# Status light migration guide
...
> `@adobe/spectrum-wc` is a monolithic package. Importing via subpath (e.g., `@adobe/spectrum-wc/components/status-light/swc-status-light.js`) registers and loads only that component's bundle.
...
| Import path       | `@spectrum-web-components/status-light` | `@adobe/spectrum-wc/components/status-light/swc-status-light.js`               |
...
- Status is conveyed by the indicator dot color. Always pair it with a visible text label so the meaning is not lost for users who cannot perceive color ([WCAG 1.4.1](https://www.w3.org/WAI/WCAG22/Understanding/use-of-color.html)).
...
- Do not rely on variant color alone to communicate critical state ([WCAG 1.4.1](https://www.w3.org/WAI/WCAG22/Understanding/use-of-color.html)). Supplement with an icon or additional context when needed.
