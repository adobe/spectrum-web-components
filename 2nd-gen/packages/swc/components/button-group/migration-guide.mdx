...
| `role="group"` on host | Set automatically; no consumer action required               |
...
`swc-button-group` defaults to `size="m"` and always propagates its size to slotted children. If you previously relied on buttons managing their own individual sizes, set an explicit size on the group.
...
Use group-level `disabled` instead of disabling each button individually:
...
{/* @todo Replace the Description column with the `@cssproperty` JSDoc descriptions from `<swc-button-group>`'s CEM entry once they are added in a follow-up PR. */}

| Custom property                      | Description                               |
| ------------------------------------ | ----------------------------------------- |
| `--swc-button-group-gap`             | Space between buttons in the group.       |
| `--swc-button-group-justify-content` | Alignment of buttons along the main axis. |
