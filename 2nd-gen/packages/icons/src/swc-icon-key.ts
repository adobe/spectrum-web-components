...
import { Icon_Key } from './Key.js';

import styles from './stylesheets/icon-base.css';

/**
 * The Key workflow icon as a custom element.
 *
 * @element swc-icon-key
 * @since 2.0.0-beta.2
 *
...
export class IconKey extends IconBase {
  ...
  protected override render(): TemplateResult {
    return html`
      <span class="swc-Icon">${unsafeSVG(Icon_Key())}</span>
    `;
  }
}
...
declare global {
  interface HTMLElementTagNameMap {
    'swc-icon-key': IconKey;
  }
}

defineElement('swc-icon-key', IconKey);
