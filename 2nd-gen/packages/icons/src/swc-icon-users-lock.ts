...
import { Icon_UsersLock } from './UsersLock.js';

import styles from './stylesheets/icon-base.css';

/**
 * The UsersLock workflow icon as a custom element.
 *
 * @element swc-icon-users-lock
 * @since 2.0.0-beta.2
 *
...
export class IconUsersLock extends IconBase {
  ...
  protected override render(): TemplateResult {
    return html`
      <span class="swc-Icon">${unsafeSVG(Icon_UsersLock())}</span>
    `;
  }
}
...
declare global {
  interface HTMLElementTagNameMap {
    'swc-icon-users-lock': IconUsersLock;
  }
}

defineElement('swc-icon-users-lock', IconUsersLock);
