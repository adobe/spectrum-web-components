...
function convertRGB(input) {
  ...
  return a !== undefined ? `rgb(${r} ${g} ${b} / ${a})` : `rgb(${r} ${g} ${b})`;
}
...
function resolveAlias(lookup, value, prefix, seen, debug) {
  ...
  if (seen.has(name)) {
    log('[⚠️ ISSUE] circular alias detected: ', [...seen, name].join(' -> '));
    return undefined;
  }
    ...
    if (typeof value === 'object' && !Array.isArray(value)) {
      if (value.light && value.dark) {
        if (!name.includes('color') && !name.match(/(\d+)$/)) {
          ...
          write(name, `light-dark(${value.light}, ${value.dark})`, nonScaling);
        }
      }
...
async function loadTokenJson(file, src) {
  ...
  try {
    return JSON.parse(await readFile(require.resolve(`${filePath}`), 'utf8'));
  } catch (error) {
