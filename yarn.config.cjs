  ...
  async constraints({ Yarn }) {
    ...
    function enforceConsistencyForWorkspaceGroup({ Yarn }, filterFn) {
      ...
      for (const dependency of Yarn.dependencies()) {
        ...
        const version =
          dependency.workspace.manifest.dependencies?.[dependency.ident] ||
          dependency.workspace.manifest.devDependencies?.[dependency.ident];

        if (version) {
          workspaceVersions.set(
            dependency.ident,
            workspaceVersions.has(dependency.ident)
              ? workspaceVersions.get(dependency.ident).add(version)
              : new Set([version])
          );
        }
      }
    ...
    function validatePackageJson(workspace) {
       ...
       */
      if (!workspace.manifest.license || workspace.manifest.license === '') {
