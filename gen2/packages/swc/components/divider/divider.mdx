...
A vertical divider automatically stretches to fill the height of its flex row or `grid-auto-flow: column` grid. The parent doesn't need an explicit height.
...
Each static color panel shows **horizontal** dividers at sizes `s`, `m`, and `l`, and **vertical** dividers at the same sizes.
