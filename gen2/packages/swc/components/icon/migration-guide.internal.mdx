...
<Meta title="Icon/Migration guide" />
...
> `@adobe/spectrum-wc` is a monolithic package. Importing via subpath (e.g., `@adobe/spectrum-wc/components/icon/swc-icon.js`) registers and loads only that component's bundle.
...
| Import path      | `@spectrum-web-components/icon` | `@adobe/spectrum-wc/components/icon/swc-icon.js` |
...
| Removed                          | Replacement                                                                                                                    |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `name` attribute                 | Use `<swc-ui-icon>` or a workflow icon element, or slot an SVG directly; see [step 2](#2-replace-name-or-src-with-slotted-svg) |
...
Spectrum's own icons ship as dedicated elements: UI icons (chevron, cross, arrow, and the like) as `<swc-ui-icon>`, and workflow icons as per-icon custom elements from `@adobe/spectrum-wc-icons`. Reach for `<swc-icon>` with slotted SVG only for custom art that is not part of a Spectrum icon set.
...
html`
  <swc-ui-icon icon="arrow" size="m" accessible-label="Arrow"></swc-ui-icon>
`;
...
<swc-icon accessible-label="Arrow">
...
<swc-icon accessible-label="My icon">
...
### 4. (Optional) Add `accessible-label` for meaningful icons

When an icon communicates information not available from surrounding text, add an `accessible-label` attribute so assistive technology can announce it.
...
<swc-icon accessible-label="Search"><svg>…</svg></swc-icon>
...
- The host owns semantics. When `accessible-label` is set, the host is exposed as an image (`role="img"` with that label); when omitted, the host is marked `aria-hidden="true"` and the icon is decorative. Use the decorative case for icons that appear alongside visible text.
- The SVG (slotted or rendered) is never given its own role, avoiding double-announcement.
...
- [ ] Add `accessible-label` to icons that convey meaning not available from surrounding text
