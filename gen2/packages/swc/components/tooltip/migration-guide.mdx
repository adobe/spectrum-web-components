...
- **Icon-only triggers:** Add `accessible-label` to the trigger (gen2 SWC components) or `aria-label` (native elements) so the trigger has an accessible name independent of the tooltip. The tooltip describes; it does not name. See [step 5](#5-change-the-authoring-pattern).
