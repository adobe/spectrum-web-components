...
import { Icon_Link } from './Link.js';

import styles from './stylesheets/icon-base.css';

/**
 * The Link workflow icon as a custom element.
 *
 * @element swc-icon-link
 * @since 2.0.0-beta.2
 *
...
export class IconLink extends IconBase {
  ...
  protected override render(): TemplateResult {
    return html`
      <span class="swc-Icon">${unsafeSVG(Icon_Link())}</span>
    `;
  }
}
...
declare global {
  interface HTMLElementTagNameMap {
    'swc-icon-link': IconLink;
  }
}

defineElement('swc-icon-link', IconLink);
