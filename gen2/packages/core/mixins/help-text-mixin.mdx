...
- **Error message replaces the description while invalid**: following React Spectrum's [TextField](https://react-spectrum.adobe.com/TextField) strategy, an invalid field shows its error message in place of the description rather than alongside it. `ariaDescribedByElements` references only the message currently on screen, so a screen reader never announces hidden description text, and the error is surfaced through `aria-describedby` rather than a separately-pointed `aria-errormessage`.
