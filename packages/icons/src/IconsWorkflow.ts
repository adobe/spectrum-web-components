...
import iconsSVG from './icons-workflow.svg.js';

export class IconsWorkflow extends IconsetSVG {
    public constructor() {
        super();
        this.name = 'workflow'; // default iconset name for these icons
    }
    ...
    protected getSVGIconName(icon: string): string {
        return `spectrum-icon-24-${icon}`;
    }
    protected getSanitizedIconName(icon: string): string {
        return icon.replace('spectrum-icon-24-', '');
    }
}
