...
describe("Tag", () => {
  ...
  it("dispatches `delete` events on click", async () => {
    ...
    const root: HTMLElement | DocumentFragment = el.shadowRoot
      ? el.shadowRoot
      : el;
    const deleteButton = root.querySelector("sp-clear-button") as ClearButton;
  ...
  it("dispatches `delete` events on keyboard input", async () => {
    ...
    `);
    const testKeyboardEvent = async (event: KeyboardEvent): Promise<void> => {
      ...
      expect(deleteSpy.called).to.be.true;
      expect(deleteSpy.callCount, `Accepts "${event.code}" key input`).to.equal(
        expectedEventCount,
      );
    };
    ...
    el.dispatchEvent(deleteEvent());
    expect(deleteSpy.callCount, "Does not respond after `focusout`").to.equal(
      expectedEventCount,
    );
  });
