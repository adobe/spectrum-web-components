...
describe("ColorArea", () => {
  testForLitDevWarnings(
    async () =>
      await fixture<ColorArea>(html` <sp-color-area></sp-color-area> `),
  );
  ...
  it("provides separate aria-labels for X and Y inputs", async () => {
    ...
    expect(inputX?.getAttribute("aria-roledescription")).to.equal("2d slider");
    expect(inputY?.getAttribute("aria-roledescription")).to.equal("2d slider");
  ...
  it('dispatches input and change events in response to "Arrow*" keypresses', async () => {
    ...
    expect(inputSpy.callCount).to.equal(2);
    expect(changeSpy.callCount).to.equal(2);
    expect(parseFloat(el.inputY.value).toFixed(2)).to.equal(Yvalue.toFixed(2));
    ...
    expect(inputSpy.callCount).to.equal(2);
    expect(changeSpy.callCount).to.equal(2);
    expect(parseFloat(el.inputX.value).toFixed(2)).to.equal(Xvalue.toFixed(2));
  ...
  const colorFormats: {
    name: string;
    color: string | number | TinyColor | HSVA | HSV | RGB | RGBA | HSL | HSLA;
  }[] = [
    //rgb
