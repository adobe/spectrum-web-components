...
export function runPickerTests(): void {
  ...
  describe("standard", () => {
    ...
    describe("tab order", () => {
      ...
      it("can close and immediately tab to the next tab stop", async () => {
        ...
        await waitUntil(() => isMenuActiveElement(el), "first item focused");
    ...
    it("scrolls selected into view on open", async () => {
      ...
      const getParentOffset = (el: HTMLElement): number => {
        const parentScroll = (
          (el as HTMLElement & { assignedSlot: HTMLSlotElement }).assignedSlot
  ...
  describe("disabled", function () {
    beforeEach(async function () {
      const test = await fixture(html` <div>${disabled(disabled.args)}</div> `);
    ...
    it('manages its "name" value in the accessibility tree when [pending]', async () => {
      ...
      expect(
        findAccessibilityNode<NamedNode>(
          snapshot,
          (node) =>
            node.name === "Pending Choose your neighborhood Where do you live?",
        ),
      ).to.not.be.null;
    });
  });
