...
const iconsPackage = require.resolve('@spectrum-css/ui-icons');
const keepColors = '';
...
const icons = (
    await fg(`**.svg`, {
        cwd: path.dirname(iconsPackage),
        absolute: true,
    })
).sort();

const iconsNameList = await fsp
    .readFile(path.join(path.dirname(iconsPackage), 'icons.json'), 'utf-8')
    .then(JSON.parse);

await buildIcons(icons, 'icons', iconsNameList);
