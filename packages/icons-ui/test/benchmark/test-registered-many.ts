/*
Copyright 2020 Adobe. All rights reserved.
This file is licensed to you under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License. You may obtain a copy
of the License at http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software distributed under
the License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR REPRESENTATIONS
OF ANY KIND, either express or implied. See the License for the specific language
governing permissions and limitations under the License.
*/

import '@lliad-ui/icons-ui/icons/sp-icon-arrow75.js';
import '@lliad-ui/icons-ui/icons/sp-icon-arrow100.js';
import '@lliad-ui/icons-ui/icons/sp-icon-arrow200.js';
import '@lliad-ui/icons-ui/icons/sp-icon-arrow300.js';
import '@lliad-ui/icons-ui/icons/sp-icon-arrow400.js';
import '@lliad-ui/icons-ui/icons/sp-icon-arrow500.js';
import '@lliad-ui/icons-ui/icons/sp-icon-arrow600.js';
import '@lliad-ui/icons-ui/icons/sp-icon-asterisk75.js';
import '@lliad-ui/icons-ui/icons/sp-icon-asterisk100.js';
import '@lliad-ui/icons-ui/icons/sp-icon-asterisk200.js';
import '@lliad-ui/icons-ui/icons/sp-icon-asterisk300.js';
import '@lliad-ui/icons-ui/icons/sp-icon-checkmark50.js';
import '@lliad-ui/icons-ui/icons/sp-icon-checkmark75.js';
import '@lliad-ui/icons-ui/icons/sp-icon-checkmark100.js';
import '@lliad-ui/icons-ui/icons/sp-icon-checkmark200.js';
import '@lliad-ui/icons-ui/icons/sp-icon-checkmark300.js';
import '@lliad-ui/icons-ui/icons/sp-icon-checkmark400.js';
import '@lliad-ui/icons-ui/icons/sp-icon-checkmark500.js';
import '@lliad-ui/icons-ui/icons/sp-icon-checkmark600.js';
import '@lliad-ui/icons-ui/icons/sp-icon-chevron75.js';
import '@lliad-ui/icons-ui/icons/sp-icon-chevron100.js';
import '@lliad-ui/icons-ui/icons/sp-icon-chevron200.js';
import '@lliad-ui/icons-ui/icons/sp-icon-chevron300.js';
import '@lliad-ui/icons-ui/icons/sp-icon-chevron400.js';
import '@lliad-ui/icons-ui/icons/sp-icon-chevron500.js';
import '@lliad-ui/icons-ui/icons/sp-icon-chevron600.js';
import '@lliad-ui/icons-ui/icons/sp-icon-corner-triangle75.js';
import '@lliad-ui/icons-ui/icons/sp-icon-corner-triangle100.js';
import '@lliad-ui/icons-ui/icons/sp-icon-corner-triangle200.js';
import '@lliad-ui/icons-ui/icons/sp-icon-corner-triangle300.js';
import '@lliad-ui/icons-ui/icons/sp-icon-cross75.js';
import '@lliad-ui/icons-ui/icons/sp-icon-cross100.js';
import '@lliad-ui/icons-ui/icons/sp-icon-cross200.js';
import '@lliad-ui/icons-ui/icons/sp-icon-cross300.js';
import '@lliad-ui/icons-ui/icons/sp-icon-cross400.js';
import '@lliad-ui/icons-ui/icons/sp-icon-cross500.js';
import '@lliad-ui/icons-ui/icons/sp-icon-cross600.js';
import '@lliad-ui/icons-ui/icons/sp-icon-dash50.js';
import '@lliad-ui/icons-ui/icons/sp-icon-dash75.js';
import '@lliad-ui/icons-ui/icons/sp-icon-dash100.js';
import '@lliad-ui/icons-ui/icons/sp-icon-dash200.js';
import '@lliad-ui/icons-ui/icons/sp-icon-dash300.js';
import '@lliad-ui/icons-ui/icons/sp-icon-dash400.js';
import '@lliad-ui/icons-ui/icons/sp-icon-dash500.js';
import '@lliad-ui/icons-ui/icons/sp-icon-dash600.js';
import '@lliad-ui/icons-ui/icons/sp-icon-double-gripper.js';
import '@lliad-ui/icons-ui/icons/sp-icon-single-gripper.js';
import '@lliad-ui/icons-ui/icons/sp-icon-triple-gripper.js';
import { html } from '@lliad-ui/base';
import { measureFixtureCreation } from '../../../../test/benchmark/helpers.js';

measureFixtureCreation(html`
    <sp-icon-arrow75></sp-icon-arrow75>
    <sp-icon-arrow100></sp-icon-arrow100>
    <sp-icon-arrow200></sp-icon-arrow200>
    <sp-icon-arrow300></sp-icon-arrow300>
    <sp-icon-arrow400></sp-icon-arrow400>
    <sp-icon-arrow500></sp-icon-arrow500>
    <sp-icon-arrow600></sp-icon-arrow600>
    <sp-icon-asterisk75></sp-icon-asterisk75>
    <sp-icon-asterisk100></sp-icon-asterisk100>
    <sp-icon-asterisk200></sp-icon-asterisk200>
    <sp-icon-asterisk300></sp-icon-asterisk300>
    <sp-icon-checkmark50></sp-icon-checkmark50>
    <sp-icon-checkmark75></sp-icon-checkmark75>
    <sp-icon-checkmark100></sp-icon-checkmark100>
    <sp-icon-checkmark200></sp-icon-checkmark200>
    <sp-icon-checkmark300></sp-icon-checkmark300>
    <sp-icon-checkmark400></sp-icon-checkmark400>
    <sp-icon-checkmark500></sp-icon-checkmark500>
    <sp-icon-checkmark600></sp-icon-checkmark600>
    <sp-icon-chevron75></sp-icon-chevron75>
    <sp-icon-chevron100></sp-icon-chevron100>
    <sp-icon-chevron200></sp-icon-chevron200>
    <sp-icon-chevron300></sp-icon-chevron300>
    <sp-icon-chevron400></sp-icon-chevron400>
    <sp-icon-chevron500></sp-icon-chevron500>
    <sp-icon-chevron600></sp-icon-chevron600>
    <sp-icon-corner-triangle75></sp-icon-corner-triangle75>
    <sp-icon-corner-triangle100></sp-icon-corner-triangle100>
    <sp-icon-corner-triangle200></sp-icon-corner-triangle200>
    <sp-icon-corner-triangle300></sp-icon-corner-triangle300>
    <sp-icon-cross75></sp-icon-cross75>
    <sp-icon-cross100></sp-icon-cross100>
    <sp-icon-cross200></sp-icon-cross200>
    <sp-icon-cross300></sp-icon-cross300>
    <sp-icon-cross400></sp-icon-cross400>
    <sp-icon-cross500></sp-icon-cross500>
    <sp-icon-cross600></sp-icon-cross600>
    <sp-icon-dash50></sp-icon-dash50>
    <sp-icon-dash75></sp-icon-dash75>
    <sp-icon-dash100></sp-icon-dash100>
    <sp-icon-dash200></sp-icon-dash200>
    <sp-icon-dash300></sp-icon-dash300>
    <sp-icon-dash400></sp-icon-dash400>
    <sp-icon-dash500></sp-icon-dash500>
    <sp-icon-dash600></sp-icon-dash600>
    <sp-icon-double-gripper></sp-icon-double-gripper>
    <sp-icon-single-gripper></sp-icon-single-gripper>
    <sp-icon-triple-gripper></sp-icon-triple-gripper>
`);
