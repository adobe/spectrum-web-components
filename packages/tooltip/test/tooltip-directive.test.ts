...
describe("Tooltip Directive", () => {
  const renderTooltip = (): TemplateResult => html` Tip me! `;

  function renderButton(
    ...directiveParams: Parameters<typeof tooltip>
  ): TemplateResult {
    return html`
      <sp-button ${tooltip(...directiveParams)}> I'm a button... </sp-button>
    `;
  }
  ...
  describe("with `options`", () => {
    ...
    it("passes `options` to the overlay", async function () {
      ...
      const tooltipEl = this.overlays[0].querySelector("sp-tooltip") as Tooltip;
