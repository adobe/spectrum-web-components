...
const overlaid = (openPlacement: Placement): TemplateResult => {
  return html`
    ${overlayStyles}
    ${(["bottom", "left", "right", "top"] as Placement[]).map((placement) => {
      return html`
        <overlay-trigger
          placement="${placement}-start"
          open=${ifDefined(openPlacement === placement ? "click" : undefined)}
        >
          <sp-button label="${placement}-start test" slot="trigger">
            Click for ${placement}-start popover
          </sp-button>
          <sp-popover tip slot="click-content">
            <sp-dialog style="--mod-dialog-min-inline-size: 0;">
              <h2 slot="heading">Popover ${placement}-start</h2>
