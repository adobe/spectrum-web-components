...
describe("Sidenav", () => {
  ...
  it("does not accept focus/click/blur when empty", async () => {
    const el = await fixture<SideNav>(html` <sp-sidenav></sp-sidenav> `);
  ...
  it("handles select", async () => {
    ...
    const sidenavItem = el.querySelector('[value="Section 2"]') as SideNavItem;
  ...
  it("prevents [tabindex=0] while `focusin`", async () => {
    ...
    const toBeSelected = el.querySelector('[value="Section 0"]') as SideNavItem;
    ...
    expect(toBeSelected.tabIndex, "will be new focusable child").to.equal(0);
  ...
  it("focuses the child anchor not the root when [tabindex=-1]", async () => {
    ...
    await elementUpdated(el);
    const firstItem = el.querySelector('[value="Section 0"]') as SideNavItem;
  ...
  it("manage tab index for late added items", async () => {
    const el = await fixture<SideNav>(html`
      <sp-sidenav manage-tab-index>
        <sp-sidenav-item value="Section 0" label="Section 0"></sp-sidenav-item>
        <sp-sidenav-item value="Section 1" label="Section 1"></sp-sidenav-item>
      </sp-sidenav>
    `);
