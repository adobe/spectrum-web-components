...
export class SideNav extends Focusable {
  ...
  rovingTabindexController = new RovingTabindexController<SideNavItem>(this, {
    focusInIndex: (elements: SideNavItem[]) => {
      ...
      let index = elements.findIndex((el) => {
        ...
        return this.value
          ? !el.disabled && !this.isDisabledChild(el) && el.value === this.value
  ...
  private isDisabledChild(child: SideNavItem): boolean {
    ...
    let parent = child.parentElement as SideNavItem | SideNav | SideNavHeading;
    ...
    while (
      ...
      parent = parent.parentElement as SideNavItem | SideNav | SideNavHeading;
    }
  ...
  protected override render(): TemplateResult {
    return html`
      ...
      >
        <div role="list">
          <slot name="descendant" @slotchange=${this.handleSlotchange}></slot>
        </div>
      </nav>
    `;
  }
