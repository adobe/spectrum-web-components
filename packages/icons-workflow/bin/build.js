...
async function buildIcon(filepath) {
    ...
    const parsedSvg = $('svg')
        ...
        .replace('aria-label="..."', `aria-label="${ Case.capital(id) }"`);

    async function writeToTemplate(filename, filepath) {
...
async function main() {
    ...
    return fg(`assets/svg/**.svg`, {
        cwd: path.dirname(require.resolve('@adobe/spectrum-css-workflow-icons', { paths: [iconsDir, path.join(iconsDir, '..', '..')] })),
        absolute: true,
    }).then(async (icons) => {
