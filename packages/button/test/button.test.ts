...
describe("Button", () => {
  testForLitDevWarnings(
    async () => await fixture<Button>(html` <sp-button>Button</sp-button> `),
  );
  describe("dev mode", () => {
    ...
    it("loads default", async () => {
      const el = await fixture<Button>(html` <sp-button>Button</sp-button> `);
    ...
    it("has a stable/predictable `updateComplete`", async () => {
      const test = await fixture<HTMLDivElement>(html` <div></div> `);
    ...
    it("manages aria-label from pending state", async () => {
      const el = await fixture<Button>(html`
        <sp-button href="test_url" target="_blank" label="clickable" pending>
          Click me
        </sp-button>
      `);
