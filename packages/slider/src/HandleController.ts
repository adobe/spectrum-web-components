...
export class HandleController {
  ...
  public formattedValueForHandle(model: ModelValue): string {
    ...
    const _forcedUnit =
      handle._forcedUnit === '' ? this.host._forcedUnit : handle._forcedUnit;
  ...
  public get focusElement(): HTMLElement {
    ...
    if (
      !input ||
      (this.host.editable && !(input as InputWithModel).model.handle.dragging)
    ) {
  ...
  private getActiveHandleElements(): HandleComponents {
    ...
    const elements = this.getHandleElements(handleSlider) as HandleReference;
  ...
  private getHandleElements(sliderHandle: SliderHandle): HandleReference {
    if (!this.handleRefMap) {
      ...
      for (const inputNode of inputNodes) {
        ...
        const model = this.handles.get(handle.getAttribute('name') as string);
    ...
    const components = this.handleRefMap.get(sliderHandle) as HandleReference;
  ...
  public trackSegments(): [number, number][] {
    ...
    return values.map((value, index, array) => [value, array[index + 1] ?? 1]);
  }
  ...
  private updateModel(): void {
    ...
    const getRangeAndClamp = (index: number): RangeAndClamp => {
      ...
      const min =
        typeof handle.min === 'number' ? handle.min : (this.host.min as number);
      const max =
        typeof handle.max === 'number' ? handle.max : (this.host.max as number);
