...
export class Slider extends SizedMixin(ObserveSlotText(SliderHandle, ''), {
  ...
  public getAriaValueText: (values: Map<string, string>) => string = (
    ...
    if (valueArray.length === 2) return `${valueArray[0]} - ${valueArray[1]}`;
  ...
  private renderTicks(): TemplateResult {
    ...
    const tickCount = ((this.max as number) - (this.min as number)) / tickStep;
        ...
        ${ticks.map(
          (_tick, i) => html`
            <div class="tick">
              ${this.tickLabels
                ? html`
                    <div class="tickLabel">${i * tickStep + this.min}</div>
                  `
                : nothing}
            </div>
          `
   ...
   */
  private getOffsetWidth(fillStartValue: number, currentValue: number): number {
  ...
  protected override willUpdate(changed: PropertyValues): void {
    if (changed.has('value') && changed.has('fillStart')) {
      ...
      if (this.getAttribute('fill-start') === '') {
        this.centerPoint =
          (Number(this.max) - Number(this.min)) / 2 + Number(this.min);
