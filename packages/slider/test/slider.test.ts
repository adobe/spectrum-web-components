...
describe("Slider", () => {
  testForLitDevWarnings(
    async () =>
      await fixture<Slider>(html` <sp-slider label="Slider"></sp-slider> `),
  );
  it("loads", async () => {
  ...
  it("receives value from the outside", async () => {
    const el = await fixture<Slider>(html` <sp-slider max="20"></sp-slider> `);
  ...
  it("accepts pointer events", async () => {
    let pointerId = -1;
    const el = await fixture<Slider>(html` <sp-slider></sp-slider> `);
  ...
  it("will `trackPointerDown` on `#controls`", async () => {
    ...
    const controls = el.shadowRoot.querySelector("#controls") as HTMLDivElement;
    const handle = el.shadowRoot.querySelector(".handle") as HTMLDivElement;
  ...
  it("can be disabled", async () => {
    let pointerId = -1;
    const el = await fixture<Slider>(html` <sp-slider disabled></sp-slider> `);
  ...
  it("accepts pointermove events in separate interactions", async () => {
    ...
    expect(el.dragging, "is dragging").to.be.true;
    expect(el.classList.contains("handle-highlight"), "not highlighted").to.be
      .false;
  ...
  it("renders fill from the centerPoint of the track when fill-start has no value", async () => {
    ...
    await nextFrame();
    await nextFrame();
    const fillElement = el.shadowRoot.querySelector(".fill") as HTMLDivElement;
  ...
  it("supports units not included in Intl.NumberFormatOptions", async () => {
    ...
    expect(shadowRoot.querySelector('input#input-0[aria-valuetext="5px"]')).to
      .exist;
    expect(shadowRoot.querySelector('input#input-1[aria-valuetext="250px"]')).to
      .exist;
  ...
  it('warns in Dev Mode when `max="next"` is leveraged on last handle', async () => {
    ...
    expect(spyCall.args.at(0).includes("next"), 'confirm "next" in message').to
      .be.true;
    expect(spyCall.args.at(-1), "confirm `data` shape").to.deep.equal({
      data: {
        localName: "sp-slider",
  ...
  it("builds both handles from a <template>", async () => {
    ...
    const createdHandles =
      el.querySelector("sp-slider")?.shadowRoot.querySelectorAll(".handle") ||
      [];
