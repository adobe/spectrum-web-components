...
describe("SplitView", () => {
  ...
  it("loads default (horizontal) split-view accessibly", async () => {
    ...
    const gripper = el.shadowRoot.querySelector("#gripper") as HTMLDivElement;

    expect(gripper).to.be.null;
    const splitter = el.shadowRoot.querySelector("#splitter") as HTMLDivElement;
  ...
  it("use auto height in primary pane", async () => {
    ...
    const el = await fixture<SplitView>(html`
      ...
      >
        <div>
          First panel Lorem Ipsum is simply dummy text of the printing and
  ...
  it("allows a custom label when resizable if specified", async () => {
    ...
    let splitter = el.shadowRoot.querySelector("#splitter") as HTMLDivElement;
