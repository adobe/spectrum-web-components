...
export class NumberField extends TextfieldBase {
  ...
  protected override handleInput(event: InputEvent): void {
    ...
    const nextSelectStart =
      (selectionStart || currentLength) - (currentLength - previousLength);
   ...
   * This method initializes the number formatter if it is not already created.
   * It handles different formatting options and ensures that the formatter is
   * created with the correct language and options.
   *
   * @returns The number formatter instance.
   */
  protected get numberFormatter(): NumberFormatter {
    // Check if the number formatter or the focused number formatter is not initialized
    if (!this._numberFormatter || !this._numberFormatterFocused) {
      ...
      // If the style is not 'unit', set the style in formatOptionsNoUnit
      if (style !== "unit") {
        (formatOptionsNoUnit as Intl.NumberFormatOptions).style = style;
      }

      // Create a focused number formatter with the language and format options without unit
      ...
      try {
        // Try to create a number formatter with the full format options
        this._numberFormatter = new NumberFormatter(
          this.languageResolver.language,
          this.formatOptions,
        );
        this._forcedUnit = "";
        this._numberFormatter.format(1); // Test the formatter with a sample value
      } catch (error) {
        // If an error occurs and the style is 'unit', set the forced unit
        if (style === "unit") {
          this._forcedUnit = unit as string;
        }

        // Fallback to the focused number formatter
        this._numberFormatter = this._numberFormatterFocused;
      }
    }

    // Return the appropriate formatter based on the focused state
        ...
        : html`
            ...
            >
              <sp-infield-button
                ...
                this.readonly ||
                (typeof this.max !== "undefined" && this.value === this.max)}
                ?quiet=${this.quiet}
              >
