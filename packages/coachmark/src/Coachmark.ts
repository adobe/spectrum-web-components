...
export class Coachmark extends Popover {
  ...
  private renderHeader(): TemplateResult {
    ...
    if (!hasTitle && !hasModifier && !hasShortcut) {
      return html` <div class="title"><slot name="title"></slot></div> `;
    }

    return html`
      ${hasTitle
        ? html` <div class="title">${this.content?.title}</div> `
        : nothing}
      ${hasModifier || hasShortcut
        ? html`
            <kbd class="keys spectrum-Body spectrum-Body--sizeS">
              ${hasModifier
                ? join(
                    this.modifierKeys?.map((k) => this.renderModifier(k)),
                    this.renderJoiner(),
                  )
                : nothing}
              ${hasShortcut && hasModifier ? this.renderJoiner() : nothing}
              ${hasShortcut
                ? this.renderModifier(this.shortcutKey!, "shortcut")
                : nothing}
            </kbd>
          `
        : nothing}
  ...
  private renderContent(): TemplateResult {
    ...
    return html` <div>${this.content?.description}</div> `;
  }
    ...
    return html`
      ...
      <div class="header">
        ...
        <div class="static-item">
          ${when(this.secondaryCTA && this.primaryCTA, this.renderActionMenu)}
        </div>
      </div>

      <div class="content">${this.renderContent()}</div>
      <div class="footer">
        ${when(this.totalSteps && this.totalSteps > 1, this.renderSteps)}
        ${this.renderButtons()}
      </div>
    `;
  }
