...
describe("Coachmark", () => {
  ...
  it("displays the slotted content as `.title`", async () => {
    ...
    const headingSlot = root.querySelector('[name="title"]') as HTMLSlotElement;
  ...
  it("if in tour coachmark loads with pagination with previous, next buttons and action menu", async () => {
    ...
    const stepCount = el.shadowRoot.querySelector('span[aria-live="polite"]');
  ...
  it("renders with shortcut", async () => {
    ...
    const shortcutKey = el.shadowRoot.querySelector('span[type="shortcut"]');
