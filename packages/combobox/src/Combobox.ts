...
export class Combobox extends Textfield {
  ...
  protected override shouldUpdate(
    ...
    if (changed.has("value")) {
      ...
      this.itemValue =
        this.availableOptions.find((option) => option.itemText === this.value)
          ?.value ?? "";
    }
    ...
    return html`
      ...
      ${this.value
        ? html`
          ...
          `
        : html` <span hidden id="applied-label">${appliedLabel}</span> `}
    `;
  }
  ...
  protected override updated(
    ...
    if (changed.has("activeDescendant")) {
      ...
      if (
        ...
        typeof (this.activeDescendant as MenuItem).focused !== "undefined"
      ) {
  ...
  protected override async getUpdateComplete(): Promise<boolean> {
    ...
    const list = this.shadowRoot.querySelector("#listbox") as HTMLUListElement;
