...
describe("Combobox Data", () => {
  ...
  it("accepts options as html", async () => {
    ...
    const processedOptions = el.availableOptions.map(({ value, itemText }) => ({
      value,
      itemText,
    }));
  ...
  it("accepts additional options as html", async () => {
    ...
    let processedOptions = el.availableOptions.map(({ value, itemText }) => ({
      value,
      itemText,
    }));
  ...
  it("accepts updated value as html", async () => {
    ...
    const option1 = el.querySelector("sp-menu-item:first-of-type") as MenuItem;
