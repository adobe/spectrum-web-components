...
describe.only('ProgressBar dev mode', () => {
    ...
    it('warns in Dev Mode when accessible attributes are not leveraged', async () => {
        ...
        expect(consoleWarnStub.called, 'confirm stub called').to.be.true;
    ...
    it('warns in Dev Mode for deprecated usage of over-background', async () => {
        const el = await fixture<ProgressBar>(html`
            <sp-progress-bar
                progress="50"
                over-background
        ...
        const spyCall = consoleWarnStub.getCall(0);
        expect(
            (spyCall.args.at(0) as string).includes('DEPRECATION'),
