...
describe("sp-overlay", () => {
  ...
  describe('[type="modal"]', () => {
    ...
    describe("interaction with other non-ancestor overlays", function () {
      beforeEach(async function () {
        ...
        this.modal = this.fixture.querySelector('[type="modal"]') as Overlay;
        this.page = this.fixture.querySelector('[type="page"]') as Overlay;
        this.hint = this.fixture.querySelector('[type="hint"]') as Overlay;
        this.auto = this.fixture.querySelector('[type="auto"]') as Overlay;
        this.manual = this.fixture.querySelector('[type="manual"]') as Overlay;
  ...
  describe('[type="hint"]', () => {
    ...
    it("stays open when pointer enters overlay from trigger element", async () => {
      ...
      const overlay = test.querySelector("sp-overlay") as unknown as Overlay;
    ...
    it("stays open when pointer enters overlay from trigger element: self managed", async () => {
      const button = await styledFixture(html`
        <sp-button>
          This is a button.
          <sp-tooltip self-managed placement="bottom"> Help text. </sp-tooltip>
        </sp-button>
      `);
  ...
  describe('[type="auto"]', () => {
    ...
    it("does not close when clicking a Slider track in the Overlay", async function () {
      const test = await fixture(html` <div>${withSlider()}</div> `);
      const el = test.querySelector("sp-overlay") as Overlay;
      const button = test.querySelector("sp-button") as Button;
      const slider = el.querySelector("sp-slider") as Slider;
      const track = slider.shadowRoot.querySelector("#track") as HTMLDivElement;
      ...
      slider.track.releasePointerCapture = (id: number) => (pointerId = id);
