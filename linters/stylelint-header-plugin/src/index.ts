...
const messages = ruleMessages(ruleName, {
    ...
    outdated: (found: string, expected: string) =>
 ...
 */
function formatHeaderComment(header: string): {
    text: string;
    raws: { left: string; right: string };
} {
    return {
        text: header
            .split('\n')
...
/**
 * Options for the header rule.
 */
interface Options {
    /**
     * Percentage threshold (0-1) for determining if a comment is similar enough
     * to be considered a header that needs updating. Default: 0.8
     */
    headerDetectionThreshold?: number;
}

const ruleFunction: Rule<string, Options> = (pathOrString, options = {}) => {
    return (root, result) => {
        const validOptions = validateOptions(
            ...
            {
                actual: pathOrString,
                possible: [(x: unknown) => typeof x === 'string'],
            },
            {
                optional: true,
                actual: options,
        ...
        const sourceFile =
            ...
            (root.source?.input as { from?: string })?.from;
        ...
        const opts = result.opts as Record<string, unknown>;
        ...
        // Find the first comment in the file and check if it's a header
        interface HeaderCheckResult {
            existingHeaderComment: Comment | null;
            isExactMatch: boolean;
            similarity: number;
        }

        const checkResult: HeaderCheckResult = {
