...
For the `window.__swc.warn()` call format and the `{ level: 'deprecation' }` option, follow the **Deprecation warnings** section of [`CONTRIBUTOR-DOCS/for-contributors/style-guide/typescript/debug-validation.md`](../../../CONTRIBUTOR-DOCS/for-contributors/style-guide/typescript/debug-validation.md). Key rules from that guide:
