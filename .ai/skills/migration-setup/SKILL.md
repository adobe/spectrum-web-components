...
3. **Run the scaffolder** — from the repo root, run `yarn plop component "<name>"` (or `yarn plop component "<name>" --force` headlessly). This produces the deterministic gen2 skeleton in one step: the `core` base/types/index, the `swc` concrete class, registration, css, per-unit `.mdx`, stories, and test files — all matching the `badge` reference layout. It also wires the `@spectrum-web-components/core` `exports` map (the `swc` package uses wildcard exports and needs no edit) and runs Prettier on the new files. See [`gen2/scaffolding/README.md`](../../../gen2/scaffolding/README.md). Do **not** re-create these files by hand; the generator is the source of truth for the skeleton.
