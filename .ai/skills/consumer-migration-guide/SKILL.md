...
- **Do not link to project-planning / `CONTRIBUTOR-DOCS` docs** from the guide. Those are maintainer-facing; consumers don't need them. Link only to public consumer docs (e.g. the 1st-gen README on npm or the 2nd-gen component Storybook page) when a link genuinely helps.
...
**2nd-gen supersedes 1st-gen.** Verify every claim — especially styling hooks and public custom property names — against the actual 2nd-gen source (`2nd-gen/packages/swc/components/[component-name]/` and `2nd-gen/packages/core/components/[component-name]/`). Do not carry 1st-gen conventions (e.g. `--mod-*` prefixes) into the guide unless the 2nd-gen implementation actually uses them.
