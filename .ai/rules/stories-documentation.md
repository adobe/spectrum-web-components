...
...consumer installation and import guidance when it is not generated by `DocsHeader`...
...
1. **Getting started** — consumer installation and import guidance when it is not generated by `DocsHeader`
2. **Anatomy** — `hideTitle` semantics: no per-story `###` heading
...
9. **Upcoming features** — `hideTitle` semantics; prose-only sections allowed (no `<Canvas>`). Positioned before the footer so it reads as forward-looking notes after the current API/behavior, not as a preamble before Options.
10. **API** — rendered by `<DocsFooter />`. Controllers ship a hand-authored `## API` ahead of `<DocsFooter />` with a Methods/Events/Options table; `<DocsFooter />` omits `<ApiTable />` when `meta.tags` contains `'controller'`.
11. **Appendix** — `hideTitle` semantics; prose-only.
12. **Feedback** — rendered by `<DocsFooter />`.
...
2. **Getting started** - Consumer installation and import guidance when it is not generated by `<DocsHeader />`
3. **Anatomy** - Both visual and technical structure (components and patterns)
4. **Usage** - How to instantiate and configure (controllers)
5. **Options** - Configuration, variants, sizes, styles
6. **States** - Different component states
7. **Behaviors** - Methods, events, automatic behaviors
8. **Accessibility** - Features and best practices
9. **Upcoming features** - Roadmap intent (prose only, no Canvas)
10. **API** - Rendered automatically by `<DocsFooter />`. Controllers hand-author a `## API` section first.
11. **Feedback** - Rendered automatically by `<DocsFooter />`.
...
- [ ] Sections appear in the canonical order (Getting started → Anatomy → Usage → Options → States → Behaviors → Accessibility → Full pattern → Upcoming features → API → Appendix → Feedback) — skip sections that do not apply
