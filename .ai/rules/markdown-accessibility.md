---
description: Markdown and MDX accessibility checks for documentation (descriptive links, alt text, heading hierarchy, plain language, list and emoji semantics, multimedia, and tables), based on GitHub's guidance for inclusive documentation.
paths:
  - 'CONTRIBUTOR-DOCS/**/*.md'
...
metadata:
  ...
  source-path: instructions/markdown-accessibility.instructions.md
...
When writing or reviewing documentation, check for the following accessibility issues. Flag violations and suggest fixes with clear explanations of the accessibility impact. Capitalization, punctuation, and code formatting follow the `text-formatting` rule.
...
- Use one top-level heading per document and follow the repository's document roots: contributor docs start with an H1 title; 1st-gen component READMEs start at `## Overview` (`component-readme` rule); per-unit MDX pages follow the section structure in the `stories-documentation` rule.
