  ...
  element.addEventListener(
    ...
    (event: Event) => {
      const closestDialog = ([...event.composedPath()] as HTMLElement[]).find(
        (el) => {
          return (
            el.classList?.contains('dialog') && el.part?.contains('dialog')
          );
        }
      );
      if (!closestDialog) {
...
> {
  ...
  return {
    total: result.bytes,
    js:
      result.breakdown.find((entry) => entry.types.includes('JS'))?.bytes || 0,
    dom:
      result.breakdown.find((entry) => entry.types.includes('DOM'))?.bytes || 0,
    shared:
      result.breakdown.find((entry) => entry.types.includes('Shared'))?.bytes ||
      0,
  };
}
