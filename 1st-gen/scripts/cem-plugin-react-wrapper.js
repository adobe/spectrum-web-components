...
async function genNextComponentSourceCode(modules, exclude) {
  const elements = modules.flatMap((m) => {
    return m.declarations
      .filter((d) => d.customElement && d.tagName && !exclude.includes(d.name))
      .map((d) => {
        return {
          name: d.name,
          tagName: d.tagName,
...
export async function generateIconWrapper(iconType) {
  ...
  for (let icon of icons) {
    ...
    await outputFile(
      resolve(__dirname, '..', `react/${iconType}/next/${componentName}.ts`),
  ...
  await outputFile(
    ...
    await prettier.format(genPackageJson(iconType, pkgName, pkgVersion, true), {
      parser: 'json',
      ...prettierConfig,
    })
  );
