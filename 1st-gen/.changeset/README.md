...
> **1st-gen only.** This folder holds changesets for `@spectrum-web-components/*` packages, released from `main`. It has its own independent `@changesets/cli` setup (this `README.md` + `config.json`), separate from gen2's. For `@adobe/spectrum-wc` / `@adobe/spectrum-wc-core` (gen2) changes, use [`gen2/.changeset/`](../../gen2/.changeset/README.md) (`yarn changeset:gen2`) instead.
