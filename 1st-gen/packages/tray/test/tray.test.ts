...
describe('Tray', () => {
  ...
  it('closes', async () => {
    ...
    const overlay = el.shadowRoot.querySelector('sp-underlay') as HTMLElement;
    overlay.click();
    await closed;
  ...
  describe('Dismiss helpers', () => {
    it('renders visually-hidden dismiss helpers when no buttons detected', async () => {
      ...
      const buttons = el.shadowRoot.querySelectorAll('.visually-hidden button');
    ...
    it('dismiss helper buttons trigger close when clicked', async () => {
      ...
      expect(dismissButton.getAttribute('aria-label')).to.equal('Dismiss');
