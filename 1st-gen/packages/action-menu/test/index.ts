...
export const testActionMenu = (mode: 'sync' | 'async'): void => {
  describe(`Action menu: ${mode}`, () => {
    ...
    it('does not alter submenu selection when top-level menu items are selected', async () => {
      ...
      expect(item1.selected, 'after clicking item1: item1 selected?').to.be
        .false;
      expect(itemA.selected, 'after clicking item1: itemA selected?').to.be
        .true;
      ...
      expect(item1.selected, 'after clicking itemB: item1 selected?').to.be
        .false;
      expect(item2.selected, 'after clicking itemB: item2 selected?').to.be
        .false;
      expect(itemA.selected, 'after clicking itemB: itemA selected?').to.be
        .false;
      expect(itemB.selected, 'after clicking itemB: itemB selected?').to.be
        .true;
      ...
      expect(item2.selected, 'after clicking item2: item2 selected?').to.be
        .false;
      expect(itemB.selected, 'after clicking item2: itemB selected?').to.be
        .true;
    ...
    it('should handle scroll event', async () => {
      const renderMenuItems = (): TemplateResult[] =>
        Array.from(
          { length: 30 },
          (_, i) => html`
            <sp-menu-item style="width: 100%;">Menu Item ${i + 1}</sp-menu-item>
          `
        );
