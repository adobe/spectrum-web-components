...
describe('Combobox accessibility', () => {
  ...
  describe('language and direction of parts', () => {
    ...
    it('keeps the input\'s dir on "auto" so typed-but-uncommitted RTL text still gets correct alignment', async () => {
      const el = await fixture<TestableCombobox>(html`
        <sp-combobox label="Language">
          <sp-menu-item value="he" lang="he" dir="rtl">עברית</sp-menu-item>
          <sp-menu-item value="en" lang="en">English</sp-menu-item>
      ...
      const input = el.shadowRoot.querySelector('#input') as HTMLInputElement;
      expect(input.getAttribute('dir'), 'dir is always "auto"').to.equal(
        'auto'
      );
      expect(
        getComputedStyle(input).direction,
        'default direction before typing'
      ).to.equal('ltr');

      // Typed text that doesn't match any option's itemText, so `itemValue`
      // (and thus `lang`) stay unresolved; `dir="auto"` should still derive
      // the direction live from the typed text alone.
      el.value = 'עב';
      await elementUpdated(el);

      expect(input.lang, 'lang stays unresolved for uncommitted text').to.equal(
        ''
      );
      expect(
        getComputedStyle(input).direction,
        'dir="auto" derives rtl from the typed text alone'
      ).to.equal('rtl');
      ...
      expect(
        getComputedStyle(input).direction,
        'dir="auto" reverts to ltr for latin-script typed text'
      ).to.equal('ltr');
    });
    it('keeps the input\'s button-clearance padding on its physical side even when its own dir="auto" direction flips', async () => {
      const el = await fixture<TestableCombobox>(html`
        <sp-combobox label="Language">
          <sp-menu-item value="he" lang="he" dir="rtl">עברית</sp-menu-item>
          <sp-menu-item value="en" lang="en">English</sp-menu-item>
      ...
      // Uncommitted Hebrew text flips #input's own dir="auto" direction to
      // rtl, but the host (and thus .button's position) stays ltr.
      el.value = 'עב';
      await elementUpdated(el);

      expect(
        getComputedStyle(input).direction,
        "input's own direction flips to rtl"
      ).to.equal('rtl');
      expect(
        getComputedStyle(input).paddingLeft,
        "padding-left stays anchored to the host's direction, not the input's own"
      ).to.equal(beforePaddingLeft);
      expect(
        getComputedStyle(input).paddingRight,
        "padding-right (clearing .button) stays anchored to the host's direction, not the input's own"
