...
export function runPickerTests(): void {
  ...
  describe('standard', () => {
    ...
    it('opens with visible focus on a menu item on `Space`', async function () {
      ...
      expect(firstItem.focused, 'should not be visually focused before opening')
        .to.be.false;
      ...
      expect(firstItem.focused, 'should be visually focused after opening').to
        .be.true;
    ...
    it('can have selection prevented', async () => {
      ...
      await changed;
      await elementUpdated(el);
      expect(preventChangeSpy.calledOnce, preventChangeSpy.callCount.toString())
        .to.be.true;
    ...
    it('quick selects on ArrowLeft/Right', async () => {
      ...
      await waitUntil(
        () =>
          (el as unknown as { menuItems: MenuItem[] }).menuItems.length === 6
      );
      ...
      it('tabs forward through the element', async function () {
        ...
        try {
          await Promise.race([
            focused,
            new Promise((_, reject) =>
              setTimeout(() => reject(new Error('Focus event timed out')), 5000)
            ),
          ]);
        } catch (error) {
  ...
  describe('initial value', function () {
    ...
    it.skip('scrolls selected into view on open', async () => {
      ...
      const lastItem = el.querySelector('sp-menu-item:last-child') as MenuItem;
      ...
      const getParentOffset = (item: HTMLElement): number => {
        const parentScroll = (item.assignedSlot?.parentElement as HTMLElement)
          .scrollTop;
