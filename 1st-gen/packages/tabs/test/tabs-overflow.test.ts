...
const renderTabsOverflow = async ({
  ...
  const tabsOverflow = theme.querySelector('sp-tabs-overflow') as TabsOverflow;
...
describe('TabsOverflow', () => {
  ...
  it('should scroll up to the last item and back in LTR', async () => {
    ...
    expect(tabsOverflow['overflowState'].canScrollLeft, 'initial: scroll left')
      .to.be.false;
    expect(
    ...
    expect(tabsOverflow['overflowState'].canScrollLeft, 'after: scroll left').to
      .be.true;
    expect(tabsOverflow['overflowState'].canScrollRight, 'after: scroll right')
      .to.be.false;

    await scrollToEnd(tabsContainer, LEFT_BUTTON_SELECTOR, 'ltr');

    expect(tabsOverflow['overflowState'].canScrollLeft, 'end: scroll left').to
      .be.false;
    expect(tabsOverflow['overflowState'].canScrollRight, 'end: scroll right').to
      .be.true;
  });
