    ...
    return [
      ...
      css`
        ...
        .icon[tabindex]:focus-visible {
          ...
          outline-offset: calc(
            var(--spectrum-alias-focus-ring-gap, var(--spectrum-spacing-50)) * 2
          );
        }
      `,
    ];
      ...
      <div class="search" part="search">
        ...
        >
          <sp-help-text slot="help-text">
            Showing ${matchingIcons.length} of ${this.filteredIcons.length}
            available icons.
          </sp-help-text>
        </sp-search>
      </div>
      ...
      ${this.iconset.map(
        (icon) => html`
          <bdo class="icon" dir="ltr">
            <sp-icon size="xl" name=${`${this.name}:${icon}`}></sp-icon>
            ${icon}
          </bdo>
        `
      )}
