...
describe('Submenu', () => {
  ...
  function selectsWithKeyboard(testData: SelectsWithKeyboardTest): void {
    it(`with keyboard: ${testData.dir}`, async function () {
      ...
      expect(this.rootItem.open, `rootItem open before ${testData.openKey}`).to
        .be.false;
      ...
      let submenuItem = this.el.querySelector('.submenu-item-1') as MenuItem;

      expect(this.rootItem.open, `rootItem open after ${testData.openKey}`).to
        .be.true;
      ...
      expect(this.rootItem.open, `rootItem open after ${testData.closeKey}`).to
        .be.false;
      ...
      expect(document.activeElement === submenuItem, `submenu active`).to.be
        .true;
      ...
      expect(this.submenuChanged.calledWith('Two'), 'submenu changed').to.be
        .true;
  ...
  function returnsFocusToRootWhenClosingSubmenu(): void {
    it('returns visible focus when submenu closed', async function () {
      ...
      await nextFrame();
      await nextFrame();
      expect(this.rootItem.active, 'menu with submenu is not active').to.be
        .false;
      expect(
        this.rootItem.focused,
  ...
  function persistsWhenMovingBetweenItemAndSubmenu(): void {
    it('stays open when mousing between menu item and submenu', async function () {
      ...
      const subItem = this.el.querySelector('.submenu-item-2') as MenuItem;
      const clickSpy = spy();
  ...
  describe('deep tree', () => {
    ...
    it('closes descendant menus when Menu Item in ancestor without a submenu is pointerentered', async function () {
      const rootMenu = this.el.querySelector('#submenu-item-1') as MenuItem;
    ...
    it('closes descendant menus when Menu Item in ancestor is clicked', async function () {
      const rootMenu1 = this.el.querySelector('#submenu-item-1') as MenuItem;
      const childMenu2 = this.el.querySelector('#submenu-item-2') as MenuItem;
      const ancestorItem = this.el.querySelector('#ancestor-item') as MenuItem;
