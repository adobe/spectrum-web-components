...
describe('Overlay Trigger - Hover and Click', () => {
  it('toggles open and closed on click', async () => {
    const el = await fixture<OverlayTrigger>(html`
      <overlay-trigger triggered-by="click hover">
        ...
        <sp-popover slot="click-content" dialog tip>Popover content</sp-popover>
        <sp-tooltip slot="hover-content" delayed>Tooltip content</sp-tooltip>
      </overlay-trigger>
    `);
  ...
  it('persists a hover overlay when clicking its trigger and closes the next highest overlay on the stack', async () => {
    ...
    await sendMouse({
      type: 'click',
      position: [rect1.left + rect1.width / 2, rect1.top + rect1.height / 2],
    });
    await opened;
    ...
    await sendMouse({
      type: 'move',
      position: [rect2.left + rect2.width / 2, rect2.top + rect2.height / 2],
    });
    await opened;
