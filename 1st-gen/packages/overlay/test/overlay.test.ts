...
describe('Overlays', () => {
  ...
  describe('shared fixture', () => {
          ...
          <div id="overlay-content">
            <sp-popover id="outer-popover" direction="bottom" tip>
              <sp-dialog no-divider>
                <div class="options-popover-content">A popover message</div>
                <sp-button id="outer-focus-target">Test 1</sp-button>
    ...
    it('opens hover overlay', async () => {
      ...
      await opened;
      await closed;
      expect(await isInteractive(clickOverlay), 'click overlay not interactive')
        .to.be.true;
  ...
  it('closes via events', async () => {
    const test = await fixture<HTMLDivElement>(html`
      <div>
        <sp-popover id="root">
          <sp-dialog dismissable>Some Content for the Dialog.</sp-dialog>
        </sp-popover>
      </div>
    `);
...
describe('Overlay - type="modal"', () => {
  ...
  it('opens children in the modal stack through shadow roots', async () => {
    ...
    const content = document.querySelector('popover-content') as PopoverContent;
  ...
  it('should not open hover overlay right after closing the click overlay using the mouse', async () => {
    const overlayTrigger = await fixture<OverlayTrigger>(clickAndHoverTarget());
  ...
  it('should prevent clicks on external elements when modal overlay is open', async () => {
    ...
    const el = await fixture<HTMLDivElement>(html`
      <div>
        ...
        <sp-overlay trigger="trigger@click" type="modal">
          <sp-popover>
            <sp-button id="internal-button">Internal Button</sp-button>
            <p>Modal content</p>
          </sp-popover>
        </sp-overlay>
    ...
    const externalButton = el.querySelector('#external-button') as HTMLElement;
...
describe('Overlay - timing', () => {
  it('manages multiple modals in a row without preventing them from closing', async () => {
    const test = await fixture<HTMLDivElement>(html`
      <div>
        <overlay-trigger id="test-1" placement="bottom" triggered-by="hover">
