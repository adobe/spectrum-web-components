...
class ScrollForcer extends HTMLElement {
  ...
  async setup(): Promise<void> {
    ...
    this.previousElementSibling?.addEventListener('sp-opened', this.doScroll);
    await nextFrame();
    await nextFrame();
...
class DefinedOverlayReady extends HTMLElement {
  ...
  async setup(): Promise<void> {
    ...
    this.overlayElement.addEventListener('sp-opened', this.handleTriggerOpened);
    await nextFrame();
    await nextFrame();
    button.click();
  }
  ...
  handleTriggerOpened = async (): Promise<void> => {
    ...
    this.popoverElement.addEventListener('sp-opened', this.handlePopoverOpen);
    await nextFrame();
    await nextFrame();
  ...
  return html`
    <overlay-trigger type="modal" triggered-by="click">
      ...
      <sp-dialog-wrapper
        ...
        @secondary=${(event: Event & { target: DialogWrapper }): void => {
          event.target.dispatchEvent(closeEvent);
        }}
  ...
  const contextMenuTemplate = (kind = ''): TemplateResult => html`
    <sp-popover
      ...
      @click=${(event: PointerEvent) => {
        if ((event.target as HTMLElement).localName === 'sp-menu-item') {
          event.target?.dispatchEvent(new Event('close', { bubbles: true }));
        }
      }}
    >
...
export const virtualElementDeclaratively = (
  ...
  const handleContextmenu = async (event: PointerEvent): Promise<void> => {
    ...
    const overlay = document.querySelector('sp-overlay:not([open])') as Overlay;
