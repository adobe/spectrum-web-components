    ...
    <sp-tabs selected="second">
      ...
      <sp-tab-panel value="first">
        <sp-action-group selects="single">
          ...
          <sp-action-button value="2">Single Button 2</sp-action-button>
          <sp-action-button value="3">Single Button 3</sp-action-button>
        </sp-action-group>
      </sp-tab-panel>
      <sp-tab-panel value="second">
        <sp-action-group selects="multiple">
          <sp-action-button value="1">Multiple Button 1</sp-action-button>
      ...
      <sp-tab-panel value="third">
        <sp-action-group>
          ...
          <sp-action-button selected value="3">None Button 3</sp-action-button>
        </sp-action-group>
      </sp-tab-panel>
    </sp-tabs>
  `);
...
describe('Action Menu inside of Action Group', () => {
  it('accurately manages the tabindex of all the elements', async () => {
    ...
    expect(actionButton2.tabIndex).to.equal(-1);
    expect(actionButton1.tabIndex).to.equal(-1);
    expect(actionMenu.shadowRoot?.querySelector('sp-action-button')).to.exist;
    expect(
      (actionMenu.shadowRoot?.querySelector('sp-action-button') as ActionButton)
        .tabIndex
    ).to.equal(0);
  });
});
