...
describe('Grid', () => {
  ...
  it('should accept focus when tabbing into the grid', async () => {
    ...
    expect(el.querySelector(el.focusableSelector) === document.activeElement).to
      .be.true;
  });
  ...
  it.skip('allows to tab in and out', async function () {
    ...
    const firstInput = test.querySelector('#first-input') as HTMLInputElement;
    const lastInput = test.querySelector('#last-input') as HTMLInputElement;
    const firstCard = grid.querySelector(grid.focusableSelector) as HTMLElement;
    const actionMenu = firstCard.querySelector('sp-action-menu') as HTMLElement;
  ...
  it('manages roving tabindex', async () => {
    ...
    focused = el.querySelector(`${el.focusableSelector}:nth-child(2)`) as Card;
    await elementUpdated(focused);
    expect(focused === document.activeElement).to.be.true;
    expect(focused.focused).to.be.true;
    ...
    focused = el.querySelector(`${el.focusableSelector}:nth-child(5)`) as Card;
    await elementUpdated(focused);
    expect(focused === document.activeElement).to.be.true;
    expect(focused.focused).to.be.true;
    ...
    focused = el.querySelector(`${el.focusableSelector}:nth-child(4)`) as Card;
    await elementUpdated(focused);
    expect(focused === document.activeElement).to.be.true;
    expect(focused.focused).to.be.true;
