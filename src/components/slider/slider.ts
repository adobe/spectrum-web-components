...
export class SpectrumSlider extends HTMLElement {
    ...
    private updateView() {
        const trackLeft = this.shadowRoot!.getElementById('track-left');
        const trackRight = this.shadowRoot!.getElementById('track-right');
        const handle = this.shadowRoot!.getElementById('handle');
        const valueLabel = this.shadowRoot!.getElementById('value');

        const leftWidth = `${this.trackProgress * 100}%`;
        const rightWidth = `${(1 - this.trackProgress) * 100}%`;
        const rightOffset = `calc(${this.trackProgress * 100}% + 8px)`;
        const handleOffset = `${this.trackProgress * 100}%`;

        trackLeft!.style.width = leftWidth;
        trackRight!.style.width = rightWidth;
        trackRight!.style.left = rightOffset;
        ...
        return `
            ...
            <div id="controls">
                <input type="range"
                      ...
                      max="${this.max}"
                  />
                <div class="track" id="track-left"></div>
                <div id="handle" class="hand"></div>
                <div class="track" id="track-right"></div>
                </div>
            </div>
        `;
    }
