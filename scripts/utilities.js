 ...
 */
async function getAllComponentNames({ ignore = [] } = {}) {
 ...
 */
async function validateComponentName(componentName) {
 ...
 */
async function findPackagePath(packageName) {
    if (!packageName) return null;

    // First try to resolve using Node's resolution
    const resolvedPath = await import.meta
        .resolve(`${packageName}/package.json`, import.meta.url)
        .then((url) => dirname(new URL(url).pathname))
        .catch(() => null);
    ...
    if (!packageName.includes('/')) {
        const scopedPath = await import.meta
            .resolve(
        ...
        if (scopedPath && existsSync(scopedPath)) {
            return scopedPath;
        }
    }
