...
export class CssTable extends SpectrumElement {
  ...
  private async copyText(text: string): Promise<void> {
    try {
      ...
      console.log(`unable to access clipboard: ${JSON.stringify(error)}`);
    }
  ...
  public renderItem = (item: Item): TemplateResult => {
    ...
    const colourName =
      splitColourName.charAt(0).toUpperCase() + splitColourName.substring(1);
    ...
    return html`
      <sp-table
        ...
        @sorted=${(event: CustomEvent<SortedEventDetails>): void => {
          ...
          items.sort(this.compareItems(sortKey as 'customVar', sortDirection));
          this.items = items;
        }}
      >
