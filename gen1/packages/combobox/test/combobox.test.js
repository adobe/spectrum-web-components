...
describe("Combobox", () => {
  ...
  describe("keyboard events", () => {
    ...
    it("moves carat to 0 with Home key", async () => {
      ...
      expect(el.activeDescendant).to.be.undefined;
      expect(el.shadowRoot.querySelector('[aria-selected="true"]')).to.be.null;
      expect(el.open).to.be.true;
    });
  ...
  describe("item selection", () => {
    it("sets the value when descendent is active and `enter` is pressed", async () => {
      ...
      await elementUpdated(el);
      expect(
        (_a = el.activeDescendant) == null ? void 0 : _a.value,
      ...
      await elementUpdated(el);
      expect(el.open, "open?").to.be.false;
      expect(el.activeDescendant, "activeDescendant after Enter?").to.be.undefined;
      expect(el.value, "value after enter").to.equal("Apple");
      expect(
        (_b = el.shadowRoot.querySelector('sp-menu-item[aria-selected="true"]')) == null ? void 0 : _b.id,
        "aria-selected"
      ).to.equal("apple");
    ...
    it("sets the value when an item is clicked", async () => {
      ...
      expect(el.activeDescendant, "initial activeDescendant").to.be.undefined;
      expect(el.open, "initially open?").to.be.false;
      const opened = oneEvent(el, "sp-opened");
      el.focusElement.click();
      await opened;
      const item = el.shadowRoot.querySelector("#cherry");
      ...
      expect(el.activeDescendant, "activeDescendant after item click").to.be.undefined;
    });
    it("reflects the selected value in menu on reopening", async () => {
      ...
      el.focusElement.click();
      await opened;
      const item = el.shadowRoot.querySelector('[value="banana"]');
      await elementUpdated(item);
      expect(el.open).to.be.true;
      ...
      expect(el.open).to.be.true;
      expect(
        (_b = (_a = el.shadowRoot.querySelector("sp-menu")) == null ? void 0 : _a.querySelector("[selected]")) == null ? void 0 : _b.textContent
    ...
    it("sets the value when an item is clicked programmatically", async () => {
      ...
      el.focusElement.click();
      await opened;
      const item = el.shadowRoot.querySelector('[value="cherry"]');
      await elementUpdated(item);
      expect(el.open).to.be.true;
      const itemValue = item.itemText;
      const closed = oneEvent(el, "sp-closed");
  ...
  describe("responds to value changes", () => {
    ...
    it("filters options when the value typed and is found", async () => {
      ...
      expect(el.open).to.be.false;
      expect(el.availableOptions.length).equal(12);
      expect((_a = el.options) == null ? void 0 : _a.length).equal(12);
      const opened = oneEvent(el, "sp-opened");
      el.click();
      await opened;
      let items = [...el.shadowRoot.querySelectorAll("#listbox sp-menu-item")];
      ...
      expect((_b = el.options) == null ? void 0 : _b.length).equal(12);
      items = [...el.shadowRoot.querySelectorAll("#listbox sp-menu-item")];
    ...
    it("filtered items only can be accessed by ArrowUp/Down events", async () => {
      ...
      await opened;
      await elementUpdated(el);
      expect((_a = el.activeDescendant) == null ? void 0 : _a.itemText).to.equal(el.value);
    });
  ...
  describe("disabled items", () => {
    ...
    it("disabled items should not be focusable using keyboard", async () => {
      ...
      await sendKeys({ press: "ArrowDown" });
      await elementUpdated(el);
      expect((_a = el.activeDescendant) == null ? void 0 : _a.itemText).to.equal("Algeria");
    });
  ...
  it("closes tooltip on button blur", async () => {
    var _a;
    const el = await fixture(withTooltip());
    await elementUpdated(el);
    const input1 = document.createElement("input");
    const input2 = document.createElement("input");
    const tooltipEl = el.querySelector("sp-tooltip");
  ...
  it("scrolls to fit window", async () => {
    ...
    await opened;
    expect(el.open).to.be.true;
    const menu = el.shadowRoot.querySelector('[role="listbox"]');
