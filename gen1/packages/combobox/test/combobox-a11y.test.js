...
describe("Combobox accessibility", () => {
  ...
  it('manages its "name" value with <sp-field-label>', async () => {
    ...
    expect(node, `node not available: ${JSON.stringify(snapshot, null, "  ")}`).to.not.be.null;
  });
  ...
  it("manages aria-activedescendant", async () => {
    ...
    const activeDescendant = el.shadowRoot.querySelector("#apple");
  ...
  it("manages aria-selected", async () => {
    ...
    let snapshot = await a11ySnapshot({});
    expect(
      findAccessibilityNode(snapshot, (node) => !!node.selected)
    ).to.be.null;
    const opened = oneEvent(el, "sp-opened");
    el.click();
    await opened;
  ...
  it("manages aria-expanded", async () => {
    ...
    let snapshot = await a11ySnapshot({});
    expect(
      findAccessibilityNode(snapshot, (node) => !!node.expanded)
    ).to.be.null;
    el.click();
    await elementUpdated(el);
    expect(el.open).to.be.true;
