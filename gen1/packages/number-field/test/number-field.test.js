...
function setupEventSpies(element) {
  const inputSpy = spy();
  const changeSpy = spy();
...
describe("NumberField", () => {
  ...
  describe("truncated value tooltip", () => {
    it("renders truncated value tooltip when neither valid nor invalid and value is clipped", async () => {
      ...
      await waitUntil(
        () => {
          var _a2;
          return ((_a2 = el.shadowRoot) == null ? void 0 : _a2.querySelector("#truncated-value-tooltip")) != null;
        },
        "Tooltip overlay (lazy-loaded) should appear when value is truncated"
      );
      const tooltipOverlay = (_a = el.shadowRoot) == null ? void 0 : _a.querySelector(
        "#truncated-value-tooltip"
      );
      expect(tooltipOverlay).to.exist;
      const tooltip = (_b = el.shadowRoot) == null ? void 0 : _b.querySelector(
        "#truncated-value-tooltip sp-tooltip"
      );
      expect((_c = tooltip == null ? void 0 : tooltip.textContent) == null ? void 0 : _c.trim()).to.equal(el.focusElement.value);
    });
    ...
    it("updates truncated value tooltip text for realtime deletions and insertions while focused", async () => {
      ...
      await waitUntil(
        () => {
          var _a;
          return ((_a = el.shadowRoot) == null ? void 0 : _a.querySelector("#truncated-value-tooltip")) != null;
        },
        "Tooltip overlay (lazy-loaded) should appear when value is truncated"
      );
      const tooltipText = () => {
        var _a, _b, _c;
        return (_c = (_b = (_a = el.shadowRoot) == null ? void 0 : _a.querySelector("#truncated-value-tooltip sp-tooltip")) == null ? void 0 : _b.textContent) == null ? void 0 : _c.trim();
      };
    ...
    it("renders truncated value tooltip when a fitting value grows to overflow during typing", async () => {
      ...
      await elementUpdated(el);
      expect(
        (_a = el.shadowRoot) == null ? void 0 : _a.querySelector("#truncated-value-tooltip"),
    ...
    it("keeps tooltip mounted while focused after value shrinks, then removes it on blur when value fits", async () => {
      ...
      await elementUpdated(el);
      expect(
        (_b = el.shadowRoot) == null ? void 0 : _b.querySelector("#truncated-value-tooltip"),
  ...
  describe("dispatched events", () => {
    ...
    it("many input, but one change", async () => {
      ...
      expect(changeSpy.callCount, "initial: no changeSpy.callCount").to.equal(
        0
      );
      await oneEvent(el, "input");
      ...
      expect(changeSpy.callCount, "second: no changeSpy.callCount").to.equal(0);
      await oneEvent(el, "input");
      ...
      expect(changeSpy.callCount, "third: no changeSpy.callCount").to.equal(0);
      await mouseMoveOver(buttonDown);
      expect(
        inputSpy.callCount,
        "fourth: inputSpy.callCount"
      ).to.be.greaterThanOrEqual(5);
      expect(changeSpy.callCount, "fourth: no changeSpy.callCount").to.equal(0);
    ...
    it("no change in committed value - using buttons", async () => {
      ...
      expect(changeSpy.callCount, "first: no changeSpy.callCount").to.equal(0);
      await oneEvent(el, "input");
      expect(el.value, "second: el.value").to.be.greaterThanOrEqual(52);
      expect(
        inputSpy.callCount,
  ...
  describe("Stepper UI interactions", () => {
    ...
    afterEach(() => {
      inputSpy == null ? void 0 : inputSpy.resetHistory();
      changeSpy == null ? void 0 : changeSpy.resetHistory();
      fixtureCleanup();
      resetMouse();
    });
  ...
  describe("manages `value` with `formatOptions`", () => {
    ...
    it("does not select all on click based `focus`", async function() {
      ...
      const el = await getElFrom(units({ value: 17 }));
      expect(el.value).to.equal(17);
      const inputElement = (_a = el.shadowRoot) == null ? void 0 : _a.querySelector(
        "input"
      );
      ...
      await waitUntil(
        () => {
          if (inputElement.selectionStart !== null && inputElement.selectionEnd !== null) {
            length = inputElement.selectionEnd - inputElement.selectionStart;
            return true;
          }
          return false;
        },
    ...
    it("selects all on `Tab` based `focus`", async function() {
      ...
      expect(length, `selection length`).to.equal(el.valueAsString.length);
    });
  ...
  describe("accessibility model", () => {
    it("applies a non-empty input label from associated field label text", async () => {
      ...
      `);
      const el = wrapper.querySelector("sp-number-field");
      await waitUntil(
        () => {
          var _a2, _b;
          return ((_b = (_a2 = el.shadowRoot) == null ? void 0 : _a2.querySelector(".input")) == null ? void 0 : _b.ariaLabel) === labelText;
        }
      );
      const input = (_a = el.shadowRoot) == null ? void 0 : _a.querySelector(".input");
