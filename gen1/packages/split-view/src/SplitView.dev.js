...
export class SplitView extends SpectrumElement {
  ...
  get splitterSize() {
    if (!this._splitterSize) {
      this._splitterSize = this.splitter && Math.round(
        parseFloat(
          window.getComputedStyle(this.splitter).getPropertyValue(this.vertical ? "height" : "width")
        )
      ) || SPLITTERSIZE;
    }
  ...
  render() {
    ...
    return html`
      <slot
        id=${ifDefined(this.resizable ? (_a = this.controlledEl) == null ? void 0 : _a.id : void 0)}
  ...
  onPointermove(event) {
    event.preventDefault();
    let pos = this.vertical || this.dir === "ltr" ? this.getPosition(event) - this.offset : this.offset - this.getPosition(event);
  ...
  onKeydown(event) {
    ...
    let direction = 0;
    const isLTRorVertical = getComputedStyle(this).direction === "ltr" || this.vertical;
    switch (event.key) {
      case "Home":
        event.preventDefault();
