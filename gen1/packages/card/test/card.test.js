...
describe("card", () => {
  ...
  it("applies block-size: 100% only to gallery and quiet variants", async () => {
    const variants = ["standard", "gallery", "quiet"];
    const cards = await Promise.all(
      variants.map(async (variant) => {
        ...
        return { variant, card: card.querySelector("sp-card") };
      })
    );
  ...
  it("loads - [horizontal]", async () => {
    const el = await fixture(Horizontal(Horizontal.args));
    await elementUpdated(el);
    await expect(el).to.be.accessible();
  });
  ...
  it("links in [href] do not pass their click", async () => {
    ...
    el.click();
    expect(clickSpy.callCount, "clickSpy.callCount after card click").to.equal(
      1
    );
    ...
    expect(clickSpy.callCount, "clickSpy.callCount after link click").to.equal(
      2
    );
  });
  ...
  it("displays the `heading` attribute as `.title`", async () => {
    ...
    expect(headingEl.textContent).to.contain(
      testHeading,
  ...
  it("displays the slotted content as `.title`", async () => {
    ...
    expect(h1Element.textContent).to.contain(
      testHeading,
  ...
  it("does not trigger click when scrolling on mobile", async () => {
    ...
    el.dispatchEvent(
      new PointerEvent("pointerdown", {
        ...
        pointerType: "touch",
        pressure: 0.5,
        // Add pressure for touch simulation
        bubbles: true,
        composed: true,
        cancelable: true
      })
    ...
    el.dispatchEvent(
      new PointerEvent("pointermove", {
        clientX: startX,
        clientY: startY + 50,
        // Move 50px down to simulate scroll
        pointerId: 1,
        pointerType: "touch",
        pressure: 0.5,
        bubbles: true,
