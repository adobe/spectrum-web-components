...
export const runOverlayTriggerTests = (type) => {
  describe(`Overlay Trigger - ${type}`, () => {
    describe("open/close", () => {
      ...
      OVERLAY_TYPES.map((type2) => {
        it(`opens a popover - [type="${type2}"]`, async function() {
          const outerTrigger = this.outerTrigger;
          outerTrigger.type = type2;
      ...
      it("blocks body scroll when modal overlay is opened and restores when closed", async function() {
        ...
        await closed;
        expect(document.body.style.overflow).to.equal(originalBodyOverflow);
      });
      it("actually prevents page scrolling when modal overlay is open and restores scrolling when closed", async function() {
        ...
        longContent.style.height = "200vh";
