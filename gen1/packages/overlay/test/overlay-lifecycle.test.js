...
describe("Overlay Trigger - accessible hover content management", () => {
  ...
  it("gardens `aria-describedby` in its target", async () => {
    ...
    const tooltip = el.querySelector('[slot="hover-content"]');
    await elementUpdated(el);
    await nextFrame();
    await nextFrame();
    await waitUntil(
  ...
  describe("describeTrigger escape hatch", () => {
    it('does not set aria-describedby on trigger when describeTrigger is "none" (hover)', async () => {
      ...
      `);
      const trigger = el.querySelector("#hover-trigger");
      const overlay = el.querySelector("#hover-overlay");
      overlay.triggerElement = trigger;
      await elementUpdated(overlay);
      await nextFrame();
      await nextFrame();
      const sentinel = el.querySelector("#focus-sentinel");
      sentinel.focus();
    ...
    it('does not set aria-describedby on trigger when describeTrigger is "none" (longpress)', async () => {
      ...
      `);
      const trigger = el.querySelector("#longpress-trigger");
      const overlay = el.querySelector("#longpress-overlay");
      overlay.triggerElement = trigger;
      await elementUpdated(overlay);
      await nextFrame();
      await nextFrame();
