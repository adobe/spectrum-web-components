...
describe("ProgressBar", () => {
  ...
  it("accepts label from `slot`", async () => {
    const el = await fixture(html`
      <sp-progress-bar role="progressbar">Label From Slot</sp-progress-bar>
    `);
    await elementUpdated(el);
  ...
  it("handles label attribute changes", async () => {
    const el = await fixture(html`
      <sp-progress-bar label="label" indeterminate>content</sp-progress-bar>
    `);
    await elementUpdated(el);
  ...
  describe("dev mode", () => {
    ...
    it("warns in devMode for deprecated usage of over-background", async () => {
      ...
      const spyCall = consoleWarnStub.getCalls().find(
        (call) => {
          var _a;
          return (_a = call.args[0]) == null ? void 0 : _a.includes("over-background");
        }
      );
    ...
    it("warns in Dev Mode when accessible attributes are not leveraged", async () => {
      ...
      const spyCall = consoleWarnStub.getCalls().find(
        (call) => {
          var _a, _b;
          return ((_b = (_a = call.args[call.args.length - 1]) == null ? void 0 : _a.data) == null ? void 0 : _b.type) === "accessibility";
        }
      );
