...
export class ColorWheel extends Focusable {
  ...
  isPointerInRing(event) {
    ...
    return this._trackWidth > 0 ? distance >= outerRadius - this._trackWidth && distance <= outerRadius : distance <= outerRadius;
  }
  ...
  calculateStyleData() {
    ...
    const borderWidth = parseFloat(styles2.getPropertyValue("--_border-width"));
    const trackWidth = parseFloat(styles2.getPropertyValue("--_track-width"));
    ...
    const translateX = (this.dir === "ltr" ? 1 : -1) * (radius - trackWidth / 2) * Math.cos(this.value * Math.PI / 180);
  ...
  cacheLayoutData() {
    ...
    this._trackWidth = parseFloat(getComputedStyle(this).getPropertyValue("--_track-width")) || 0;
  }
