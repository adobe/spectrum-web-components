...
describe("Checkbox", () => {
    ...
    testFixture = await fixture(html`
      <div>
        <div id="test-checkbox">
          ...
          <sp-checkbox id="checkbox1" tabindex="2" checked>Check 1</sp-checkbox>
          ...
          <sp-checkbox id="checkbox4" tabindex="0">Check 4</sp-checkbox>
          <sp-checkbox id="checkbox5" tabindex="-1">Check 5</sp-checkbox>
        </div>
      </div>
    `);
  });
  ...
  it("`indeterminate, not checked` becomes `checked` on click", async () => {
    const el = await fixture(html`
      <sp-checkbox .indeterminate=${true}>indeterminate, checked</sp-checkbox>
    `);
    expect(el.checked).to.be.false;
    expect(el.indeterminate).to.be.true;
    const inputEl = inputForCheckbox(el);
  ...
  it("updates partialCheckmark icons in response to size", async function() {
    ...
    const getPartialCheckmarkLocalName = () => {
      return el.shadowRoot.querySelector("#partial-checkmark").localName;
    };
    expect(el.size).to.equal("m");
