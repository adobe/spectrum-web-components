...
describe("ColorSlider", () => {
  ...
  it("accepts pointer events in right to left context", async () => {
    const el = await fixture(html`
      <sp-color-slider
        dir="rtl"
    ...
    expect(el.sliderHandlePosition).to.equal(0);
    const clickCloseToRightEdge = rect.right - 10;
    el.gradient.dispatchEvent(
      new PointerEvent("pointerdown", {
        pointerId: 1,
    ...
    const clickCloseToLeftEdge = rect.left + 10;
    el.handle.dispatchEvent(
      new PointerEvent("pointermove", {
        pointerId: 1,
  ...
  colorFormats.map((format) => {
    it(`maintains \`color\` format as ${format.name}`, async () => {
      ...
      if (format.name.startsWith("Hex")) {
        ...
        expect(
          el.color,
          `${JSON.stringify(el.color)} ${JSON.stringify(format.color)}`
