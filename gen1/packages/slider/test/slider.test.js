...
describe("Slider", () => {
  ...
  it("stops propagation of arrow key events but allows other keys to propagate", async () => {
    ...
    expect(keydownSpy.callCount, "ArrowUp event should not propagate").to.equal(
      0
    );
  ...
  it("goes [disabled] while dragging", async () => {
    ...
    await sendMouse({
      type: "move",
      position: [0, handleBoundingRect.top + handleBoundingRect.height / 2]
    });
    expect(el.value).to.equal(63);
  });
  ...
  it("accepts pointermove events - [step=0]", async () => {
    ...
    await sendMouse({
      type: "move",
      position: [125, handleBoundingRect.y + handleBoundingRect.height / 2]
    });
    await inputEvent;
  ...
  it("renders fill from the centerPoint of the track when fill-start has no value", async () => {
    ...
    await nextFrame();
    await nextFrame();
    const fillElement = el.shadowRoot.querySelector(".fill");
  ...
  it("supports units not included in Intl.NumberFormatOptions", async () => {
    ...
    expect(shadowRoot.querySelector('input#input-1[aria-valuetext="250px"]')).to.exist;
    el = await fixture(html`
  ...
  it("resets to default value on double click after moving pointer", async () => {
    ...
    await sendMouse([
      {
        type: "move",
        position: [150, handleBoundingRect.y + handleBoundingRect.height + 100]
      },
      {
        type: "up"
      }
  ...
  describe("dev mode", () => {
    ...
    it("warns in Dev Mode when using the default slot for text labels", async () => {
      ...
      expect(
        spyCall.args[0].includes("default slot"),
    ...
    it('warns in Dev Mode when `min="previous"` is leveraged on first handle', async () => {
      ...
      const spyCall = consoleWarnStub.getCall(0);
      expect(
        spyCall.args[0].includes("previous"),
    ...
    it("warns in Dev Mode when multi-handle slider has handles without labels", async () => {
      ...
      expect(
        spyCall.args[0].includes("2 handle(s)"),
    ...
    it("does not warn when multi-handle slider has labels on all handles", async () => {
      ...
      `);
      await elementUpdated(el);
      const labelWarningCalled = consoleWarnStub.getCalls().some(
        (call) => call.args[0].includes("label")
      );
  ...
  describe("Multi-handle value tooltips", () => {
    it('shows value tooltips for multi-handle sliders when label-visibility is "none"', async () => {
      ...
      expect((_a = tooltips[0].textContent) == null ? void 0 : _a.trim()).to.equal("25");
      expect((_b = tooltips[1].textContent) == null ? void 0 : _b.trim()).to.equal("75");
    });
    it('shows value tooltips for multi-handle sliders when label-visibility is "text"', async () => {
      ...
      const tooltips = el.shadowRoot.querySelectorAll(".value-tooltip");
      expect(tooltips.length).to.equal(2);
      expect((_a = tooltips[0].textContent) == null ? void 0 : _a.trim()).to.equal("30");
      expect((_b = tooltips[1].textContent) == null ? void 0 : _b.trim()).to.equal("80");
    });
    ...
    it("updates value tooltip content when handle value changes", async () => {
      ...
      `);
      await elementUpdated(el);
      const minHandle = el.querySelector("#min-handle");
      minHandle.value = 40;
      await elementUpdated(el);
      const tooltips = el.shadowRoot.querySelectorAll(".value-tooltip");
      expect((_a = tooltips[0].textContent) == null ? void 0 : _a.trim()).to.equal("40");
    });
    ...
    it("shows formatted value in tooltip when formatOptions is set", async () => {
      ...
      await elementUpdated(el);
      const tooltips = el.shadowRoot.querySelectorAll(".value-tooltip");
      expect((_a = tooltips[0].textContent) == null ? void 0 : _a.trim()).to.include("%");
      expect((_b = tooltips[1].textContent) == null ? void 0 : _b.trim()).to.include("%");
    });
  });
});
