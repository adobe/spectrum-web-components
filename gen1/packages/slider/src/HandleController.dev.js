...
export class HandleController {
  constructor(host) {
    ...
    this.model = [];
    this.handleOrder = [];
    this._hasWarnedAboutMissingLabels = false;
  ...
  calculateHandlePosition(event, model) {
    ...
    const dir = (_a = getComputedStyle(this.host).direction) != null ? _a : "ltr";
    const directionalOffset = dir === "ltr" ? offset - minOffset : size - (offset - minOffset);
  ...
  renderHandle(model, index, zIndex, isMultiHandle) {
    ...
    const showValueTooltip = isMultiHandle && (this.host.labelVisibility === "none" || this.host.labelVisibility === "text");
    const classes = {
      handle: true,
  ...
  trackSegments() {
    ...
    return values.map((value, index, array) => {
      var _a;
      return [value, (_a = array[index + 1]) != null ? _a : 1];
    });
  }
  updateModel() {
    ...
    if (isMultiHandle && !this._hasWarnedAboutMissingLabels) {
      const handlesWithoutLabels = handles.filter(
        (handle) => {
          var _a;
          return handle !== this.host && !((_a = handle.label) == null ? void 0 : _a.length);
        }
      );
