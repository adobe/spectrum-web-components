...
export const testActionMenu = (mode) => {
  describe(`Action menu: ${mode}`, () => {
    ...
    it("has menuitems in accessibility tree", async () => {
      ...
      const button = findAccessibilityNode(
        snapshot,
        (node) => node.name === "More Actions"
      );
      const menu = findAccessibilityNode(
        snapshot,
        (node) => node.role === "menu"
      );
      const deselect = findAccessibilityNode(
        snapshot,
        (node) => node.role === "menuitem" && node.name === "Deselect"
      );
      const workPath = findAccessibilityNode(
        snapshot,
        (node) => node.role === "menuitem" && node.name === "Make Work Path" && node.disabled
      );
    ...
    it("scroll event does not reach the action menu host when inside a parent overlay", async () => {
      ...
      const actionMenu = test.querySelector("sp-action-menu");
      ...
      const menu = actionMenu.optionsMenu;
