...
const _Theme = class _Theme extends HTMLElement {
  ...
  get styles() {
    const themeKinds = [..._Theme.themeFragmentsByKind.keys()];
...
_Theme.CORE_VERSION = coreVersion;
...
function checkForIssues(instance, system, color, scale, themeFragmentsByKind) {
  if (true) {
    ...
    const checkForAttribute = (name, resolvedValue, actualValue) => {
      ...
      } else if (!((_a = themeFragmentsByKind.get(name)) == null ? void 0 : _a.get(resolvedValue + (name === "system" ? "" : systemModifier)))) {
        issues.push(
