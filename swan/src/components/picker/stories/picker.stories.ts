...
import { spreadProps } from '../../../utils/test-utils.js';
import { areIconsPresent, isOverlayOpen } from '../../../utils/test-utils.js';
const states: {
    id: string;
    label: string;
}[] = [
...
export const slottedLabel = (args: StoryArgs): TemplateResult => {
    return html`
        <swan-picker @change=${handleChange(args)} ${spreadProps(args)}>
    ...
    return html`
        ...
        <sp-button
            @click=${() => {
                ...
                const picker = document.querySelector('swan-picker');
