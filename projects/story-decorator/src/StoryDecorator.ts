...
export class StoryDecorator extends SpectrumElement {
    ...
    public get backgroundStyle() {
        const hasStaticWhite = this.matches(':has([static-color="white"])');
        const hasStaticBlack = this.matches(':has([static-color="black"])');

        return `background: ${hasStaticWhite ? 'linear-gradient(45deg, rgb(64 0 22), rgb(14 24 67))' : hasStaticBlack ? 'linear-gradient(45deg, rgb(255 241 246), rgb(238 245 255))' : 'var(--spectrum-background-base-color)'}`;
    }
