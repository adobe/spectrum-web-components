...
export class LayoutElement extends LitElement {
    ...
    if (!this.alerts.has(target)) {
      ...
      this.alerts.set(target, {
        ...
        element: (count: number, message: string) => {
          ...
          return html`
            <sp-toast .timeout=${count} variant="info" @close=${close} open>
  ...
  private get settingsContent(): TemplateResult {
    ...
    return html`
      <sp-underlay class="scrim" @close=${this.closeSettings}></sp-underlay>
  ...
  private get manageTheme(): TemplateResult {
    ...
    return html`
      <div class="manage-theme" role="form" aria-label="Settings">
        ...
        <div class="theme-control">
          <sp-field-label for="theme-direction"> Direction </sp-field-label>
          <sp-picker
            id="theme-direction"
            label="Direction"
            quiet
  ...
  private get header(): TemplateResult {
    const triggerOptions: Partial<OverlayTriggerOptions> = {
      ...
      insertionOptions: {
        el: () => this.shadowRoot?.querySelector("#body") as HTMLElement,
        where: "afterbegin",
      },
    };
    ...
    return html`
      ...
      >
        ...
        <div id="body">
          ...
          <div id="page" @alert=${this.addAlert} @copy-text=${this.copyText}>
  ...
  override updated(changes: PropertyValues) {
    ...
    if (changes.has("color")) {
      ...
      examples.forEach((example) => {
        example.codeTheme = this.color as "dark" | "light";
      });
