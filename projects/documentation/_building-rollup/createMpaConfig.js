...
async function createMpaConfig(options) {
    ...
    if (!userOptions.inputGlob) {
        throw new Error(
            'Cannot generate multi build outputs when html plugin is disabled'
        );
    }
    ...
    for (const inputPath of htmlFiles) {
        ...
        const htmlPlugin = pluginWithOptions(html, userOptions.html, {
            ...
            transform: [
                userOptions.injectServiceWorker &&
                    applyServiceWorkerRegistration,
                userOptions.html.inputPathBoundTransform &&
                    userOptions.html.inputPathBoundTransform(inputPath),
            ].filter(isFalsy),
            inject: false,
            inputPath,
            name,
    ...
    if (userOptions.legacyBuild) {
        ...
        for (const htmlPlugin of htmlPlugins) {
            ...
            basicConfig.output[1].plugins.push(
                htmlPlugin.addOutput('nomodule')
            );
        }
