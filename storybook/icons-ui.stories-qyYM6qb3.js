...
var iconsUi_stories = {
  ...
  swc_vrt: {
    preload: async () => {
      await import('./icon-manifest-YsUnGbhQ.js');
    }
  }
};
const elements = ({ color, size }) => {
  const content = import('./icon-manifest-YsUnGbhQ.js').then(
    (iconManifest) => x`
            <icons-demo
                style="color: ${color}"
                size=${size}
...
const Icons = ({ color, size }) => {
  const content = import('./index-lPYZgR96.js').then((icons) => {
    const iconTemplates = [];
