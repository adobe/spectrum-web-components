...
var iconsWorkflow_stories = {
  ...
  swc_vrt: {
    preload: async () => {
      await import('./icon-manifest-BAaaHZEZ.js');
    }
  }
};
const elements = ({ color, size }) => {
  const content = import('./icon-manifest-BAaaHZEZ.js').then(
    (iconManifest) => x`
            <icons-demo
                style="color: ${color}"
                size=${size}
...
const Icons = ({ color, size }) => {
  const content = import('./index-hbYA_yPp.js').then((icons) => {
    const iconTemplates = [];
