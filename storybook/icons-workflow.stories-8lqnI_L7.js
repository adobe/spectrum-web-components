...
var iconsWorkflow_stories = {
  ...
  swc_vrt: {
    preload: async () => {
      await import('./icon-manifest-8zyAkRZm.js');
    }
  }
};
const elements = ({ color, size }) => {
  const content = import('./icon-manifest-8zyAkRZm.js').then(
    (iconManifest) => x`
            <icons-demo
                style="color: ${color}"
                size=${size}
...
const Icons = ({ color, size }) => {
  const content = import('./index-4MJkaTmj.js').then((icons) => {
    const iconTemplates = [];
