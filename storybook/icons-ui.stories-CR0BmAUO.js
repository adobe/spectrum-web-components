...
var iconsUi_stories = {
  ...
  swc_vrt: {
    preload: async () => {
      await import('./icon-manifest-CzYVCMZs.js');
    }
  }
};
const elements = ({ color, size }) => {
  const content = import('./icon-manifest-CzYVCMZs.js').then(
    (iconManifest) => x`
            <icons-demo
                style="color: ${color}"
                size=${size}
...
const Icons = ({ color, size }) => {
  const content = import('./index-D3D_nbVf.js').then((icons) => {
    const iconTemplates = [];
