...
import { n, S as SpectrumElement, d as defineElement } from './define-element-xyi5NDDf.js';
...
var c=Object.defineProperty;var p=Object.getOwnPropertyDescriptor;var r=(u,n,t,e)=>{for(var o=e>1?void 0:e?p(n,t):n,s=u.length-1,l;s>=0;s--)(l=u[s])&&(o=(e?l(n,t,o):l(o))||o);return e&&o&&c(n,t,o),o};const toastVariants=["negative","positive","info","error","warning"];class Toast extends FocusVisiblePolyfillMixin(SpectrumElement){constructor(){super(...arguments);this.open=!1;this._timeout=null;this._variant="";this.countdownStart=0;this.nextCount=-1;this.doCountdown=t=>{this.countdownStart||(this.countdownStart=performance.now()),t-this.countdownStart>this._timeout?(this.shouldClose(),this.countdownStart=0):this.countdown();};this.countdown=()=>{cancelAnimationFrame(this.nextCount),this.nextCount=requestAnimationFrame(this.doCountdown);};this.holdCountdown=()=>{this.stopCountdown(),this.addEventListener("focusout",this.resumeCountdown);};this.resumeCountdown=()=>{this.removeEventListener("focusout",this.holdCountdown),this.countdown();};}static get styles(){return [o]}set timeout(t){const o=typeof t!==null&&t>0?Math.max(6e3,t):null,s=this.timeout;o&&this.countdownStart&&(this.countdownStart=performance.now()),this._timeout=o,this.requestUpdate("timeout",s);}get timeout(){return this._timeout}set variant(t){if(t===this.variant)return;const e=this.variant;toastVariants.includes(t)?(this.setAttribute("variant",t),this._variant=t):(this.removeAttribute("variant"),this._variant=""),this.requestUpdate("variant",e);}get variant(){return this._variant}renderIcon(t,e){switch(t){case"info":return x`
                    <sp-icon-info
                        label=${e||"Information"}
                        class="type"
                    ></sp-icon-info>
                `;case"negative":case"error":return x`
                    <sp-icon-alert
                        label=${e||"Error"}
                        class="type"
                    ></sp-icon-alert>
        ...
        `}updated(t){super.updated(t),t.has("open")&&(this.open?this.timeout&&this.startCountdown():this.timeout&&this.stopCountdown()),t.has("timeout")&&(this.timeout!==null&&this.open?this.startCountdown():this.stopCountdown());}}r([n({type:Boolean,reflect:!0})],Toast.prototype,"open",2),r([n({type:Number})],Toast.prototype,"timeout",1),r([n({type:String})],Toast.prototype,"variant",1),r([n({type:String,attribute:"icon-label"})],Toast.prototype,"iconLabel",2);
