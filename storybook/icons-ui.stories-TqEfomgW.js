...
var iconsUi_stories = {
  ...
  swc_vrt: {
    preload: async () => {
      await import('./icon-manifest-TgyqT0bj.js');
    }
  }
};
const elements = ({ color, size }) => {
  const content = import('./icon-manifest-TgyqT0bj.js').then(
    (iconManifest) => x`
            <icons-demo
                style="color: ${color}"
                size=${size}
...
const Icons = ({ color, size }) => {
  const content = import('./index-CtCrW5QQ.js').then((icons) => {
    const iconTemplates = [];
