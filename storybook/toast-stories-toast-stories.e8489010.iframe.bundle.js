...
`,__defProp=Object.defineProperty,__getOwnPropDesc=Object.getOwnPropertyDescriptor,__decorateClass=(decorators,target,key,kind)=>{for(var decorator,result=kind>1?void 0:kind?__getOwnPropDesc(target,key):target,i=decorators.length-1;i>=0;i--)(decorator=decorators[i])&&(result=(kind?decorator(target,key,result):decorator(result))||result);return kind&&result&&__defProp(target,key,result),result};const toastVariants=["negative","positive","info","error","warning"];class Toast extends((0,focus_visible_dev.p)(index_dev.wG)){constructor(){super(...arguments),this.open=!1,this._timeout=null,this._variant="",this.countdownStart=0,this.nextCount=-1,this.doCountdown=time=>{this.countdownStart||(this.countdownStart=performance.now()),time-this.countdownStart>this._timeout?(this.shouldClose(),this.countdownStart=0):this.countdown()},this.countdown=()=>{cancelAnimationFrame(this.nextCount),this.nextCount=requestAnimationFrame(this.doCountdown)},this.holdCountdown=()=>{this.stopCountdown(),this.addEventListener("focusout",this.resumeCountdown)},this.resumeCountdown=()=>{this.removeEventListener("focusout",this.holdCountdown),this.countdown()}}static get styles(){return[toast_css]}set timeout(timeout){const newTimeout=null!==typeof timeout&&timeout>0?Math.max(6e3,timeout):null,oldValue=this.timeout;newTimeout&&this.countdownStart&&(this.countdownStart=performance.now()),this._timeout=newTimeout,this.requestUpdate("timeout",oldValue)}get timeout(){return this._timeout}set variant(variant){if(variant===this.variant)return;const oldValue=this.variant;toastVariants.includes(variant)?(this.setAttribute("variant",variant),this._variant=variant):(this.removeAttribute("variant"),this._variant=""),this.requestUpdate("variant",oldValue)}get variant(){return this._variant}renderIcon(variant,iconLabel){switch(variant){case"info":return index_dev.qy`
          <sp-icon-info
...
//# sourceMappingURL=toast-stories-toast-stories.e8489010.iframe.bundle.js.map