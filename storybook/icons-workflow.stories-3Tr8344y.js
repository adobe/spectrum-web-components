...
var iconsWorkflow_stories = {
  ...
  swc_vrt: {
    preload: async () => {
      await import('./icon-manifest-aWMm7vKh.js');
    }
  }
};
const elements = ({ color, size }) => {
  const content = import('./icon-manifest-aWMm7vKh.js').then(
    (iconManifest) => x`
            <icons-demo
                style="color: ${color}"
                size=${size}
...
const Icons = ({ color, size }) => {
  const content = import('./index-DI2sclPz.js').then((icons) => {
    const iconTemplates = [];
