...
2. Open up any `*.stories.js` files inside the component's folder:
   ...
   c. In the parameters section, there are 2 important updates to make: - Add `chromatic: { disable: true },` to ensure it no longer runs regression tests. - Update the `status` type to `deprecated`:
   `json parameters: { chromatic: { disable: true }, status: { type: "deprecated" } }, `
