...
var iconsWorkflow_stories = {
  ...
  swc_vrt: {
    preload: async () => {
      await import('./icon-manifest-BfPni4zV.js');
    }
  }
};
const elements = ({ color, size }) => {
  const content = import('./icon-manifest-BfPni4zV.js').then(
    (iconManifest) => x`
            <icons-demo
                style="color: ${color}"
                size=${size}
...
const Icons = ({ color, size }) => {
  const content = import('./index-izgHfJaX.js').then((icons) => {
    const iconTemplates = [];
