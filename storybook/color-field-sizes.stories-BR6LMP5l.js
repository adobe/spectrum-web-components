import './sp-field-label-DvdKN6Ly.js';
import './sp-help-text-j2yJecXO.js';
import { C as ColorFieldMarkup } from './template-CgrblWsw.js';
import './random-id-BST1Puzz.js';
import './custom-tag-B5IH9PTE.js';
import './IconBase-q22s6ZK0.js';
import './lit-element-BL-po2DW.js';
import './lit-html-COgVUehj.js';
import './define-element-C5BYKBZP.js';
import './condition-attribute-with-id-Cnyhr7Mp.js';
import './ElementResolution-B9KteuX8.js';
import './sizedMixin-DyuXUK9p.js';
import './query-DQF6X5qW.js';
import './base-u8Z1Hrsd.js';
import './sp-icon-alert-C1Xt_mWd.js';
import './custom-tag-Diwq7nXX.js';
import './Textfield-CTAxcsQN.js';
import './manage-help-text-83_bseGo.js';
import './if-defined-DDJGFaN4.js';
import './focusable-BQuZpn6Y.js';
import './focus-visible-D29Av9Xb.js';
import './spectrum-icon-checkmark.css-C_afKhIc.js';
import './directive-Bn5c4u4M.js';
import './directive-helpers-icdnqxxc.js';
import './state-CnHyAYDv.js';
import './index-DII9xnLW.js';

var colorFieldSizes_stories = {
  component: "sp-color-field",
  title: "Color Field/Sizes"
};
const s = () => ColorFieldMarkup({ size: "s" });
const m = () => ColorFieldMarkup({ size: "m" });
const l = () => ColorFieldMarkup({ size: "l" });
const xl = () => ColorFieldMarkup({ size: "xl" });
const __namedExportsOrder = ['s', 'm', 'l', 'xl'];

export { __namedExportsOrder, colorFieldSizes_stories as default, l, m, s, xl };
