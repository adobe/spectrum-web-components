"use strict";(self.webpackChunk_spectrum_web_components_1st_gen=self.webpackChunk_spectrum_web_components_1st_gen||[]).push([[6130],{"./packages/toast/stories/toast.stories.js":function(__unused_webpack_module,__webpack_exports__,__webpack_require__){__webpack_require__.r(__webpack_exports__),__webpack_require__.d(__webpack_exports__,{Default:function(){return Default},Info:function(){return Info},Negative:function(){return Negative},Positive:function(){return Positive},Wrapping:function(){return Wrapping},__namedExportsOrder:function(){return __namedExportsOrder},default:function(){return toast_stories},overlaidBottom:function(){return overlaidBottom},overlaidLeft:function(){return overlaidLeft},overlaidRight:function(){return overlaidRight},overlaidTop:function(){return overlaidTop},overlay:function(){return overlay}});var index_dev=__webpack_require__("./tools/base/src/index.dev.js"),directives_dev=__webpack_require__("./tools/base/src/directives.dev.js"),define_element_dev=__webpack_require__("./tools/base/src/define-element.dev.js"),decorators_dev=__webpack_require__("./tools/base/src/decorators.dev.js"),focus_visible_dev=__webpack_require__("./tools/shared/src/focus-visible.dev.js");__webpack_require__("./packages/button/sp-close-button.dev.js"),__webpack_require__("./packages/icons-workflow/icons/sp-icon-alert.js"),__webpack_require__("./packages/icons-workflow/icons/sp-icon-info.js"),__webpack_require__("./packages/icons-workflow/icons/sp-icon-checkmark-circle.js");var toast_css=index_dev.AH`
...
`,__defProp=Object.defineProperty,__getOwnPropDesc=Object.getOwnPropertyDescriptor,__decorateClass=(decorators,target,key,kind)=>{for(var decorator,result=kind>1?void 0:kind?__getOwnPropDesc(target,key):target,i=decorators.length-1;i>=0;i--)(decorator=decorators[i])&&(result=(kind?decorator(target,key,result):decorator(result))||result);return kind&&result&&__defProp(target,key,result),result};const toastVariants=["negative","positive","info","error","warning"];class Toast extends((0,focus_visible_dev.p)(index_dev.wG)){constructor(){super(...arguments),this.open=!1,this._timeout=null,this._variant="",this.countdownStart=0,this.nextCount=-1,this.doCountdown=time=>{this.countdownStart||(this.countdownStart=performance.now()),time-this.countdownStart>this._timeout?(this.shouldClose(),this.countdownStart=0):this.countdown()},this.countdown=()=>{cancelAnimationFrame(this.nextCount),this.nextCount=requestAnimationFrame(this.doCountdown)},this.holdCountdown=()=>{this.stopCountdown(),this.addEventListener("focusout",this.resumeCountdown)},this.resumeCountdown=()=>{this.removeEventListener("focusout",this.holdCountdown),this.countdown()}}static get styles(){return[toast_css]}set timeout(timeout){const newTimeout=null!==timeout&&timeout>0?Math.max(6e3,timeout):null,oldValue=this.timeout;newTimeout&&this.countdownStart&&(this.countdownStart=performance.now()),this._timeout=newTimeout,this.requestUpdate("timeout",oldValue)}get timeout(){return this._timeout}set variant(variant){if(variant===this.variant)return;const oldValue=this.variant;toastVariants.includes(variant)?(this.setAttribute("variant",variant),this._variant=variant):(this.removeAttribute("variant"),this._variant=""),this.requestUpdate("variant",oldValue)}get variant(){return this._variant}renderIcon(variant,iconLabel){switch(variant){case"info":return index_dev.qy`
          <sp-icon-info
            label=${iconLabel||"Information"}
            class="type"
          ></sp-icon-info>
        `;case"negative":case"error":return index_dev.qy`
          <sp-icon-alert
            label=${iconLabel||"Error"}
            class="type"
          ></sp-icon-alert>
        `;case"warning":return index_dev.qy`
          <sp-icon-alert
            label=${iconLabel||"Warning"}
            class="type"
          ></sp-icon-alert>
        `;case"positive":return index_dev.qy`
          <sp-icon-checkmark-circle
            label=${iconLabel||"Success"}
            class="type"
          ></sp-icon-checkmark-circle>
        `;default:return index_dev.qy``}}startCountdown(){this.countdown(),this.addEventListener("focusin",this.holdCountdown)}stopCountdown(){cancelAnimationFrame(this.nextCount),this.countdownStart=0}shouldClose(){this.dispatchEvent(new CustomEvent("close",{composed:!0,bubbles:!0,cancelable:!0}))&&this.close()}close(){this.open=!1}render(){return index_dev.qy`
    ...
    `}updated(changes){super.updated(changes),changes.has("open")&&(this.open?this.timeout&&this.startCountdown():this.timeout&&this.stopCountdown()),changes.has("timeout")&&(null!==this.timeout&&this.open?this.startCountdown():this.stopCountdown())}}__decorateClass([(0,decorators_dev.MZ)({type:Boolean,reflect:!0})],Toast.prototype,"open",2),__decorateClass([(0,decorators_dev.MZ)({type:Number})],Toast.prototype,"timeout",1),__decorateClass([(0,decorators_dev.MZ)({type:String})],Toast.prototype,"variant",1),__decorateClass([(0,decorators_dev.MZ)({type:String,attribute:"icon-label"})],Toast.prototype,"iconLabel",2),(0,define_element_dev.e)("sp-toast",Toast);__webpack_require__("./packages/button/sp-button.dev.js"),__webpack_require__("./packages/overlay/sp-overlay.dev.js"),__webpack_require__("./packages/overlay/overlay-trigger.dev.js");const toast=({variant:variant="",open:open=!0,content:content="",timeout:timeout=0,iconLabel:iconLabel=""})=>index_dev.qy`
  <sp-toast
    variant=${variant}
    ?open=${open}
    timeout=${(0,directives_dev.JR)(timeout)}
    .iconLabel=${iconLabel}
  >
    ${content}
    <sp-button
...
`;var toast_stories={component:"sp-toast",title:"Toast",args:{content:"This is a toast message.",open:!0},argTypes:{content:{name:"content",type:{name:"string",required:!1},table:{type:{summary:"string"},defaultValue:{summary:""}}},open:{name:"open",type:{name:"boolean",required:!1},table:{type:{summary:"boolean"},defaultValue:{summary:!1}}},variant:{name:"variant",options:["","negative","positive","info","error","warning"],table:{type:{summary:"string"},defaultValue:{summary:""}},control:{type:"select"}},timeout:{name:"timeout",type:{name:"number",required:!1},table:{type:{summary:"number"},defaultValue:{summary:null}}},iconLabel:{name:"iconLabel",type:{name:"string",required:!1},table:{type:{summary:"string"},defaultValue:{summary:""}}}}};const Default=({variant:variant,open:open,content:content,timeout:timeout,iconLabel:iconLabel})=>toast({variant:variant,open:open,content:content,timeout:timeout,iconLabel:iconLabel}),variantDemo=({variant:variant,open:open,content:content,timeout:timeout,iconLabel:iconLabel})=>toast({variant:variant,open:open,content:content,timeout:timeout,iconLabel:iconLabel}),Positive=args=>variantDemo({...args,variant:"positive"}),Negative=args=>variantDemo({...args,variant:"negative"}),Info=args=>variantDemo({...args,variant:"info"}),Wrapping=args=>variantDemo({...args,variant:"info",content:"A new version of Lightroom Classic is now available. Use the Update button below to start using the new version."}),overlayStyles=index_dev.qy`
  <style>
    html,
    body,
    #root,
...
`,overlaid=openPlacement=>index_dev.qy`
    ${overlayStyles}
    ${[["bottom",""],["left","negative"],["right","positive"],["top","info"]].map(([placement,variant])=>index_dev.qy`
        <overlay-trigger
          placement=${placement}
          open=${(0,directives_dev.JR)(openPlacement===placement?"click":void 0)}
        >
          <sp-button label="${placement} test" slot="trigger">
            Click for ${variant||"toast"} on the ${placement}
          </sp-button>
      ...
      `)}
  `,overlaidTop=()=>overlaid("top"),overlaidRight=()=>overlaid("right"),overlaidBottom=()=>overlaid("bottom"),overlaidLeft=()=>overlaid("left"),overlay=args=>index_dev.qy`
    <style>
      sp-toast {
        position: fixed;
        bottom: 1em;
  ...
  `,__namedExportsOrder=["Default","Positive","Negative","Info","Wrapping","overlaidTop","overlaidRight","overlaidBottom","overlaidLeft","overlay"];Default.parameters={...Default.parameters,docs:{...Default.parameters?.docs,source:{originalSource:"({\n  variant,\n  open,\n  content,\n  timeout,\n  iconLabel\n}) => {\n  return toast({\n    variant,\n    open,\n    content,\n    timeout,\n    iconLabel\n  });\n}",...Default.parameters?.docs?.source}}},Positive.parameters={...Positive.parameters,docs:{...Positive.parameters?.docs,source:{originalSource:'args => variantDemo({\n  ...args,\n  variant: "positive"\n})',...Positive.parameters?.docs?.source}}},Negative.parameters={...Negative.parameters,docs:{...Negative.parameters?.docs,source:{originalSource:'args => variantDemo({\n  ...args,\n  variant: "negative"\n})',...Negative.parameters?.docs?.source}}},Info.parameters={...Info.parameters,docs:{...Info.parameters?.docs,source:{originalSource:'args => variantDemo({\n  ...args,\n  variant: "info"\n})',...Info.parameters?.docs?.source}}},Wrapping.parameters={...Wrapping.parameters,docs:{...Wrapping.parameters?.docs,source:{originalSource:'args => variantDemo({\n  ...args,\n  variant: "info",\n  content: "A new version of Lightroom Classic is now available. Use the Update button below to start using the new version."\n})',...Wrapping.parameters?.docs?.source}}},overlaidTop.parameters={...overlaidTop.parameters,docs:{...overlaidTop.parameters?.docs,source:{originalSource:'() => overlaid("top")',...overlaidTop.parameters?.docs?.source}}},overlaidRight.parameters={...overlaidRight.parameters,docs:{...overlaidRight.parameters?.docs,source:{originalSource:'() => overlaid("right")',...overlaidRight.parameters?.docs?.source}}},overlaidBottom.parameters={...overlaidBottom.parameters,docs:{...overlaidBottom.parameters?.docs,source:{originalSource:'() => overlaid("bottom")',...overlaidBottom.parameters?.docs?.source}}},overlaidLeft.parameters={...overlaidLeft.parameters,docs:{...overlaidLeft.parameters?.docs,source:{originalSource:'() => overlaid("left")',...overlaidLeft.parameters?.docs?.source}}},overlay.parameters={...overlay.parameters,docs:{...overlay.parameters?.docs,source:{originalSource:'args => {\n  return html`\n    <style>\n      sp-toast {\n        position: fixed;\n        bottom: 1em;\n        left: 1em;\n      }\n    </style>\n    <sp-button id="overlay">Toggle Toast overlay</sp-button>\n    <sp-overlay trigger="overlay@click" type="auto" open>\n      ${variantDemo({\n    ...args,\n    variant: "positive"\n  })}\n    </sp-overlay>\n  `;\n}',...overlay.parameters?.docs?.source}}}},"./packages/button/sp-button.dev.js":function(__unused_webpack_module,__unused_webpack___webpack_exports__,__webpack_require__){var _spectrum_web_components_base_src_define_element_js__WEBPACK_IMPORTED_MODULE_0__=__webpack_require__("./tools/base/src/define-element.dev.js"),_src_Button_dev_js__WEBPACK_IMPORTED_MODULE_1__=__webpack_require__("./packages/button/src/Button.dev.js");(0,_spectrum_web_components_base_src_define_element_js__WEBPACK_IMPORTED_MODULE_0__.e)("sp-button",_src_Button_dev_js__WEBPACK_IMPORTED_MODULE_1__.$n)},"./packages/button/sp-close-button.dev.js":function(__unused_webpack_module,__unused_webpack___webpack_exports__,__webpack_require__){var _spectrum_web_components_base_src_define_element_js__WEBPACK_IMPORTED_MODULE_0__=__webpack_require__("./tools/base/src/define-element.dev.js"),_src_CloseButton_dev_js__WEBPACK_IMPORTED_MODULE_1__=__webpack_require__("./packages/button/src/CloseButton.dev.js");(0,_spectrum_web_components_base_src_define_element_js__WEBPACK_IMPORTED_MODULE_0__.e)("sp-close-button",_src_CloseButton_dev_js__WEBPACK_IMPORTED_MODULE_1__.J)},"./packages/icons-workflow/icons/sp-icon-checkmark-circle.js":function(__unused_webpack_module,__unused_webpack___webpack_exports__,__webpack_require__){var index_dev=__webpack_require__("./tools/base/src/index.dev.js"),src_index_dev=__webpack_require__("./packages/icon/src/index.dev.js"),custom_tag=__webpack_require__("./packages/icons-workflow/src/custom-tag.js");var CheckmarkCircle=__webpack_require__("./packages/icons-workflow/src/icons/CheckmarkCircle.js");class IconCheckmarkCircle extends src_index_dev.z{render(){return(0,custom_tag.L)(index_dev.qy),2===this.spectrumVersion?(({width:e=24,height:l=24,hidden:r=!1,title:t="Checkmark Circle"}={})=>custom_tag.T`<svg
...
//# sourceMappingURL=toast-stories-toast-stories.5600247a.iframe.bundle.js.map