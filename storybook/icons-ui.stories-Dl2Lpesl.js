...
var iconsUi_stories = {
  ...
  swc_vrt: {
    preload: async () => {
      await import('./icon-manifest-Bvgsid-Y.js');
    }
  }
};
const elements = ({ color, size }) => {
  const content = import('./icon-manifest-Bvgsid-Y.js').then(
    (iconManifest) => x`
            <icons-demo
                style="color: ${color}"
                size=${size}
...
const Icons = ({ color, size }) => {
  const content = import('./index-rr3jYv9x.js').then((icons) => {
    const iconTemplates = [];
