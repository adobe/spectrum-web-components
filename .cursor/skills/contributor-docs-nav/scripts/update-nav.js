...
function extractHeadings(filepath) {
  try {
    ...
    return headingMatches
      .filter((m) => {
        ...
        return !codeBlockRanges.some(
          (range) => headingPos >= range.start && headingPos < range.end
        );
      })
      .map((m) => {
    ...
    console.error(
      `❌ Error extracting headings from ${filepath}:`,
      err.message
    );
    return [];
  }
...
function generateBeneathDoc(filePath, metadata, indentLevel, depth) {
  ...
  if (!folderMeta || !folderMeta.children || folderMeta.children.length === 0) {
    return null;
  }
  ...
  for (const childName of folderMeta.children) {
    const childPath = path.join(parentFolder, childName).replace(/\\/g, '/');
    ...
    if (metadata.files[childPath]) {
      ...
      if (childFolderMeta.hasReadme) {
        ...
        const relPath = path.relative(path.dirname(filePath), readmePath);
        lines.push(`${indent}- [${childFolderMeta.displayName}](${relPath})`);
      } else {
...
function generateGrandchildren(
  ...
  for (const childName of folderMeta.children) {
    ...
    if (metadata.files[childPath]) {
      ...
      const relPath = path.relative(path.dirname(originFilePath), childPath);
      lines.push(`${indent}- [${childFileMeta.displayName}](${relPath})`);
      ...
      if (childFolderMeta.hasReadme) {
        const readmePath = path.join(childPath, 'README.md');
        const relPath = path.relative(path.dirname(originFilePath), readmePath);
        lines.push(`${indent}- [${childFolderMeta.displayName}](${relPath})`);
      } else {
        // No README, show as plain text
        lines.push(`${indent}- ${childFolderMeta.displayName}`);
  ...
  try {
    ...
    if (titlePos !== -1 && contentPos !== -1) {
      ...
      if (h1Match) {
        ...
        documentContent = content.substring(h1Index + h1Content.length).trim();
      } else {
        console.warn(`⚠️  No H1 found in ${filePath}, using display name`);
