...
function extractH1(filepath) {
    try {
        ...
        const titlePos = content.indexOf(MARKER_TITLE);
        if (titlePos === -1) {
        ...
        const tocPos = content.indexOf(MARKER_TOC);
...
function extractHeadings(filepath) {
    try {
        ...
        const contentPos = content.indexOf(MARKER_CONTENT);
        ...
        let match;
        while ((match = fencedBlockRegex.exec(docContent)) !== null) {
...
function generateTOC(filePath, metadata) {
    ...
    const beneathLines = generateBeneathDoc(filePath, metadata, 0);
    if (beneathLines) {
        const lines = [
            '<details open>',
...
function generateBeneathDoc(filePath, metadata, indentLevel) {
    const fileMeta = metadata.files[filePath];
