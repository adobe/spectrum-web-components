...
| **Q7** | Event naming: keep `change` as-is or rename to `swc-change`? Per the [JSDoc standards guide](../../../for-contributors/style-guide/typescript/jsdoc-standards.md), DOM-style names are preferred and `swc-` is reserved for custom events with no native equivalent. **Needs team alignment — flag for sync discussion.** | **Yes** | API reviewer |
...
| **Q12** | `--highcontrast-tabs-*` tokens — per [forced-colors requirements](../../../for-contributors/style-guide/css/component-css.md#forced-colors-requirements), only add overrides if an actual problem is observed. Tabs is an invented control, so overrides are likely needed. Verify during implementation. | No | CSS reviewer / Implementation |
...
- [Migration guide](../../../../2nd-gen/packages/swc/components/tabs/migration-guide.mdx)
