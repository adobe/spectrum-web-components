...
- [Update styles in the gen2 component](#update-styles-in-the-gen2-component)
...
- `dist/index.css` - This is the processed version of the component styles; for now, this is the best place to source styles for SWC. When copying files from Spectrum CSS into the gen2 component, using this file will ensure you get the benefits of the CSS build tooling.
...
Next, we need to copy the styles from the Spectrum CSS component to the gen2 component.
...
## Update styles in the gen2 component
...
If these styles are found, we need to confirm if they are needed in the gen2 component. Not all 1st-gen overrides or component-specific styles are needed in the gen2 components and sometimes there are other ways to source those styles using the original classes provided by the Spectrum CSS asset.

It might be helpful, at this point, to define the render function for the gen2 component so you can spin up Storybook and start seeing these new styles in action. A quick way to kick this off is to copy the `spectrum-css/components/<component-name>/stories/template.js` file into a render function on your new gen2 component.
...
As you can see, the gen2 implementation leverages the `classMap` function to conditionally apply classes to the component based on the component's properties. This is a common pattern in the gen2 components. This approach has several benefits:
...
In our gen2 version, we will likely want to maintain any slots available in the 1st-generation component unless design changes from S2 provide a compelling reason to change or remove them.
