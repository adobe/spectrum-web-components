...
The guide works for **human engineers** and **AI agents**. Follow each phase in order. At "Stop and ask" points, pause and get human input before continuing.
...
1. **Factor rendering out of 1st-gen** — [Step 2](02_step-by-step/02_factor-rendering-out-of-1st-gen-component.md): Rename to `[Component].base.ts`, create new `[Component].ts` that extends the base, move `render()` and `styles` to the concrete class. Confirm tests still pass.
2. **Move base class to 2nd-gen core** — [Step 3](02_step-by-step/03_move-base-class-to-2nd-gen-core.md): Create `core/components/<name>/`, move the base and types there, update 1st-gen to import from core. Confirm tests still pass.
...
1. **Read the component analysis** (if it exists) for your component. Analysis docs live under [03_components/](../../03_components/) in a folder named after the component (e.g. `badge/rendering-and-styling-migration-analysis.md`). This describes required rendering and styling changes for 2nd-gen.
   - **To generate or refresh the analysis:** Use the [Analyze rendering and styling](02_step-by-step/01_analyze-rendering-and-styling/README.md) step: follow the README and the [Cursor prompt](02_step-by-step/01_analyze-rendering-and-styling/cursor_prompt.md) to produce or update the markdown file in `03_components/<component-name>/rendering-and-styling-migration-analysis.md`.
...
**Goal:** Move properties, methods, and types from 1st-gen to 2nd-gen; keep a clear public API. Align with [Step 4: Formalize Spectrum data model](02_step-by-step/04_formalize-spectrum-data-model.md) and [Step 5: Add 2nd-gen SWC component](02_step-by-step/05_implement-2nd-gen-component.md).
...
**Goal:** Migrate CSS to 2nd-gen structure and follow the CSS style guide. See [Step 6: Migrate rendering & styles from Spectrum CSS](02_step-by-step/06_migrate-rendering-and-styles.md) for copying from spectrum-css `spectrum-two` and implementing `render()` with `classMap`.
...
2. **Use design tokens:** Replace hard-coded values with `var(--spectrum-*)` tokens.
3. **Extract custom properties** for theming (e.g. `--spectrum-badge-*` in Badge).
4. **Implement variants with classes:** e.g. `.spectrum-Badge--positive`, `.spectrum-Badge--sizeM`. Use `classMap()` in `render()` (see Badge).
...
**Goal:** JSDoc, Storybook stories, and usage docs so others can use and migrate. Align with [Step 7: Add stories for 2nd-gen component](02_step-by-step/07_add-stories-for-2nd-gen-component.md).
