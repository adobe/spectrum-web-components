...
- **This is a deliberate deviation from the PoC's own reference implementation.** The [form-strategy proof-of-concept](https://github.com/nikkimk/web-component-form-strategy-demos)'s [`my-radio.js`](https://github.com/nikkimk/web-component-form-strategy-demos/blob/main/my-radio.js) (and its `LiveSelectionController` experiment, `my-radio-live.js`) puts `role="radio"` on the host via `ElementInternals` for the item too, with no inner native input at all; its own header comment gives the same "no live value" rationale as the button-like exception above. This RFC departs from that reference implementation for the item specifically: a native `<input type="radio">` gets `checked`, its implicit `aria-checked` mapping, and keyboard <kbd>Space</kbd> activation entirely from the browser, instead of `swc-radio` hand-writing `internals.ariaChecked` at every state-change call site the way `my-radio.js` does (`select()`, `deselect()`, `formResetCallback()`, and its general state-sync method); and it keeps the number of host-role exceptions in this repo to only roles with no native HTML equivalent at all (`radiogroup`), rather than extending it to every widget role that merely lacks a live value. `swc-radio-group` keeps the PoC's host-role treatment unchanged; only the item's role placement diverges.
