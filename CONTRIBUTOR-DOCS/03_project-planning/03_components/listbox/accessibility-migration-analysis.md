...
- The [`LiveSelectionController`](../../../../2nd-gen/packages/core/controllers/live-selection-controller/live-selection-controller.mdx) — selection enforcement, used in every mode. The [`FocusgroupNavigationController`](../../../../2nd-gen/packages/core/controllers/focusgroup-navigation-controller/focusgroup-navigation-controller.mdx) applies **only** to a standalone listbox using **roving `tabindex`** (where DOM focus moves onto the option); an active-descendant listbox and its embedding combobox do **not** use it, because they keep DOM focus on the container/input and move only the active descendant.
...
- The APG describes two focus models: **roving `tabindex`** (DOM focus moves to the active option) and **active-descendant** (DOM focus stays on the listbox container, which carries `aria-activedescendant`). A standalone `swc-listbox` may use either: the roving model uses the `FocusgroupNavigationController`; the active-descendant model manages the active option itself (moving no DOM focus), the same way the combobox does. When **embedded in a combobox**, the active-descendant model is required, the reference lives on the combobox input rather than the listbox, and the `FocusgroupNavigationController` is not used — see [`swc-combobox`](../combobox/accessibility-migration-analysis.md#keyboard-and-focus).
...
| **Option traversal** | Depends on the focus model. A **roving-`tabindex`** standalone listbox uses the **`FocusgroupNavigationController`** (`direction: 'vertical'`, `skipDisabled: true`) to move DOM focus between options. An **active-descendant** listbox (and every combobox-embedded listbox) does **not** use that controller: it moves only the active-descendant pointer and keeps DOM focus on the container/input, skipping disabled options in its own handler. Embedded in a combobox, traversal is owned by the combobox — see [`swc-combobox`'s Keyboard and focus](../combobox/accessibility-migration-analysis.md#keyboard-and-focus). |
...
- **Standalone.** One Tab stop for the whole widget. In the **roving-`tabindex`** variant, the `FocusgroupNavigationController` (`direction: 'vertical'`, `skipDisabled: true`, `wrap` per design) handles <kbd>ArrowUp</kbd>/<kbd>ArrowDown</kbd>, <kbd>Home</kbd>/<kbd>End</kbd>, and optional typeahead by moving DOM focus onto the option; in the **active-descendant** variant, the listbox moves the active-descendant pointer itself (no `FocusgroupNavigationController`, focus stays on the container). Selection is made with <kbd>Enter</kbd>/<kbd>Space</kbd>; multi-select adds <kbd>Shift</kbd> + arrows and select-all where the design calls for it, following the [APG listbox keyboard model](https://www.w3.org/WAI/ARIA/apg/patterns/listbox/#keyboardinteraction).
- **Embedded in a combobox/picker.** The listbox is **not** a Tab stop and does not hold DOM focus; the combobox input or picker button is the focusable element and owns the keyboard model, including active-descendant traversal. The `FocusgroupNavigationController` is **not** used in this mode — it moves DOM focus, which would take focus off the input — so the parent moves only `aria-activedescendant` while keeping DOM focus on its own control (see [`swc-combobox`'s Keyboard and focus](../combobox/accessibility-migration-analysis.md#keyboard-and-focus)). Pointer users can click an option to select it in every mode.
