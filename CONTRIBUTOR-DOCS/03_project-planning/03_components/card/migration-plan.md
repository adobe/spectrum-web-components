...
`card-template.css` (implemented; shared across every concrete card) uses **CSS Grid** for the shared structural layout — the `.swc-CardBase` root and its media/content/footer regions are grid areas/tracks.
