...
**Default slot presence is observed via `ObserveSlotText`:** `CardBase` mixes in the existing `ObserveSlotText` mixin (`core/mixins/observe-slot-text.ts`) for the default slot (empty `slotName`), exposing `this.slotHasContent`. Each concrete card's `render()` passes `hasDefaultSlotContent: this.slotHasContent` into `renderCardTemplate()`, which applies a `--hasDefault` class to the `.swc-CardBase` wrapper via `classMap` when true — a derived-state class (not a consumer-settable attribute), per the "when to use classes vs attributes" style guide convention.

**Two latent bugs found and fixed in the shared `ObserveSlotText` mixin itself** (`core/mixins/observe-slot-text.ts`, also used by `ButtonBase` for `hasLabel` and `Badge.ts` for its no-label class) while wiring this up for Card:

1. **`MutationController` only watched `characterData`, not `childList`.** Adding or removing an entire slotted node (as opposed to editing an existing text node's content in place) is a `childList` mutation, so it went completely undetected — `slotHasContent` never recomputed after content was dynamically added or removed post-connection. Fixed by adding `childList: true` to the observed config.
2. **`@queryAssignedNodes` didn't resolve against the mixin's `Symbol`-keyed private field**, always returning `undefined` regardless of what was actually slotted. This silently no-opped the entire reactive re-check path (the guard `if (!this[assignedNodesList]) return;` always took the early exit) — the only reason anything appeared to work at all was the separate one-time synchronous seed in `update()`, which reads `this.childNodes` directly and doesn't go through the decorator. Fixed by replacing the decorated field with a plain `getAssignedNodes()` method that queries `this.renderRoot` directly (the same query, just not routed through the decorator).

Neither bug was caught by any existing test — every test that touched this mixin (`Button`, `Badge`) only exercised the static initial-render path, never dynamically added/removed slot content afterward. Added regression tests to both `card.test.ts` (`DefaultSlotClassUpdatesDynamicallyTest`) and `button.test.ts` (`LabelSlotUpdatesDynamicallyTest`) covering the previously-broken dynamic path.
...
| Rendered layout geometry (grid placement, aspect ratios, media overlay stacking) | `card.test.ts` asserts the CSS *contracts* (the custom-property values, `display` toggles, and `elementFromPoint` hit-testing that drive the layout) but not the resulting pixels — grid tracks, `3/2` vs. square aspect ratios, and the `media`-slot overlay's visual stacking are geometric | VRT (`.vrt.ts`) |
