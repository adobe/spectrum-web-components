...
| `.spectrum-Accordion-itemDirectActions` | `slot="direct-actions"` projected into `.spectrum-Accordion-itemDirectActions` container (sibling to button, inside heading) | In scope — container rendered in initial migration; `slotchange` observer hides container when slot is empty. Specific content (`swc-action-button`, `swc-switch`) deferred until 2nd-gen components are available. |
| `.spectrum-Accordion-itemHeader:has(+ .spectrum-Accordion-itemDirectActions)` | Applied when `slotchange` observer detects assigned nodes | In scope — driven by the `slotchange` observer on the `direct-actions` slot. |
...
- **Direct actions area** — `.spectrum-Accordion-itemDirectActions` is a new optional region inside the item heading (sibling of the button) that houses direct action content. Neither the region nor the associated `:has(+ .spectrum-Accordion-itemDirectActions)` state selector has any 1st-gen equivalent. **In scope for the initial migration** — the `direct-actions` slot and container are rendered; a `slotchange` observer hides the container when the slot is empty and `stopPropagation` prevents slotted content from toggling the accordion. The slot is open-ended by design; specific content constraints and disabled-state propagation are deferred until 2nd-gen components are available (see [migration plan — Public API](./migration-plan.md#public-api)).
...
- **Direct actions area added**: New `div.spectrum-Accordion-itemDirectActions` inside the item heading (sibling of the button, after it), housing optional direct action content via `slot="direct-actions"`. **In scope for the initial migration** — container and slot are rendered; `slotchange` hides the container when the slot is empty; `stopPropagation` prevents slot clicks from toggling the accordion. Specific content constraints deferred until 2nd-gen components are available (see [migration plan — Public API](./migration-plan.md#public-api)).
