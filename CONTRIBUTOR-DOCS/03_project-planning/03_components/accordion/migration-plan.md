...
- [gen2 API decisions (planned)](#gen2-api-decisions-planned)
...
> Planning output for migrating **`sp-accordion`** / **`sp-accordion-item`** to gen2. **Must be reviewed before implementation begins.**
...
| `level` | `number` | `3` | `level` | Public on the accordion; pushed to each assigned item. Item clamps to **2–6** for `<h*>` tags. In **planned gen2**, only the parent exposes **`level`**; the item receives a **protected `heading`** (see [`AccordionItem`](#sp-accordion-item-accordionitem)). |
...
**1st-gen note:** Today **`level`** is also reflected on **`sp-accordion-item`** and overwritten when slotted; gen2 removes that public surface on the item in favor of **`protected` `heading`** driven by the parent (**B9**).
...
**gen2:** This full modifier surface **will not** be carried forward. Replace with S2 tokens and a **narrow**, reviewed customization story (see Phase 4 mapping).
...
| `@spectrum-web-components/reactive-controllers` | `1.11.2` | 1st-gen: `FocusGroupController` on `Accordion` (arrow/Home/End + roving tabindex — **not** carried forward). gen2: consider a **`RadioController`**-style primitive in **core** or shared controllers (see [Selection sync](#selection-sync--radiocontroller-pattern-consider)); gen2 already ships **`FocusgroupNavigationController`** in `gen2/packages/core/controllers/` for arrow-key roving elsewhere — selection sync is a **different** concern (property reflection, not focus traversal). |
...
> - **Additive** work can follow without breaking consumers already on gen2.
...
> The 1st-gen component is a **reference only** — gen2 is built independently. Neither generation imports from the other at runtime.

Follow the [washing machine — core vs SWC](../../02_workstreams/02_gen2-component-migration/02_step-by-step/01_washing-machine-workflow.md#core-vs-swc-where-does-code-go) split. The [Badge migration reference](../../02_workstreams/02_gen2-component-migration/02_step-by-step/01_washing-machine-workflow.md#reference-badge-migration) is the concrete pattern for file layout.

| Layer | Path | Contains |
|---|---|---|
| **Core** | `gen2/packages/core/components/accordion/` | `AccordionBase` / item base (if split), types, `allow-multiple` coordination, **public** `level` validation, propagation of **`level`** → item **`protected` `heading`**, **accordion-wide** **`quiet`** / **`disabled`** effective visuals and interaction to assigned items ([React Spectrum alignment considerations](#react-spectrum-alignment-considerations)), toggle event contract, disabled + `inert` rules testable without full render. **Optional:** compose a **`RadioController`** (or equivalent) for exclusive **`open`** / **`aria-selected`** sync ([Selection sync](#selection-sync--radiocontroller-pattern-consider)). **No** Lit template/CSS. |
| **SWC** | `gen2/packages/swc/components/accordion/` | `swc-accordion`, `swc-accordion-item` Lit classes, `.css`, registration, stories, tests, S2 styling. Use **`swc-icon`** for internal chevron / disclosure graphics ([Open — API and scope](#open--api-and-scope)). |
...
gen2 exposes a narrower customization surface than 1st-gen. Most visual values are driven by Spectrum 2 design tokens internally and are not overridable via custom properties.

| 1st-gen property | gen2 property | Notes |
|---|---|---|
...
| `--mod-accordion-edge-to-*-space` (other) | Not exposed | Legacy 1st-gen spacing mods without a 1:1 gen2 override |
...
gen2 **does not** use the HTML **`hidden`** attribute on the content panel.
...
Implementation: [`AccordionItem.ts`](../../../../gen2/packages/swc/components/accordion/AccordionItem.ts), [`accordion-item.css`](../../../../gen2/packages/swc/components/accordion/accordion-item.css) (`@supports (height: calc-size(auto, size))` block).
...
Gates align with [01_washing-machine-workflow.md](../../02_workstreams/02_gen2-component-migration/02_step-by-step/01_washing-machine-workflow.md).
...
- [x] Create `gen2/packages/core/.../accordion` and `gen2/packages/swc/.../accordion`
- [x] Wire exports; build passes with placeholder implementation

### API

- [x] Public properties, attributes, and events match agreed gen2 surface (including renames; **no** string **`label`** on item — heading via slot only); include reflected **`density`** (`compact` \| `regular` \| `spacious`), **`quiet`** (host only), and host **`disabled`** per [React Spectrum alignment considerations](#react-spectrum-alignment-considerations)
- [x] Dev-mode warning when **`density`** is omitted (if adopted)
- [x] Exclusive open (`allow-multiple` false): decide **`RadioController`** vs inline sibling-close logic; if controller is shared, document API and add tests — **decision:** inline **`closeSiblingsOnOpen`** in **`AccordionBase`** for this migration; **`RadioController`** deferred ([A3](#additive--ships-when-ready-zero-breakage-for-consumers-already-on-gen2))
...
- [x] [`keyboard.test.ts`](../../../../1st-gen/packages/accordion/test/keyboard.test.ts) expectations updated (Tab order, no `FocusGroup` arrows — covered by `NoArrowKeyNavTest`, `KeyboardEnterTest`, `KeyboardSpaceTest` in gen2 test file)
...
- [x] Playwright ARIA snapshots — [`accordion.a11y.spec.ts`](../../../../gen2/packages/swc/components/accordion/test/accordion.a11y.spec.ts) (Overview, Item states, Disabled accordion, Allow multiple, Quiet, Heading level, Direct actions, Accessibility)
- [x] Storybook play functions ([`accordion.test.ts`](../../../../gen2/packages/swc/components/accordion/test/accordion.test.ts)) — 27 stories covering defaults, property reflection, anatomy/slots, toggle open/close, exclusive open, allow-multiple, canceled toggle, keyboard (Space, Space-preventDefault, no arrow-key navigation between headers [B3 guard], panel keys), disabled item (aria-disabled, not native; panel inert), host disabled, host re-enable preserves per-item state, direct actions no-toggle, events (toggle incl. bubbles+composed, open/close, after-open/close), ARIA (aria-expanded true/false asserted explicitly, aria-hidden + no `hidden` attr, aria-controls, role=region, aria-labelledby), heading level propagation + clamping, standalone item default, dynamic size propagation, enforce-exclusive-open on re-enable. The `aria-expanded="true"` open state is additionally validated by the Playwright ARIA snapshots ([`accordion.a11y.spec.ts`](../../../../gen2/packages/swc/components/accordion/test/accordion.a11y.spec.ts)); the closed `aria-expanded="false"` value is asserted in the unit test because ARIA snapshots cannot distinguish it from a missing attribute
...
- [x] Do not document arrow-key navigation between headers for gen2 (contrast with legacy README)
...
- [ ] [01_status.md](../../02_workstreams/02_gen2-component-migration/01_status.md) updated when migration lands
