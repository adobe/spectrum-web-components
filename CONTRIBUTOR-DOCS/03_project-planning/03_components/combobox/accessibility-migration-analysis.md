...
- Its options are author-supplied [`swc-option`](../option/accessibility-migration-analysis.md) elements — a new 2nd-gen component that owns `role="option"` on its own host, rather than the `sp-menu-item` (`role="menuitem"`) 1st-gen borrows. Options may sit directly in the combobox or be grouped under a labeled [`swc-option-group`](../option-group/accessibility-migration-analysis.md) (`role="group"`), implementing the APG [grouped listbox](https://www.w3.org/WAI/ARIA/apg/patterns/listbox/examples/listbox-grouped/) pattern. The listbox and its options do **not** have to live in the same shadow root as the input; see [Shadow DOM and cross-root ARIA Issues](#shadow-dom-and-cross-root-aria-issues).
