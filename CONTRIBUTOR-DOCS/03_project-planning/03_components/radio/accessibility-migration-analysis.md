...
- One option in a mutually exclusive set. `swc-radio` is always used inside a `swc-radio-group`, which owns its tabindex and answers keyboard activation for it; `swc-radio` has no independent Tab stop of its own. This is a deliberate change from gen1 `sp-radio`, which defaults its own `tabIndex` to `0` and answers `Space` on its own so it can be used without a wrapping `sp-radio-group`; 2nd-gen drops that standalone support in favor of the group always owning tabindex management (see [Keyboard and focus](#keyboard-and-focus)).
...
| **`checked` / `aria-checked`** | Comes from the browser for free: the native `<input type="radio">`'s `checked` IDL property drives its own implicit `aria-checked`, so `swc-radio` does not need to write `aria-checked` manually the way gen1 `sp-radio` does on itself in `updated()`. Keep `swc-radio`'s own `checked` (or `selected`) property in sync with the inner input's `checked`. Radio's `aria-checked` only ever takes `true`/`false`, never `"mixed"` (unlike checkbox's tri-state support). |
...
| **Per-item description (confirmed in scope)** | [React Spectrum's `Radio`](https://react-spectrum.adobe.com/RadioGroup) accepts an optional per-radio `description` (used for enriched option text, for example "Delivers in 5–7 business days" under a shipping-method label). gen1 `sp-radio` has no equivalent, and the reviewed Figma files show only a group-level description and error message, matching gen1's `sp-radio-group` help-text slots rather than a per-radio one. Design has since confirmed per-item descriptions will be supported, so `swc-radio` should plan for a per-radio `description` rather than treat it as deferrable scope; finalize the exact API in the migration plan (SWC-2350) and Phase 3 (API). Implement it with a plain same-root `aria-describedby` pointing at an element in the input's own shadow root, the same pattern `swc-text-field` uses for its description; no element-reference API is needed for this case. |
...
| **Form association (moved to the group — Q12, resolved)** | `swc-radio` has no `FieldAssociationController`, no `attachInternals()`, and no form-value participation of its own. A native `<input type="radio">` inside `swc-radio`'s own shadow root is invisible to an ancestor light-DOM `<form>` regardless of where `ElementInternals` attaches, so per-item form association would gain nothing; form-value submission and constraint validity live entirely on `swc-radio-group`'s single `FieldAssociationController` instance instead, driven by the group's own `selected` state. `name` is not propagated onto the inner input either, for the same reason: since the item never participates in form value, doing so would be cosmetic only, so it's dropped (B12). Full reasoning: [radio migration plan](./migration-plan.md#decision-log), Q12/B11/B12, confirmed with a11y SME review. See the [radio group doc](../radio-group/accessibility-migration-analysis.md#aria-roles-states-and-properties) for the group's form-participation model. |
...
| --- | --- |
| **Unit** | `swc-radio`'s shadow DOM contains a real `<input type="radio">`; the host itself sets no `role`; `checked` on `swc-radio` stays in sync with the inner input's `checked`. Form-value participation (`setFormValue`, `FormData` exclusion of the unchecked items) is tested at `swc-radio-group`, not here (Q12, resolved — see the [radio group doc](../radio-group/accessibility-migration-analysis.md#testing)). |
...
- [ ] Per-item `description` (React Spectrum parity, confirmed in scope by design) is implemented via a same-root `aria-describedby` on the inner input, with the exact API finalized during Phase 3 (API).
- [ ] `disabled` reflects onto the inner input's native `disabled` attribute rather than a hand-written `aria-disabled`.
- [ ] No per-radio `readonly` property; that state moves to `swc-radio-group` and its enforcement gap in gen1 is fixed there, not carried forward.
...
- [ ] `swc-radio` has no `FieldAssociationController`/`ElementInternals` of its own; form-value participation and `FormData` exclusion of unchecked items are the group's responsibility alone (Q12, resolved).
- [ ] `name` is not propagated onto the item's inner input (B12); the item never participates in form value, so doing so would be cosmetic only.
...
- gen1: [`sp-radio`/`sp-radio-group`](../../../../gen1/packages/radio/README.md)
