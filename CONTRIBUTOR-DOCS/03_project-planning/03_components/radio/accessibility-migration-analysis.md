...
- **This is a deliberate departure from the [form-strategy proof-of-concept](https://github.com/nikkimk/web-component-form-strategy-demos)'s reference implementation**, not an oversight. The PoC's [`my-radio.js`](https://github.com/nikkimk/web-component-form-strategy-demos/blob/main/my-radio.js) puts `role="radio"` on the host via `ElementInternals`, with no inner native input at all, and its own header comment gives the same "no live value" justification the button-like exception uses. `swc-radio` departs from that specifically because a native input gets `checked`/`aria-checked`/keyboard activation for free from the browser, instead of the PoC's pattern of hand-writing `internals.ariaChecked` at every state-change call site (`select()`, `deselect()`, `formResetCallback()`, and its state-sync method); and it keeps this repo's host-role exceptions limited to roles with no native HTML equivalent at all (`radiogroup`), rather than every role that merely lacks a live value. See the RFC's [§3.2](../../05_strategies/forms-strategy-rfc.md#32-where-aria-roles-live) for the same rationale recorded at the strategy level.
...
| **Accessible name** | In precedence order: (1) `accessible-labelledby`, for the rare case a radio's name is composed from elements it doesn't own; (2) `accessible-label`, for a radio with no visible text label of its own (for example, an icon-only swatch-style radio, if that variant is ever added); (3) the **slotted default-content text**, the normal case, rendered as a real, same-root `<label for="…">` paired with the inner `<input type="radio">`'s `id`, the same pattern `swc-text-field` uses for its own label. Because the input (and its role) live in the same shadow root as the label, this is plain same-root `<label for>`/`aria-labelledby` wiring, not a host-attached `ElementInternals` element-reference API — see [Shadow DOM and cross-root ARIA Issues](#shadow-dom-and-cross-root-aria-issues). |
...
None. Because `swc-radio` renders a real, native `<input type="radio">` inside its own shadow DOM, its label, and (if added) its description, all resolve through plain same-root `<label for>`/`aria-labelledby`/`aria-describedby` wiring inside that one shadow root, the same as `swc-text-field`'s inner `<input type="text">`. There is no host-attached `ElementInternals` element-reference wiring needed here, unlike `swc-radio-group`, which has no native container element and must attach its own label/description directly to its host (see the [radio group doc](../radio-group/accessibility-migration-analysis.md#shadow-dom-and-cross-root-aria-issues)).
...
- [ ] Accessible name precedence (`accessible-labelledby` > `accessible-label` > slotted label rendered as a real `<label for>`) resolves through same-root wiring inside the shadow root, not a host-attached `ElementInternals` element reference.
