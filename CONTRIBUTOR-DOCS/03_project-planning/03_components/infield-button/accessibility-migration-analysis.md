...
**`sp-infield-button`** extends gen1 **`ButtonBase`** with **`block`**, **`inline`**, **`quiet`**, and **`label`** (maps to **`aria-label`**); public docs may list **`pending`** from **`ButtonBase`**. gen1 **`block`** / **`inline`** chiefly adjusted **corner radius** and edge padding for **stacked** and **inline** groups. 2nd-gen should **drop** **`block`**, **`inline`**, and **`pending`** on **`swc-infield-button`** (let the **field** host own **`pending`** and **layout**), standardize on **`accessible-label`**, and **not** ship **`href`** / link APIs inherited from older **button** stacks.
...
| **`inline` and `block` removed** | **Do not** implement or document **`inline`** or **`block`**. gen1 attributes existed for **corner rounding** and group edges; S2 uses **consistent** in-field button radius—**number-field** steppers and **inline** groups are composed by the **parent field** (DOM + CSS), not per-button position props. Name each control for **function** (**“Clear search”**, **“Increment”**), not **“start”** / **“end”**. |
...
- **gen1 gap:** **`label`** instead of **`accessible-label`**; **`block`** / **`inline`** documented in [gen1 docs](https://opensource.adobe.com/spectrum-web-components/components/infield-button/)—2nd-gen docs must **not** copy **position-attribute** or **stacked** examples.
...
- [ ] **`block`** and **`inline`** **removed**; migration guide and Storybook **omit** position-attribute and gen1 **stacked** examples; triage [SWC-1130](https://jira.corp.adobe.com/browse/SWC-1130) / [SWC-1197](https://jira.corp.adobe.com/browse/SWC-1197) against **field-composed** S2 patterns.
...
- [Infield Button — Spectrum Web Components (gen1)](https://opensource.adobe.com/spectrum-web-components/components/infield-button/)
...
- [`InfieldButton.ts` (gen1)](../../../../gen1/packages/infield-button/src/InfieldButton.ts)
