...
| Submenus | On **`swc-menu-item`**: when the **`submenu` slot** has **content**, **submenu** **parent** `menuitem` (`aria-haspopup="menu"`, `aria-expanded`); **submenu** **trigger** and **child** **`role="menu"`** are **in** **`swc-menu-item`’s** **shadow** **DOM** (**not** the **item** **CE** **host**), **with** **popover** **anchoring** ([Menu item — Recommendations](../menu-item/accessibility-migration-analysis.md#recommendations-swc-menu-item); **not** a **nested** **`swc-menu`** **in** the list). |
...
| Open/close + roles | Responsibility may sit in a **controller module**, in **`swc-popover`**, and in the **menu-button** host (**`swc-menu`** or **`swc-action-menu`**) that **owns** the **trigger** and **wiring** to the **internal** **`role="menu"`** **node**—**for example** **ActionMenu**-specific **trigger** chrome (icon-only vs **visible** label) on **`swc-action-menu`**, or **Menu**-specific **trigger** on **`swc-menu`**. Confirm the actual split in 2nd-gen source. In all cases keep open/close, initial and return focus, and ID wiring inside one composable subtree so IDREFs do not span unrelated shadow roots. |

### Shadow DOM and cross-root ARIA Issues

Keep the **trigger**, the **internal** **`role="menu"`** **element** (**`swc-menu`’s** **shadow** **tree**), and **item** **ID** **targets** **in one composable subtree** for **that** top-level **menu** so `aria-controls`, `aria-labelledby`, and focus order do not depend on IDREFs across disconnected shadow roots. Prefer roving `tabindex` on items over `aria-activedescendant` for 2nd-gen (see action menu doc).

For **nested** **submenus** (when **`swc-menu-item`’s** **`submenu` slot** is **used**), the **submenu** **trigger** and **child** **`role="menu"`** are **in** **`swc-menu-item`’s** **shadow** **DOM** (**not** the **item** **CE** **host**), **analogous** to the **top-level** **pattern**; **keep** their **ARIA** and **ID** **wiring** **coherent** with the **parent** **menu** (see [Menu item — Shadow DOM](../menu-item/accessibility-migration-analysis.md#shadow-dom-and-cross-root-aria-issues)).

### Accessibility tree expectations

- Closed: menu hidden; focus on trigger (or parent item).
- Open: **internal** **`role="menu"`** **exposed** to **ATs**; first keystroke lands on an item per pattern; items expose roles and names; disabled items use `aria-disabled="true"` when appropriate.
