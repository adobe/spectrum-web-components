...
- **`swc-menu`** (this doc) and **`swc-action-menu`** ([action menu doc](../action-menu/accessibility-migration-analysis.md)) are **both** top-level **menu-button** hosts. **Each** includes: a **trigger** button, **`swc-popover`** for **anchoring** the open surface, a **menu** **target** with **`role="menu"`**, and **slots** for **`swc-menu-group`**, **`swc-menu-item`**, and **`swc-menu-divider`**. **Submenus** are **not** a **second** **nested** **`swc-menu`** or **`swc-action-menu`** **in** the **list**; they are **implemented** on **`swc-menu-item`** through the **`submenu` slot**: when that **slot** has **child** **nodes**, **the** **item** **renders** its **own** **popover** and **`role="menu"`** **container** (see [Menu item — What `swc-menu-item` is (2nd-gen)](../menu-item/accessibility-migration-analysis.md#what-swc-menu-item-is-2nd-gen); **verify** in 2nd-gen **source**). Checkbox / radio menu items and selection UX (`aria-checked`, modes) defer per [Migration scope](#migration-scope-current).
...
- Two coordination pieces (conceptual; how they ship may split across packages—verify in 2nd-gen source):
  1. Open/close and role wiring — must satisfy the [menu button pattern](https://www.w3.org/WAI/ARIA/apg/patterns/menu-button/) (`role`, `aria-*` on trigger and **`role="menu"`** target, open state, focus on open with first item or return on close, Escape) **on whichever top-level host** the author uses (**`swc-menu`** or **`swc-action-menu`**). Implementation may use a **dedicated controller** (root **`swc-action-menu`**, **`swc-menu`**, or **`swc-menu-item`** when it **hosts** a **submenu** **via** the **`submenu` slot**), plus **`swc-popover`** for **anchoring** and dismiss as applicable (including **per-item** **submenu** **popovers** where implemented). **`swc-action-menu`**-specific product defaults (for example **icon-only** “more” vs **visible** label) do **not** change the fact that **both** top-level menu hosts **own** trigger + menu; **menu ARIA and in-menu keyboard** are still **not** implemented by **`swc-popover`** alone (see [Popover](../popover/accessibility-migration-analysis.md)).
...
| Submenus | On **`swc-menu-item`**: when the **`submenu` slot** has **content**, **submenu** **parent** `menuitem`: `aria-haspopup="menu"`, `aria-expanded`; **child** **`role="menu"`** **surface** and **popover** are **owned** by **that** **item** ([Menu item — Recommendations](../menu-item/accessibility-migration-analysis.md#recommendations-swc-menu-item); **not** a **nested** **`swc-menu`** **host** in the list). |
...
- Open, close, and focus hand-off to first, last, or return are coordinated with the **top-level** **`swc-menu`** or **`swc-action-menu`** **host** (or the **`swc-menu-item`** **submenu** **parent** and its **item-scoped** **popover** / **`role="menu"`**).
...
- [ ] `swc-menu` is a full menu-button host (trigger + `swc-popover` + `role="menu"` target + slots for groups, items, dividers), parallel to `swc-action-menu`; do not document a detached `role="menu"` list. **Submenus** are authored on **`swc-menu-item`’s** **`submenu` slot** (item-owned **popover** + child **`role="menu"`**), not by nesting another **`swc-menu`** or **`swc-action-menu`** in the list.
