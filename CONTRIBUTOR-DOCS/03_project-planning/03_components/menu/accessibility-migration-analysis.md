...
This document sets accessibility expectations for 2nd-gen Menu in Spectrum Web Components: `swc-menu` as the `role="menu"` surface with `menuitem` (and related `swc-menu-item`, `swc-menu-group`, `separator`) in a [WAI-ARIA menu](https://www.w3.org/WAI/ARIA/apg/patterns/menubar/) / [menu button](https://www.w3.org/WAI/ARIA/apg/patterns/menu-button/) composition—not a stand-alone product widget. The target is **WCAG 2.2 Level AA**. Product alignment for structure and placement: [React Spectrum `Menu`](https://react-spectrum.adobe.com/Menu) with MenuTrigger (long-term, `menuitemcheckbox` / `menuitemradio` and selection UX remain tracked separately; see below).
...
- [Action menu accessibility migration analysis](../action-menu/accessibility-migration-analysis.md) for `swc-action-menu`, ellipsis trigger, `swc-popover` for positioning (not 1st-gen overlay), and submenus. Mobile tray and menu item selection (`menuitemcheckbox` / `menuitemradio`, modes) defer per [Migration scope (current)](#migration-scope-current).
...
- A menu is the popup list with `role="menu"` and focusable `menuitem` rows (and `separator`, `group` where used) per the APG. Checkbox / radio menu items and menu item selection (modes, `aria-checked`, toggle keyboard) are deferred pending design and accessibility clarification (see [Migration scope](#migration-scope-current)). In React Spectrum, Menu is always associated with MenuTrigger; 2nd-gen SWC matches that: `swc-menu` is always a child of `swc-action-menu` (top-level) or of a parent `swc-menu` / `swc-menu-item` (submenu), never a detached root in product.
- Placement (anchor to trigger, flip, z-index) uses the shared [popover](https://react-spectrum.adobe.com/Popover) / `swc-popover` host, not the 1st-gen overlay stack. ARIA for `menu` and items lives on the menu tree; `swc-popover` does not implement `role="menu"` or in-list keyboard (see [Popover](../popover/accessibility-migration-analysis.md)). Mobile tray (alternative small-viewport presentation) is out of scope for the current migration and is not part of the 2nd-gen positioning spec in these docs.
...
- [Menu divider accessibility migration analysis](../menu-divider/accessibility-migration-analysis.md) for **`swc-menu-divider`** (**`separator`** between **`menuitem`** rows).
...
Bottom line: `swc-menu` implements the menu subtree structure (`menu` plus `menuitem` navigation and activation); open/close and role wiring may be shared by a controller, `swc-popover` (lifecycle/anchoring), and menu/action-menu hosts (`swc-action-menu` trigger chrome such as icon vs label is one example—confirm in implementation). `FocusgroupNavigationController` owns in-list keys. Anchor placement uses `swc-popover`, not 1st-gen overlay. Mobile tray and menu item selection (checkbox, radio, modes) remain out of scope for the wave documented here ([Migration scope](#migration-scope-current)).
...
- [ ] Open/close + role wiring vs in-list keys: implementation may split across a controller, `swc-popover`, and `swc-menu` / `swc-action-menu` (verify in source); `FocusgroupNavigationController` for in-menu movement; `swc-popover` for anchor placement (not 1st-gen overlay). Mobile tray is out of scope for the current migration.
