...
- [Menu separator accessibility migration analysis](../menu-separator/accessibility-migration-analysis.md) for **`swc-menu-separator`** (**`separator`** between **`menuitem`** rows).
...
| Menu groups | Do not nest one `swc-menu-group` inside another in the same `role="menu"` list; a group **may** appear inside a **submenu** (child `role="menu"` on `swc-menu-item`). See [Menu group — What `swc-menu-group` is (2nd-gen)](../menu-group/accessibility-migration-analysis.md#what-swc-menu-group-is-2nd-gen). |
