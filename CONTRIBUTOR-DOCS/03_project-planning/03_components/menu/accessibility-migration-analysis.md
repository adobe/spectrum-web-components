...
| Topic | What to do |
| --- | --- |
| Internal `role="menu"` (not the CE host) | `role="menu"` belongs on an element inside `swc-menu`’s shadow DOM, not on the `<swc-menu>` custom element host. Verify final markup in 2nd-gen source before shipping docs. |
| Items | `menuitem` and `separator` / `group` as for plain commands; no `listitem` inside `menu`. Slotted row content need not share the same DOM as the menu host’s trigger or the internal `role="menu"` node (see [Shadow DOM](#shadow-dom-and-cross-root-aria-issues)). Checkbox and radio `menuitem*` rows and selection behavior (`aria-checked`, multi-select) are out of scope until clarification ([Migration scope](#migration-scope-current)). |
...
| Open/close + roles | Responsibility may sit in a controller module, in `swc-popover`, or in the menu-button host (`swc-menu` or `swc-action-menu`) that owns the trigger and wiring to the internal `role="menu"` node—for example ActionMenu-specific trigger chrome (icon-only vs visible label) on `swc-action-menu`, or Menu-specific trigger on `swc-menu`. Confirm the actual split in 2nd-gen source. Open/close, initial/return focus, and in-menu behaviour must still satisfy the [menu button pattern](https://www.w3.org/WAI/ARIA/apg/patterns/menu-button/); slotted items and other row primitives do not have to be in the same DOM as the trigger or the internal `role="menu"` (portals, slots, and controllers may bridge across roots as implemented). IDREF-centric ARIA (for example pairing IDs across unconnected subtrees) can break—prefer roving `tabindex` and no `aria-activedescendant` across unrelated shadow roots (see [SWC-617](https://jira.corp.adobe.com/browse/SWC-617) and the action menu doc); verify wiring in 2nd-gen source. |

### Shadow DOM and cross-root ARIA Issues

Slotted `swc-menu-item`, `swc-menu-group`, and `swc-menu-divider` nodes (and their descendant content) do not have to be in the same DOM tree as the menu-button trigger or the internal `role="menu"` element; an implementation may project or relayer rows (for example via `swc-popover`, portals, or slot composition) while preserving exposed `menu` / `menuitem` semantics and focus management (verify in 2nd-gen source). IDREF-based ARIA across separate document fragments (for example `aria-controls`, `aria-activedescendant`, or `aria-labelledby` / `aria-describedby` to IDs in unrelated roots) is unreliable; prefer roving `tabindex` for in-menu focus and avoid patterns that depend on ID resolution between the trigger and rows when they are not co-located (see action menu doc, [SWC-617](https://jira.corp.adobe.com/browse/SWC-617)).
