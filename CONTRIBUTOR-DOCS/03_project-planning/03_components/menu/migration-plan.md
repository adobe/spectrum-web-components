...
| --- | --- |
| Role | `role="menuitem"` lives on whichever element is the actual interactive node, never both: the host itself for plain command rows, or the internal `<a>` for link rows. When the `<a>` carries it, the host takes `role="presentation"` instead (5t3ph review, [Q19](#cross-component-follow-ups-not-blocking-swc-menu)) — otherwise the host is an unstyled intervening semantic node between `swc-menu`'s `role="menu"` and the `menuitem`, and the required-owned-elements relationship the menu pattern depends on won't validate. |
...
| Link rows | An `href` attribute is necessary and sufficient to signal a link row (5t3ph review, matching 1st-gen's `LikeAnchor` surface) — no separate boolean. When present, `swc-menu-item` owns the `<a href>` internally as a wrapper (revised, [Q17](#cross-component-follow-ups-not-blocking-swc-menu)) so it's the row's sole focusable element and carries the row's role per the Role row above; not a consumer-slotted anchor |
...
Out of scope for this table (and for Phase A): submenus, groups, selection states, thumbnail/badge/unavailable content, printable-character navigation — all tracked under their own B/A items and Phase B/C.
...
| --- | --- | --- |
| **B2** | Link items | Proxy `change` on `href` items removed; real `<a>` is the only activation path ([SWC-923](https://jira.corp.adobe.com/browse/SWC-923)). `swc-menu-item` owns the `<a>` internally as a wrapper, not a consumer-slotted anchor, so it's the row's sole focusable element and carries the row's role (revised conclusion, [Q17](#cross-component-follow-ups-not-blocking-swc-menu)) — this plan doesn't implement it, but the model is settled enough to design Phase A's minimal item surface against. |
...
| `change` (href path) | Drop, superseded by native anchor activation (B2) |
...
- [ ] Link-like rows: `swc-menu-item` owns its `<a href>` internally as a wrapper, not a consumer-slotted anchor, so it's the row's sole focusable element and carries the row's role (revised, [Q17](#cross-component-follow-ups-not-blocking-swc-menu)); retest against [SWC-923](https://jira.corp.adobe.com/browse/SWC-923)
...
| Q15 | Migration order among `swc-menu`, `swc-menu-item`, and `swc-menu-group` is not yet decided (`swc-divider` already exists, and `swc-menu-separator` isn't being built — [Q14](#blockers-and-open-questions)). `swc-popover` is **not a sequencing variable** — it shipped in [PR #6356](https://github.com/adobe/spectrum-web-components/pull/6356) (merged 2026-07-14, before this plan was drafted) and `swc-menu` doesn't depend on it anyway. Each menu-family component either has or will have its own plan; sequencing among them is what remains open. **Decided not to block on this** (5t3ph review): rather than pausing `swc-menu`'s Phase A to wait for a `swc-menu-item` plan, this plan now documents the minimal item surface Phase A needs directly — see [Phase A minimal `swc-menu-item` surface](#phase-a-minimal-swc-menu-item-surface). The relative-priority question for a full `swc-menu-item` plan is still open. | Open | Ticket owner |
| Q17 | (Raised by Ruben, on [B2](#must-ship); evolved during 5t3ph's review) If link-style `swc-menu-item` rows work by having the consumer slot a real `<a href>` into the item's light DOM (per the [menu-item a11y doc](../menu-item/accessibility-migration-analysis.md)), styling that anchor to match the row's own hover state, full-row hit target, focus ring, and icon alignment is materially harder than 1st-gen's shadow-internal `<a class="anchor">` render. A CSS link-expansion trick (as used for Card) was considered and set aside: it doesn't solve the deeper problem, which is that a consumer-slotted `<a>` risks becoming a second focusable element nested inside the row, and the wrong node ends up carrying the row's role. **Leaning conclusion: `swc-menu-item` should own the `<a>` internally as a wrapper**, matching 1st-gen's shadow-internal render, so it's provably the row's sole focusable element and carries the role directly. This contradicts the current [menu-item a11y doc](../menu-item/accessibility-migration-analysis.md), which says "consumers should slot a real `<a href>`" — that doc needs correcting to match, the same pattern as [Q13](#cross-component-follow-ups-not-blocking-swc-menu). This plan doesn't implement `swc-menu-item`, but the conclusion is settled enough to use in [Phase A minimal `swc-menu-item` surface](#phase-a-minimal-swc-menu-item-surface) above. | Open — direction settled, needs `swc-menu-item` plan owner to formalize and correct the a11y doc | `swc-menu-item` plan owner |
...
| Q19 | (5t3ph review) Once [Q17](#cross-component-follow-ups-not-blocking-swc-menu)'s link-wrapper conclusion ships, `swc-menu-item` cannot put `role="menuitem"` on both its host and the internal `<a>` — the `menu`/`menuitem` required-owned-elements relationship needs `menuitem` on exactly one node, with nothing else carrying a competing implicit or explicit role in between. For link rows, that means `role="presentation"` on the host so the accessibility tree skips over it and sees `menu` > `a[role=menuitem]` directly, not `menu` > (unstyled semantic host) > `a`. Not this plan's implementation to make, but the constraint is concrete enough to hand to whoever builds `swc-menu-item` rather than leaving it to be discovered during an a11y audit. | Open | `swc-menu-item` plan owner |
