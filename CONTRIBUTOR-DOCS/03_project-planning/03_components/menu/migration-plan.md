...
| --- | --- |
| Role | `role="menuitem"` on the host or its shadow-internal row, per the [Menu item a11y doc](../menu-item/accessibility-migration-analysis.md) |
| Content | Default slot (label text) plus one leading `visual` slot (icon; `swc-thumbnail` once that component exists — [A4](#additive)), not both content types combined |
...
| Link rows | Owns its `<a href>` internally as a wrapper (revised, [Q17](#cross-component-follow-ups-not-blocking-swc-menu)) so it's the row's sole focusable element and carries the row's role; not a consumer-slotted anchor |
