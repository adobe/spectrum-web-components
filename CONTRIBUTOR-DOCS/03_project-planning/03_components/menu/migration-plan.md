...
- **All Menu migration work lands on the `rajdeep/menu-migration` branch, not `main`.** Point every phase PR (Setup, API, Styling, Accessibility, Testing, Documentation, Review) at `rajdeep/menu-migration`; it merges to `main` as a whole once the migration is complete. Matches the pattern already used for `opacity-checkerboard-migration` (currently live) and, historically, `accordion-migration`/`button-group-migration`/`meter-migration` before those completed and were merged. Setup ([PR #6596](https://github.com/adobe/spectrum-web-components/pull/6596)) is in progress there now.
