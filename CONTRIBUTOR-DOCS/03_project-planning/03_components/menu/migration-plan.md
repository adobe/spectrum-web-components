...
- [x] `Menu.types.ts`: define allowed slot children (`swc-menu-item` this pass; `swc-menu-group` and `swc-divider` join in Phase B, not now) and the size union (`s`/`m`/`l`/`xl`, default `m` — confirmed). No separate open/close state-shape type: the `open` boolean property is the state shape, nothing else to declare.
- [x] `Menu.base.ts`: `for`/`triggerElement`/`direction`/`align`/`shouldFlip`/`open` properties; `resolveTrigger`-based ARIA wiring (`aria-haspopup="menu"`, `aria-expanded`) and click-to-toggle onto the resolved trigger; `direction`/`align` translate to a `PlacementController` `Placement` value; `swc-open`/`swc-after-open`/`swc-close`/`swc-after-close` dispatch on `open` changes (synchronously for now, no transition exists yet to key off); `validateAllowedChildren` on the default slot. No `label` property, no shadow-rendered trigger (decided, [Q16](#blockers-and-open-questions)); no `selects`/`value`/`selectedItems` — stays Additive/Phase B ([Q5](#blockers-and-open-questions)). `disabled` still skipped, open question on its semantics now that the trigger is external, not assumed here. `PlacementController.start()` is fully wired but no-ops until Phase 5 (rendering) provides the actual shadow-internal surface element via a `surfaceElement` hook the base class already calls. Keyboard/focus management (arrow keys, Escape, `FocusgroupNavigationController`) is Phase 4's job, not started here.
