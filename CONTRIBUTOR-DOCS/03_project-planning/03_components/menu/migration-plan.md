...
| `selectedKeys` / `defaultSelectedKeys` / `selectionMode` / `onSelectionChange` / `disallowEmptySelection` | various | **A section can carry its own selection state independent of the parent Menu's, and different sections in the same menu can run different modes at once** — e.g. a plain command section (`selectionMode: 'none'`) alongside a single-select section and a multi-select section, all in one `Menu`. "Selection" is not one monolithic per-menu decision; it's per-group. See [Mixed menus](#behavioral-semantics) below for the consequence this has for `swc-menu-group`'s design in Phase B, given the decided break from `MenuGroup extends Menu` ([Q11](#blockers-and-open-questions)). |
...
| **A2** | Selection semantics (`selects`, `value`, `selectedItems`, single/multiple, `menuitemcheckbox`/`menuitemradio`), including **mixed menus** — different sections running different selection modes at once (see [Behavioral semantics — Mixed menus](#behavioral-semantics)) | Deferred per [a11y Migration scope](./accessibility-migration-analysis.md#migration-scope-current) — **but see [Q2](#blocking-for-swc-menu)**: the reviewed Figma file's dedicated "Changes in S2 Menu" frame and a live multi-select "Examples" frame show this is real design intent, not just a property-table inference. Decided ([Q5](#blockers-and-open-questions)) to keep this Additive rather than pull it into Phase A for `picker`/`combobox`, since neither has a scheduled migration yet. When [Q2](#blocking-for-swc-menu) resolves, this moves to Must-ship **in this same plan** (it's entirely `Menu`-owned, unlike B1/B3/B8) — do not spin up a separate document for it. Phase B's design must account for per-group selection scope explicitly, since it can no longer rely on `swc-menu-group` inheriting it. |
...
- **Mixed menus — capability vs. guidance tension.** React Spectrum's `MenuSection` scopes `selectionMode` independently per section, so one `Menu` can mix a plain command section, a single-select section, and a multi-select section simultaneously (React's own docs example does exactly this). But the Figma "Changes in S2 Menu" note quoted in [Q2](#blocking-for-swc-menu) says the opposite as *design guidance*: "Please avoid mixing checkboxes with other menu types for a better user experience." So there are two separate questions, not one: (1) should `swc-menu` even *allow* mixed-mode sections at the API level, and (2) if it does, what's the per-group selection-scope mechanism. On (2): 1st-gen supported mixing incidentally, because `MenuGroup extends Menu` gave every group its own inherited `selects` API for free; that inheritance is gone by design ([Q11](#blockers-and-open-questions), [B1](#must-ship--breaking-or-a11y-required)), so Phase B has to design per-group selection scope deliberately — most likely a `selection-mode`-style property declared directly on `swc-menu-group` that `swc-menu`'s selection controller reads per group, not a selection engine owned by the group itself. On (1): resolve with Design as part of [Q2](#blocking-for-swc-menu), since "avoid mixing" reads as UX guidance for authors, not necessarily a hard API restriction — don't assume either answer. Not a Phase A concern; Phase A ships no selection at all.
...
| --- | --- | --- | --- | --- |
| Q2 | Beyond the Menu group/Menu item property tables, the same Figma file's dedicated "Changes in S2 Menu" frame (node `37252:5063`) states "We will continue to support checkboxes in menu items... evaluating optimal use cases for multi-select checkboxes in picker menus... avoid mixing checkboxes with other menu types," and its "Examples" frame (node `37252:4608`) shows a live multi-select checkbox menu ("Select up to 4 languages"). This is concrete design intent, not just a property-table inference, and it directly contradicts the [a11y analysis's Migration scope](./accessibility-migration-analysis.md#migration-scope-current), which defers all selection pending a product decision this evidence suggests has already been made. Get the a11y analysis's "Migration scope" section corrected. **Also unresolved as part of this same question (raised by Ruben):** the Figma note's "avoid mixing checkboxes with other menu types" reads as UX guidance, but React Spectrum's `MenuSection` explicitly supports independent per-section selection modes in one menu — decide whether `swc-menu` allows mixed-mode sections at the API level at all, separate from how it'd be implemented if allowed (see [Behavioral semantics — Mixed menus](#behavioral-semantics)). | Yes | Open — no longer "is this real," now "when/how does it ship," plus the mixed-menu allow/disallow question above. Once resolved, [A2](#additive--ships-when-ready-zero-breakage-for-consumers-already-on-2nd-gen) moves to Must-ship as Phase B **in this plan** (see [Implementation phasing](#implementation-phasing)); selection is `Menu`-owned. | Design + accessibility reviewer |
