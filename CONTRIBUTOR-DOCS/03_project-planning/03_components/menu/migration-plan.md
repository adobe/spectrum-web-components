...
- **Placement is public as a single `placement` property, matching `Tooltip`/`Popover`'s shape** (revised, [Q20](#blockers-and-open-questions), reopened during the API migration PR review by 5t3ph). The original decision narrowed to a `direction`/`align` pair to match React Spectrum S2's `MenuTrigger` props one-to-one; review feedback preferred `Tooltip`/`Popover`'s existing `placement` shape instead. `swc-menu`'s `MenuPlacement` union keeps all six `MenuTrigger.direction` sides (unlike `Popover`, which narrows to its logical sides only and drops `left`/`right`), each with its two alignments, so this is Menu's own narrowed subset rather than a re-export of `Popover`'s. `shouldFlip` carries straight over from all three precedents; `offset`/`cross-offset` are not exposed, since a menu's trigger-to-surface gap is a fixed visual-spec value with no consumer request to tune it, in either generation.
...
| `triggerElement` | `HTMLElement \| null` | `null` | — (`attribute: false`) | **Decided** ([Q16](#blockers-and-open-questions)) — direct trigger reference; overrides `for` when set. Use for cross-shadow-root triggers or programmatic wiring, matching `Tooltip`/`Popover`. No `VirtualTrigger` support yet ([Q23](#blockers-and-open-questions), open) — `Popover.base.ts` accepts one for cursor-anchored context menus, but that needs its own ARIA/click-listener carve-outs, deferred rather than assumed here. |
| `placement` | `MenuPlacement` (12 values: `bottom`/`top`/`left`/`right`/`start`/`end`, each with its two alignments) | `'bottom-start'` | `placement` | **Revised** ([Q20](#blockers-and-open-questions), reopened by 5t3ph's API migration PR review) — single hyphenated property matching `Tooltip`/`Popover`'s `placement` shape, superseding the earlier `direction`/`align` pair. Keeps all six `MenuTrigger.direction` sides (unlike `Popover`, which drops `left`/`right`), so it is Menu's own `MenuPlacement` union in `Menu.types.ts`, not a re-export of `Popover`'s. |
...
| `disabled` | — | — | — | **Open question, skipped for now** ([Q22](#blockers-and-open-questions)). With the trigger externally referenced ([Q16](#blockers-and-open-questions)), this can no longer mean "disable `swc-menu`'s own rendered trigger" — it would need to mean "prevent the menu from opening even if the resolved trigger fires," a `Popover`/`Tooltip`-style suppression flag rather than a trigger-level disabled state. The open/close wiring this was waiting on now exists; not implementing it is a decision gap, not a blocker. |
...
> **Separately, `.swc-Menu` uses `popover="auto"`** for native top-layer rendering and light-dismiss ([Q25](#blockers-and-open-questions)). Not a reversal of the above: Q3 only ruled out wrapping `<swc-popover>` as a component; the native `popover` attribute on `swc-menu`'s own surface is unrelated.
...
- [x] Plan reviewed by at least one other engineer (Ruben, 5t3ph, nikkimk, aramos-adobe, rise-erpelding)
...
`Menu.types.ts` is intentionally not part of Setup: it's out of scope until Menu's Phase A properties are actually decided (see [API](#api) below), matching the project's own `04_formalize-spectrum-data-model.md` step, which treats the data model as its own step after the base class exists. (The `03_move-base-class-to-2nd-gen-core.md` step also lists a types file as one of its own bullets — a minor inconsistency between those two docs worth reconciling separately; not blocking here either way, since Menu has no real constants or type unions yet to put in one.)
...
- [x] `Menu.types.ts`: define allowed slot children (`swc-menu-item` this pass; `swc-menu-group` and `swc-divider` join in Phase B, not now), the size union (`s`/`m`/`l`/`xl`, default `m` — confirmed), and the `MenuPlacement` union (12 values covering all six `MenuTrigger.direction` sides — [Q20](#blockers-and-open-questions)). No separate open/close state-shape type: the `open` boolean property is the state shape, nothing else to declare.
- [x] `Menu.base.ts`: `for`/`triggerElement`/`placement`/`shouldFlip`/`open` properties; `resolveTrigger`-based ARIA wiring (`aria-haspopup="menu"`, `aria-expanded`) and click-to-toggle onto the resolved trigger; `placement` (a Menu-specific `MenuPlacement` union, [Q20](#blockers-and-open-questions)) passes straight through to `PlacementController`; `swc-open`/`swc-after-open`/`swc-close`/`swc-after-close` dispatch on `open` changes (synchronously for now, no transition exists yet to key off); `validateAllowedChildren` on the default slot. No `label` property, no shadow-rendered trigger (decided, [Q16](#blockers-and-open-questions)); no `selects`/`value`/`selectedItems` — stays Additive/Phase B ([Q5](#blockers-and-open-questions)). `disabled` still skipped, open question on its semantics now that the trigger is external, not assumed here. `PlacementController.start()` is fully wired but no-ops until Phase 5 (rendering) provides the actual shadow-internal surface element via a `surfaceElement` hook the base class already calls. Keyboard/focus management (arrow keys, Escape, `FocusgroupNavigationController`) is Phase 4's job, not started here.
...
| Q20 | (Raised by aramos-adobe) Should `swc-menu` expose `placement`/`offset`/`should-flip`, matching `Tooltip`/`Popover`'s public `PlacementController` properties, or keep this private/internal? | **Revised.** First decided as a `direction`/`align` pair, matching React Spectrum S2's `MenuTrigger` props one-to-one (source priority #1 for API naming) rather than inventing a third vocabulary alongside `Tooltip`'s and `Popover`'s single hyphenated `placement` string. Reopened during the API migration PR review: 5t3ph asked to keep the single `placement` shape instead, since the full `placement` values already include alignment, making a separate `align` property redundant. Final: single `placement` property, but with its own `MenuPlacement` union rather than reusing `Popover`'s exact 12 values, so `MenuTrigger.direction`'s `left`/`right` sides are not lost (`Popover` narrows to its logical sides only). `shouldFlip` carries straight over (all three precedents already agree on name, type, and default). `offset`/`cross-offset` are **not** exposed: React Spectrum's `MenuTrigger` has no equivalent, and unlike `Tooltip`/`Popover` (which wrap arbitrary, variously-shaped content and need a tunable gap), a menu's trigger-to-surface gap is a fixed Spectrum visual-spec value with no consumer request to override it, in either generation. |
| Q24 | (Raised by Ruben, a11y migration PR review, Slack) Some accessibility behavior (focus into the first item on open, focus back to the trigger on close, the surface actually leaving the accessibility tree when closed) can only be genuinely verified against a real open/close visual state, not just wiring reviewed in isolation — risking a second full re-review once Phase 5 lands if the PR ships with `surfaceElement` still a no-op. | **Decided: pull a minimal, unstyled slice of Phase 5 forward into the a11y ticket.** `Menu.ts` now overrides `surfaceElement` to return the real `.swc-Menu` node, `menu.css` adds only the `position: absolute` `PlacementController` needs (no Spectrum tokens, no `spectrum-css` baseline), and the render template gains a plain `?hidden=${!this.open}` binding. The Playground story also gained a real `swc-button` trigger wired via `for`, matching `Popover.stories.ts`'s pattern, so the flow is actually clickable in Storybook. This also resolves the closed-state accessibility-tree exposure gap flagged in the a11y migration PR's self-review. Full visual styling (tokens, transitions, `spectrum-css` baseline) stays Phase 5's job; this is positioning/hiding plumbing only. |
| Q25 | Whether `swc-menu`'s surface should use the native Popover API (`popover` attribute) — separate from [Q3](#blockers-and-open-questions)'s "don't wrap `<swc-popover>` as a component" decision, which never addressed this. | **Decided: yes, `popover="auto"`** on `.swc-Menu`, matching `swc-popover`'s own default mode (not `swc-tooltip`'s `manual`). Fixes real top-layer clipping and adds entrance/exit animation; native light-dismiss replaces the hand-rolled outside-click/Escape-closing logic. `_hidden`/`?hidden` binding removed. See [PR #6734](https://github.com/adobe/spectrum-web-components/pull/6734). |
...
| Q22 | Whether `swc-menu` needs its own top-level `disabled` property, and if so what it means now that the trigger is external ([Q16](#blockers-and-open-questions)): it can't mean "disable the rendered trigger" anymore, since there is none — it would have to mean a `Popover`/`Tooltip`-style suppression flag that blocks `open` from taking effect even when the resolved trigger fires. Resurfaced during the API migration ticket: the open/close wiring this question was waiting on now exists (`Menu.base.ts`'s `for`/`triggerElement`/`open` handling), so this is no longer blocked on anything except an actual decision. `disabled` was skipped rather than guessed at. | No | Open — API migration shipped without it; add additively later, no breaking change either way | Architecture reviewer |
| Q23 | (Raised by Ruben, API migration PR review) Should `triggerElement` accept a `VirtualTrigger` (a `getBoundingClientRect()`-only anchor with no real DOM element), matching `Popover.base.ts`'s `triggerElement: HTMLElement \| VirtualTrigger \| null`? A common use case there is anchoring to a cursor position for a right-click context menu. Not a drop-in type widening: the shared `resolveTrigger` utility's `triggerElement` option is typed `HTMLElement \| null` only, and `Popover` bypasses it for the virtual case since ARIA wiring (`aria-controls`/`aria-expanded`/`aria-haspopup`) and the click-to-toggle listener both assume a real element to attach to, neither of which applies to a virtual anchor. | No | Open — deferred; `triggerElement` stays `HTMLElement \| null` for Phase A, add `VirtualTrigger` additively later without a breaking change if a concrete use case shows up | Architecture reviewer |
...
| Q15 | Migration order among `swc-menu`, `swc-menu-item`, `swc-menu-group`, and the new `swc-menu-link` ([Q17](#cross-component-follow-ups-not-blocking-swc-menu)) is not yet decided (`swc-divider` already exists, and `swc-menu-separator` isn't being built — [Q14](#blockers-and-open-questions)). `swc-popover` is **not a sequencing variable** — it shipped in [PR #6356](https://github.com/adobe/spectrum-web-components/pull/6356) (merged 2026-07-14, before this plan was drafted) and `swc-menu` doesn't depend on it anyway. Each menu-family component either has or will have its own plan; sequencing among them is what remains open. **Decided not to block on this** (5t3ph review): rather than pausing `swc-menu`'s Phase A to wait for a `swc-menu-item` plan, this plan now documents the minimal item surface Phase A needs directly — see [Phase A minimal `swc-menu-item` surface](#phase-a-minimal-swc-menu-item-surface). The relative-priority question for full `swc-menu-item` and `swc-menu-link` plans is still open. **Reconfirmed on [PR #6596](https://github.com/adobe/spectrum-web-components/pull/6596)** (rise-erpelding review, non-blocking): asked whether that PR's Setup work should also scaffold `swc-menu-item`'s file structure, since Phase A needs at least a minimal item surface to test against. Answer stands: no, `swc-menu-item` gets its own ticket and PR once one exists; SWC-1983 is scoped to `swc-menu` only, matching the scope correction already made on that PR. **Update:** a migration-prep ticket for `swc-menu-item` has been drafted (Phase 1: formalize the "Phase A minimal `swc-menu-item` surface" table above into a real migration-prep doc and accessibility migration analysis, referencing 1st-gen `MenuItem.ts` and this plan's Q17/Q19 decisions), pending filing. `swc-menu` itself needs no further changes once that component ships — it already collects roving-tabindex targets and restricts its default slot by the `swc-menu-item` tag name, not against anything more specific. | Open — ticket drafted, not yet filed | Ticket owner |
