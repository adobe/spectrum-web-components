...
- [Changes overview](#changes-overview)
    ...
    - [Disposition of remaining gen1 members](#disposition-of-remaining-gen1-members)
...
| `Menu.handleSubmenuOpened` / `handleSubmenuClosed` | `(event: Event) => void` | Public; re-dispatches `sp-menu-submenu-opened` / `sp-menu-submenu-closed` from the overlay target. Tied to the gen1 overlay-based submenu implementation. |
...
| `sp-menu-submenu-opened` | yes / yes | no | overlay target (via `Menu.handleSubmenuOpened`) | Internal to the gen1 overlay-based submenu wiring. |
| `sp-menu-submenu-closed` | yes / yes | no | overlay target (via `Menu.handleSubmenuClosed`) | Internal to the gen1 overlay-based submenu wiring. |
...
| `href`, `download`, `target`, `rel`, `referrerPolicy`, `ping`, `hrefLang` | various | Same link-item surface as gen1's `LikeAnchor` mixin, plus `ping`/`hrefLang` which gen1 `MenuItem` does not have. |
| `shouldCloseOnSelect` | `boolean` | Per-item override of the Menu-level setting. |
| `children` | `ReactNode` | Icon/image, label, description, keyboard-shortcut content — same shape as gen1's `icon`/default/`description`/`value` slots. |
...
| `@spectrum-web-components/overlay` | workspace | gen1 submenu presentation (`sp-overlay` inside `MenuItem`); superseded by direct `PlacementController` use in 2nd-gen, **not** `swc-popover` (see [Architecture](#architecture-core-vs-swc-split)). |
...
<!-- If useful later, the query is `project = SWC AND component = "Menu" AND labels NOT IN (a11y, gen2) AND issuetype NOT IN (Epic, Initiative) AND status != Done` (adjust field names to match the actual Jira schema) — do not reuse the "Related gen1 accessibility (Jira)" table from accessibility-migration-analysis.md, since those rows carry the `a11y` label and are excluded from this table by definition. -->
...
| # | What's added | Note |
| --- | --- | --- |
| **A1** | `xl` size | Figma adds `xl` to gen1's `s`/`m`/`l`; default confirmed `m`, matching React Spectrum S2's `Menu.size` default |
...
### Disposition of remaining gen1 members

`MenuItem`/`MenuGroup`/`MenuDivider` members not listed here are out of scope for this plan.

| gen1 member | Disposition |
| --- | --- |
| `label` | Drop (revised, [Q16](#blockers-and-open-questions)) — gen1's was the menu surface's own accessible name; with the trigger externally referenced instead of shadow-rendered, the accessible name comes from whatever trigger the consumer authored |
...
These are derived from the gen1 implementation, the accessibility migration analysis, the Figma source reviewed for this plan, and the React Spectrum S2 implementation. Confirmed items are marked; open items are tracked in [Blockers and open questions](#blockers-and-open-questions).
...
| Property | Type | Default | Attribute | Notes |
| --- | --- | --- | --- | --- |
| `label` | — | — | — | **Decided: dropped** ([Q16](#blockers-and-open-questions), revised). gen1's `label` was the accessible name for the menu surface itself; with the trigger externally referenced (`for`/`triggerElement`, below) rather than shadow-rendered, the accessible name comes from the trigger's own content or `aria-label`, matching how `Tooltip`/`Popover` don't expose a separate label property for this either. No fallback-text chain to own. |
...
| `trigger` | — | — | — | **Decided: not included** ([Q6](#blockers-and-open-questions)). React Spectrum S2's `MenuTrigger.trigger` has no gen1 equivalent and no current consumer request; `swc-menu` leaves trigger-invocation entirely to how the consumer wires the trigger, matching gen1. Can be added additively later without a breaking change if a concrete need arises. |
| `shouldCloseOnSelect` | `boolean` | `true` (**inferred**, matches gen1's default close-on-select for non-`selects` items) | `should-close-on-select` or similar | **Open question**, tied to [Q2](#blockers-and-open-questions) — only meaningful once selection ships, since gen1's unconditional `close` event today only fires for non-selectable items. React Spectrum S2 exposes this at both `Menu` and `MenuItem` level. |
| `escapeKeyBehavior` | `'clearSelection' \| 'none'` | `'clearSelection'` (React Spectrum S2 default) | `escape-key-behavior` or similar | **Open question** — gen1 has no equivalent; only relevant if selection ships. Carries React Spectrum's own caveat: "Most experiences should not modify this option as it eliminates a keyboard user's ability to easily clear selection" — repeat that caveat in JSDoc if this ships. |
...
| --- | --- | --- | --- | --- |
| Menu group / menu item | Yes (**Confirmed**, Figma) | Yes (**Confirmed**, Figma; gen1 default context) | Yes (**Confirmed**, Figma) | Yes (**Inferred** — new, not in gen1) |
...
- Show leading icon, show drill-in (submenu chevron), show description, show value — **Confirmed**, map to existing gen1 slots/behavior
- Show thumbnail, show highlight badge, show external link, unavailable — **Inferred** additive, no gen1 equivalent ([A3](#deferred-to-later-phases)–[A6](#deferred-to-later-phases))
...
| `swc-close` | yes / yes | Supersedes gen1's unconditional, non-cancelable `close` event; fires when the menu should close (trigger re-press, Escape, non-selectable item activation, outside interaction). **Inferred**. |
| `swc-after-close` | yes / yes | Fires after the close transition completes. **Inferred**. |

Phase C (selection) will add a `change`-equivalent event whenever it's scheduled ([Q2](#blockers-and-open-questions): not blocking); whether it's named `change` (matching React Spectrum S2 and gen1) or a `swc-`-prefixed equivalent is an open question for that follow-up ticket, not this plan.

Submenu open/close events (on `swc-menu-item`) are owned jointly with the `swc-menu-item` plan; this plan only asserts that gen1's `sp-menu-submenu-opened`/`sp-menu-submenu-closed` are superseded by the new anchoring model ([B3](#deferred-to-later-phases)).
...
Initial expectation for Menu is a small reviewed set; the thin gen1 `--mod-*` surface (see [CSS custom properties](#css-custom-properties)) suggests few, if any, are strictly required for parity.
...
- [ ] `Menu.base.ts`: retain open/close orchestration and disallowed-child warnings; add `for`/`triggerElement` properties and `resolveTrigger`-based ARIA wiring (`aria-haspopup="menu"`, `aria-expanded`) onto the resolved `interactiveElement` — no `label` property, no shadow-rendered trigger (decided, [Q16](#blockers-and-open-questions)); do not retain gen1's `selects`/`value`/`selectedItems` — decided to stay Additive/Phase B ([Q5](#blockers-and-open-questions), see [Implementation phasing](#implementation-phasing))
- [ ] *(Phase B, not this pass)* `swc-menu-group` does not share a base class with `swc-menu`; it ships as an independent, lighter-weight component per the decided break from gen1's `MenuGroup extends Menu` inheritance ([B1](#deferred-to-later-phases), [Q11](#blockers-and-open-questions))
...
- [ ] gen1 defects in [Open gen1 issues](#open-gen1-issues) and the [a11y analysis's Jira table](./accessibility-migration-analysis.md#related-gen1-accessibility-jira) are retested or explicitly superseded
...
Ruben's original concern: carrying the `label` property forward unchanged as "accessible name for the trigger" leaves no way to put visible content on the trigger, since gen1 `sp-menu` never had a built-in trigger to put content on in the first place.

**First resolution (superseded):** add a `label` slot for a shadow-rendered trigger, modeled on [gen1 `sp-action-menu`'s `label` slot](../../../../gen1/packages/action-menu/src/ActionMenu.ts).
...
- [gen1 source: `Menu.ts`](../../../../gen1/packages/menu/src/Menu.ts), [`MenuItem.ts`](../../../../gen1/packages/menu/src/MenuItem.ts), [`MenuGroup.ts`](../../../../gen1/packages/menu/src/MenuGroup.ts), [`MenuDivider.ts`](../../../../gen1/packages/menu/src/MenuDivider.ts)
- [gen1 tests](../../../../gen1/packages/menu/test/menu.test.ts) — plus `menu-selects.test.ts`, `submenu.test.ts`, `menu-group.test.ts`, `menu-item.test.ts`, `menu-memory.test.ts`
- [gen1 README](../../../../gen1/packages/menu/README.md)
