...
- It may be a direct child of a [`swc-combobox`](../combobox/accessibility-migration-analysis.md) or a child of a [`swc-option-group`](../option-group/accessibility-migration-analysis.md); the group adds a labeled category around a set of options.
...
- [`swc-option-group`](../option-group/accessibility-migration-analysis.md) — a labeled `role="group"` container that groups a set of `swc-option`s inside the combobox.
...
What `swc-option` must guarantee for that to work: it carries `role="option"` **on the referenceable host** (not on an inner shadow node the parent cannot reach), and its `lang`, name, and `aria-selected`/`aria-disabled` state are all on that same host node. Because the option is the author's real element (slotted into the combobox, or placed in a listbox), there is no duplicate shadow-DOM copy to keep in sync — the defect class 1st-gen's re-render created ([SWC-592](https://jira.corp.adobe.com/browse/SWC-592), [SWC-2359](https://jira.corp.adobe.com/browse/SWC-2359)) does not exist here.
