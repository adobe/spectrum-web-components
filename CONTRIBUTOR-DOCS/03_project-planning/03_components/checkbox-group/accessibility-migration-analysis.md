...
The important thing to hold onto, and the reason this doc reads differently from [radio group](../radio-group/accessibility-migration-analysis.md), is that **the group coordinates presentation, focus, and validation, not value**. Each `swc-checkbox` is independently form-associated and submits its own `name`/`value` (see the [checkbox doc](../checkbox/accessibility-migration-analysis.md#aria-roles-states-and-properties)); the group does not own a single selected value the way a radio group does. Like the radio group, though, the checkbox group **does** manage focus across its items with a roving tabindex and arrow-key navigation via `FocusgroupNavigationController`; the difference is that arrow movement in a checkbox group **only moves focus and never toggles** an item (a user presses <kbd>Space</kbd> to toggle), whereas arrow movement in a radio group both moves focus and changes selection. So the group's accessibility responsibilities are: a fixed `role="group"`, a group accessible name, roving-tabindex focus management, group-level `required`/`invalid`/`read-only` semantics, a description/error-message association, and a disabled cascade to its items.
...
- Not a radio group. It never enforces "only one selected" and never owns a single value; and although both use roving tabindex, arrow movement here only moves focus, while in a radio group it also changes the selection. Keeping the two patterns distinct in the accessibility tree (`group` versus `radiogroup`, per-item versus group-owned value) is the point.
...
- [`swc-radio-group`](../radio-group/accessibility-migration-analysis.md), the single-select sibling. Several controller pieces this doc relies on (`LabellingController`, `SlotAttributePropagationController`, `FocusgroupNavigationController`) are shared with the radio group and text field; the one piece the radio group needs but the checkbox group does **not** (a cache-authoritative selection controller for mutual exclusion) is called out below.
...
- The [Checkbox pattern](https://www.w3.org/WAI/ARIA/apg/patterns/checkbox/) covers the items; a set of related checkboxes is wrapped in a container with `role="group"` and an accessible name (the group label), so assistive technology announces the shared context when a user enters the set. Each checkbox exposes its own state and is toggled with <kbd>Space</kbd>.
- APG's minimal checkbox pattern leaves each checkbox as its own Tab stop, but Spectrum's checkbox group adopts the same **roving tabindex and arrow-key focus management** as the radio group for a consistent grouped-field experience: one Tab stop enters the group and arrow keys move focus between items. The essential difference from the [Radio Group pattern](https://www.w3.org/WAI/ARIA/apg/patterns/radio/) is that arrow movement here **only moves focus** (it never toggles), because checkboxes are multi-select and independent; a radio group's arrow movement additionally changes the single selection.
...
**2nd-gen note:** the roving-tabindex focus mechanics carry over from the radio group: use the same `FocusgroupNavigationController` for one-Tab-stop entry and arrow-key movement across items. What does **not** carry over is the radio group's cache-authoritative selection controller and its select-on-arrow-move behavior: a checkbox group has no single value and no mutual exclusion, so arrow movement moves focus only and each item toggles independently on <kbd>Space</kbd>. Also carried over: the label/description/error wiring (`LabellingController`), the disabled cascade (`SlotAttributePropagationController`), and group-level validity, plus fixing the three 1st-gen gaps above (overridable role, missing `aria-invalid`/`aria-required`, assertive live region).

### Guidelines that apply

| Idea | Plain meaning |
| --- | --- |
| [Checkbox pattern (APG)](https://www.w3.org/WAI/ARIA/apg/patterns/checkbox/) | Related checkboxes sit inside a named `role="group"`; each item is toggled with <kbd>Space</kbd>. Spectrum layers roving-tabindex focus management over this (one Tab stop, arrow keys move focus), with arrow movement moving focus only, never toggling. |
...
| **Roving tabindex and arrow navigation** | Use the already-implemented [`FocusgroupNavigationController`](../../../../2nd-gen/packages/core/controllers/focusgroup-navigation-controller/focusgroup-navigation-controller.mdx) for one-Tab-stop entry and arrow-key movement across items, mirroring `swc-radio-group`'s `wrap: true`, `skipDisabled: true` (confirm whether `direction` should be `'both'` as the radio group uses, or track the group's `orientation`, during implementation). The **critical difference from the radio group**: do **not** wire any select-on-move behavior. The radio group reacts to the controller's `focusgroupNavigationActiveChange` event with `source === 'keyboard'` to change its single selection as focus moves (see the [radio group doc](../radio-group/accessibility-migration-analysis.md#keyboard-and-focus)); a checkbox group must not, because there is no group selection to move. Arrow movement changes only which item has focus; toggling happens per item on <kbd>Space</kbd>. |
| **Form value stays on the items (the key difference from radio group)** | The group does **not** call `setFormValue` and owns **no** single value. Because a checkbox group is multi-select, each `swc-checkbox` is independently form-associated and submits its own `name`/`value` (see the [checkbox doc](../checkbox/accessibility-migration-analysis.md#aria-roles-states-and-properties)). There is no cache-authoritative selection controller and no mutual exclusion here, unlike `swc-radio-group` (this is separate from focus management: the group still uses `FocusgroupNavigationController` for roving tabindex, just not for selection). React Spectrum's group `name` and `value[]`/`onChange` are a convenience view over the items' individual values, not a group-owned form value. |
...
The checkbox group uses the same roving-tabindex focus model as the [radio group](../radio-group/accessibility-migration-analysis.md#keyboard-and-focus), via [`FocusgroupNavigationController`](../../../../2nd-gen/packages/core/controllers/focusgroup-navigation-controller/focusgroup-navigation-controller.mdx). The one difference is that arrow movement here **only moves focus and never toggles** an item.

| Key | Behavior |
| --- | --- |
| <kbd>Tab</kbd> | Moves focus into the group (one Tab stop total, via roving tabindex), landing on the first enabled checkbox (or the last-focused one), and moves focus out of the group entirely. Does not toggle anything. |
| <kbd>ArrowUp</kbd> / <kbd>ArrowDown</kbd> / <kbd>ArrowLeft</kbd> / <kbd>ArrowRight</kbd> | Move focus one step through the item sequence, wrapping at both ends. Moving focus this way **does not** toggle the newly focused item (the essential contrast with the radio group, where arrow movement also changes the selection). |
| <kbd>Home</kbd> / <kbd>End</kbd> | Move focus to the first / last enabled checkbox, without toggling. |
| <kbd>Space</kbd> | Toggles the currently focused checkbox, supplied natively by that item's inner `<input type="checkbox">`. This is the only way to change a value. |
| N/A (disabled items) | A disabled checkbox is skipped by Tab-entry, arrow movement, and Home/End (`skipDisabled: true`). |

Use `FocusgroupNavigationController` (`wrap: true`, `skipDisabled: true`, with `direction` confirmed during implementation) for the movement mechanics, and, unlike the radio group, do **not** react to its `focusgroupNavigationActiveChange` event to change any selection: focus movement and toggling are fully decoupled here. When the group is read-only, keep all focus movement above and suppress only the <kbd>Space</kbd> toggle on each item.
