...
**2nd-gen note:** replace `RovingTabindexController` with the already-implemented `FocusgroupNavigationController` ([mdx](../../../../2nd-gen/packages/core/controllers/focusgroup-navigation-controller/focusgroup-navigation-controller.mdx)) configured as `direction: 'both'`, `wrap: true`, `skipDisabled: true`. Verify one behavioral detail before relying on it as a drop-in replacement: `FocusgroupNavigationController` exposes an `onActiveItemChange` callback / `focusgroupNavigationActiveChange` event when "the active item changes," but its docs don't specify whether that fires only on arrow-key-driven movement or also on the initial roving-tabindex positioning that happens when focus first enters the group (which must *not* select). If the controller doesn't already distinguish those two cases, `swc-radio-group` (or its composing controller) must call the select hook only from the arrow-key path, not from every active-item change, to preserve the "Tab-entry never selects, arrows always select" split confirmed above.
