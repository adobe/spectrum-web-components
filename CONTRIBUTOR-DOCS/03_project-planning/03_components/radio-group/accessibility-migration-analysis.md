...
- Sets `role="radiogroup"` on its own host via `ElementInternals` (`internals.role = 'radiogroup'`), using the forms strategy RFC's host-role exception (see [§3.2](../../05_strategies/forms-strategy-rfc.md#32-where-aria-roles-live)): the group has no native container element to render, so the role goes on the host instead. This is unlike `swc-radio`, which *does* have a native equivalent (`<input type="radio">`) and so renders its role on an inner shadow-DOM input rather than its own host (see the [radio doc](../radio/accessibility-migration-analysis.md#aria-roles-states-and-properties)).
...
- Not a generic `role="group"` wrapper for arbitrary form controls. gen1's `sp-radio-group` extends the shared `FieldGroup`/`sp-field-group` base (which defaults to `role="group"`) and overrides it to `role="radiogroup"` specifically because it only ever contains radios. `swc-radio-group` should not be generalized back into a plain field group; keep the fixed `radiogroup` role.
...
gen1 `sp-radio-group` already implements the full APG model above using `RovingTabindexController` from `@spectrum-web-components/reactive-controllers`, confirmed directly against its test suite:
...
| [Labels or instructions (WCAG 3.3.2)](https://www.w3.org/WAI/WCAG22/Understanding/labels-or-instructions.html) | The group as a whole needs a visible, programmatically associated label, not just labels on the individual radios. [SWC-1178](https://jira.corp.adobe.com/browse/SWC-1178) tracks a report of a missing visible group label in gen1's anatomy documentation; verify against the live `sp-radio-group` API (which does support a `label` property/attribute) rather than assuming the underlying capability is missing, but treat the report as a signal to double-check every 2nd-gen anatomy and Storybook example actually supplies one. |
...
| [Error identification (WCAG 3.3.1)](https://www.w3.org/WAI/WCAG22/Understanding/error-identification.html) | When the group is invalid, that must be exposed via `aria-invalid` on the group host, not only through visible error-message styling. gen1 never sets this (see [ARIA roles, states, and properties](#aria-roles-states-and-properties)); this is a real gap to fix, not a pattern to carry forward. |
...
| Topic | What to do |
| --- | --- |
| **Host role** | `role="radiogroup"`, set via `ElementInternals` (`internals.role = 'radiogroup'`), fixed and never author-overridable. gen1 achieves the same fixed role imperatively (`this.setAttribute('role', 'radiogroup')` in `willUpdate`); the 2nd-gen version should use the host-role exception from the forms strategy RFC's [naming table](../../05_strategies/forms-strategy-rfc.md#4-naming-table) instead of a plain attribute write. |
| **Accessible name** | In precedence order: `accessible-labelledby` (composing a name from elements the group doesn't own) > `accessible-label` (no visible label) > a visible label rendered inside the group's own shadow root (the common case, matching the Figma "top label"/"side label" positions). Wired by the **`LabellingController`** ([SWC-2466](https://jira.corp.adobe.com/browse/SWC-2466), *pending research*). Because the group's role lives on its own host (unlike `swc-radio`, whose role and name both live on its inner input; see the [radio doc](../radio/accessibility-migration-analysis.md#shadow-dom-and-cross-root-aria-issues)), the controller resolves all three sources against the host's own `ElementInternals` (`internals.ariaLabel`, `internals.ariaLabelledByElements`) instead of an inner control. See [Shadow DOM and cross-root ARIA Issues](#shadow-dom-and-cross-root-aria-issues). gen1's `FieldGroup` base sets `aria-label` directly from a `label` property; that specific mechanism (a plain string, not an IDREF) already works correctly across the shadow boundary today, so this is a wiring-location change, not a bug fix. |
...
| **`aria-invalid` (add — real gen1 gap)** | gen1 `sp-radio-group` has an `invalid` property, but only uses it to switch which help-text slot renders (`negative-help-text` vs. `help-text`) and to drive `:invalid`-style CSS; it never sets `aria-invalid` on the group host itself (confirmed: no `aria-invalid` handling anywhere in `FieldGroup.ts`, `RadioGroup.ts`, or the shared `HelpTextManager`). This is a real [WCAG 3.3.1](https://www.w3.org/WAI/WCAG22/Understanding/error-identification.html) gap: a screen reader user gets the error text (via `aria-describedby`, see below) but never gets the `invalid` *state* itself unless they read the description. Set `internals.ariaInvalid` (or the equivalent `aria-invalid` mapping) alongside the error-message wiring; do not carry the gen1 omission forward. |
| **`aria-readonly` (relocate here from `swc-radio`)** | `aria-readonly` is valid on the `radiogroup` role per the [WAI-ARIA global states table](https://www.w3.org/TR/wai-aria-1.2/#global_states); it is *not* valid on the `radio` role. Implement React Spectrum's `isReadOnly` here, on `swc-radio-group`, not per item (see the [radio doc's note](../radio/accessibility-migration-analysis.md#aria-roles-states-and-properties) on gen1's unenforced per-radio `readonly`). When read-only, every item stays focusable and roving tabindex still works, but neither click, `Space`, nor arrow-key movement changes the checked item — read-only blocks the *selection* half of the interaction model, not the *focus* half. |
...
| **Roving tabindex and arrow navigation** | Use the already-implemented [`FocusgroupNavigationController`](../../../../2nd-gen/packages/core/controllers/focusgroup-navigation-controller/focusgroup-navigation-controller.mdx) with `direction: 'both'`, `wrap: true`, `skipDisabled: true`. See [First-gen keyboard model](#first-gen-sp-radio-group-keyboard-model) above for exactly which gen1 behaviors these options need to reproduce, and its note on reacting only to the `focusgroupNavigationActiveChange` event's `source: 'keyboard'` case (confirmed against the controller's own source and the PoC's `radio-group-controller.js`) to keep Tab-entry from selecting. |
| **Attribute propagation (`disabled`, not `name`)** | Use the already-implemented [`SlotAttributePropagationController`](../../../../2nd-gen/packages/core/controllers/slot-attribute-propagation-controller/slot-attribute-propagation-controller.mdx) to propagate the group's `disabled` onto every slotted radio, since the group has no native fieldset-like disabling mechanism of its own. `name` is **not** propagated, replacing the PoC's flagged hand-rolled `slotchange -> setAttribute('name', ...)` loop entirely rather than porting it onto the shared controller: per Q12 (officially signed off, see the [radio migration plan](../radio/migration-plan.md#decision-log)), no radio participates in form value at all, so setting `name` on an item's inner input would be purely cosmetic, with no functional benefit. |
| **Form association — value and validity, one instance (Q12, officially signed off)** | React Spectrum's `isRequired`/`isInvalid`/`validate`/`validationBehavior` live on `RadioGroup`, not `Radio`. `swc-radio-group` needs its own `ElementInternals` participation for both constraint validation (`setValidity`/`checkValidity`/`reportValidity`, `:invalid`/`:user-invalid` matching) **and** value submission (`setFormValue`, driven by the group's own `selected`): per the [radio migration plan](../radio/migration-plan.md#decision-log) (Q12), `swc-radio` has no `FieldAssociationController` of its own at all, not just no value-submission role — a native `<input type="radio">` inside its own shadow root is invisible to an ancestor `<form>` regardless of where `ElementInternals` attaches, so per-item form association gains nothing, and centralizing at the group avoids keeping N per-item `ElementInternals` instances in lockstep with the group's own `selected` state. One `FieldAssociationController` instance on the group handles both. This is resolved independently of [SWC-2470](https://jira.corp.adobe.com/browse/SWC-2470), which concerns sibling-discovery/mutual-exclusion composition only, not form association. |
| **Disabled cascade** | Since no radio is independently form-associated (Q12, above), there is no per-item `formDisabledCallback` for an ancestor `<fieldset disabled>` or owning form's disabled cascade to reach on its own. The group must propagate its own explicit `disabled` property down to its items itself (see **Attribute propagation** above). |
| **Coordinated reset** | `formResetCallback()` fires only on the group (Q12: no radio has its own form association to fire it on). "The default checked item" is a group-level fact (which radio had `checked`/was the initial selection); the group tracks the default selection itself and drives every item's `checked` back to it on reset, matching the PoC's description of `SelectionController` (which sibling). Whether that sibling-tracking piece ships as a new shared controller or is composed inline is [SWC-2470](https://jira.corp.adobe.com/browse/SWC-2470)'s open question; the reset invariant itself (restore the default-checked item, not just clear the selection) does not depend on that outcome. |

### Shadow DOM and cross-root ARIA Issues

The `LabellingController` ([SWC-2466](https://jira.corp.adobe.com/browse/SWC-2466), *pending research*) attaches the group's label, description, and error message directly to `swc-radio-group`'s own host via `ElementInternals` (`internals.ariaLabel`, `internals.ariaLabelledByElements`, `internals.ariaDescribedByElements`), because `role="radiogroup"` lives on the host rather than an inner control. This is different from `swc-radio`, where (per the confirmed PoC pattern in the [radio doc](../radio/accessibility-migration-analysis.md#shadow-dom-and-cross-root-aria-issues)) the same controller wires the equivalent `ariaLabelledByElements`/`ariaDescribedByElements` properties onto its inner `<input type="radio">` instead of its host, since that input is the role-bearing element there. `LabellingController`'s eventual API needs to support both an "attach to host" mode (for container roles like `radiogroup` with no native element) and an "attach to an inner control" mode (`swc-text-field`, `swc-radio`); confirm this during Phase 3/4 implementation.

Separately, and worth stating explicitly because the forms strategy RFC's [§3.3](../../05_strategies/forms-strategy-rfc.md#33-idref-strategy-label-help-text-and-errors) describes a real cross-root violation for `sp-field-label`/`sp-textfield`: **gen1's radio-group help-text association does not have that problem today.** `HelpTextManager` in `"external"` mode sets `aria-describedby` on the group's own host, pointing at the `id` of the *slotted* (light-DOM) help-text element, and both the attribute and the referenced `id` live in the same tree (the group's light DOM), so the IDREF resolves correctly without crossing a shadow boundary. When re-implementing this via `LabellingController`, preserve that correctness property: don't accidentally introduce a cross-root break by moving the description into the group's own shadow root without also giving it the equivalent host-attached element-reference wiring described above.

**Dev-warning validation is not blocked by the host-role pattern.** Even though `role="radiogroup"` lives on the host with no inner shadow-DOM control to observe, the accessible-name inputs feeding that role (`accessible-label` / `accessible-labelledby`, which `LabellingController` wires into `internals.ariaLabel` / `internals.ariaLabelledByElements`) are themselves ordinary host-level `@property` fields on `swc-radio-group`, exactly like on any other component. A "missing accessible name" dev-warning (see the dev-warning story noted in [Testing](#automated-tests)) can call the shared `warnIf` helper directly from the host's own lifecycle (`firstUpdated`/`updated`), the same way existing host-role components already do it: [`Button.base.ts`](../../../../2nd-gen/packages/core/components/button/Button.base.ts) warns when an icon-only button has no `accessible-label`, and [`Tabs.base.ts`](../../../../2nd-gen/packages/core/components/tabs/Tabs.base.ts) warns whenever its own `accessibleLabel` is empty. Neither needs an inner element to hang the warning off, because the property being validated already lives on the host; the same holds for `swc-radio-group`.
...
- **Invalid:** `aria-invalid="true"` on the group host (the gen1 gap this doc adds — see [ARIA roles, states, and properties](#aria-roles-states-and-properties)); error message text both visible and reachable via `aria-describedby`/`aria-errormessage`.
...
Implement the full [APG radio pattern](https://www.w3.org/WAI/ARIA/apg/patterns/radio/), reproducing the specific gen1 behaviors confirmed in [First-gen keyboard model](#first-gen-sp-radio-group-keyboard-model):
...
| --- | --- |
| **Unit** | Exactly one child ever has `aria-checked="true"` after any selection change; `aria-invalid`, `aria-required`, and `aria-readonly` on the host track their respective properties; `disabled` propagates to every slotted radio (`name` does not — see **Attribute propagation**); a reset restores the item that was checked by default, not just clears the current selection. |
...
- [ ] Accessible name precedence (`accessible-labelledby` > `accessible-label` > visible label) resolves through the host's own `ElementInternals`, the host-attached counterpart to the inner-input-attached mechanism the [radio doc](../radio/accessibility-migration-analysis.md) uses for `swc-radio`.
- [ ] `aria-invalid` is added on the group host when invalid — a real gen1 gap, not previously implemented at all.
- [ ] `aria-readonly` is implemented once, correctly, at the group level (not the unenforced per-radio version gen1 shipped).
- [ ] `aria-required` reflects the group's `required`/`isRequired`.
- [ ] Description/help text and error message render inside the group's own shadow root and associate via host-attached `aria-describedby`/`aria-errormessage`, preserving the fact that gen1's light-DOM-slot association already resolves correctly (no cross-root regression introduced).
...
- [ ] Arrow/Home/End movement always selects the newly focused item; Tab-entry and programmatic `.focus()` never do. Implemented by reacting only to `focusgroupNavigationActiveChange`'s `source === 'keyboard'`, matching the PoC's `radio-group-controller.js`.
- [ ] `disabled` propagates to every slotted radio via `SlotAttributePropagationController`; `name` does not (see [radio-group migration plan](./migration-plan.md#decision-log)).
- [ ] One `FieldAssociationController` instance on the group handles both value submission (`setFormValue`, from `selected`) and validity (`setValidity`/`checkValidity`); no radio has a `FieldAssociationController` of its own (Q12, officially signed off — see the [radio migration plan](../radio/migration-plan.md#decision-log)).
...
- gen1: [`sp-radio`/`sp-radio-group`](../../../../gen1/packages/radio/README.md), [`sp-field-group`](../../../../gen1/packages/field-group/README.md), [`sp-help-text`](../../../../gen1/packages/help-text/README.md)
