...
| Topic | What to do |
| --- | --- |
| Host / surface | `role="group"` on the shadow **container** that wraps the **label** region and the **default** (items) **slot** (or equivalent wiring—verify in 2nd-gen source). The containing `role="menu"` must be the internal menu node in `swc-menu` / `swc-action-menu` / `swc-menu-item`’s shadow tree (not the CE host). |
