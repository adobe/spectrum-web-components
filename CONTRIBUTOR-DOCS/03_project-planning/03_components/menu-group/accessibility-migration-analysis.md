...
This document sets accessibility expectations for 2nd-gen **Menu group** in Spectrum Web Components: **`swc-menu-group`** is a labeled grouping of [`menuitem`](https://www.w3.org/WAI/ARIA/apg/patterns/menu-button/) rows. **Parent requirement:** it can **only** be a **slotted** **direct** **list** **child** of **`swc-menu`**, **`swc-action-menu`**, or **`swc-menu-item`** (the **submenu** list: a row’s parent `swc-menu-item` hosts the child `role="menu"` in its shadow tree when the `submenu` slot is used; verify slotting in 2nd-gen source). It is **not** a **slotted** **child** of **another** **`swc-menu-group`**, a stand-alone field group, or page-level sectioning. **Slots (current migration scope):** the **`label` slot** provides the **section** **label**; the **`default` slot** supports **only** **`swc-menu-item`** as **row** **children** ( **not** **nested** **`swc-menu-group`**, **not** **`swc-menu-separator`**—section **dividers** use [`swc-menu-separator`](../menu-separator/accessibility-migration-analysis.md) as a **sibling** in the **parent** **`swc-menu` / `swc-action-menu` / `swc-menu-item`** list, **verify** in 2nd-gen **source**). That usage composes with the **internal** [`role="menu"`](../menu/accessibility-migration-analysis.md) **surface**—the `role="menu"` **element** lives in the relevant parent host’s shadow tree, not on the CE host (see [Menu](../menu/accessibility-migration-analysis.md) and [Menu item](../menu-item/accessibility-migration-analysis.md)). The component does not own keyboard or roving focus—the parent menu host and [`FocusgroupNavigationController`](https://github.com/adobe/spectrum-web-components/pull/6129) do (see [Keyboard and focus](#keyboard-and-focus)). In tests, use a harness that mirrors the same slotted placement and internal `role="menu"` shape. The target is **WCAG 2.2 Level AA**.
...
- [Menu separator accessibility migration analysis](../menu-separator/accessibility-migration-analysis.md) for **`swc-menu-separator`**: direct **list** child of **`swc-menu`**, **`swc-action-menu`**, or **`swc-menu-item`** only (not slotted into a **group**’s default slot).
...
- A **group** is a logical section of a menu: a container with [`role="group"`](https://www.w3.org/WAI/ARIA/apg/patterns/menu-button/) **only** when it is **slotted** **as** a **direct** **list** **child** of **`swc-menu`**, **`swc-action-menu`**, or **`swc-menu-item`**, **composing** with that **parent**’s **internal** `role="menu"` **surface** in **shadow** **DOM** (see [Overview](#overview)), optionally with a visible **label** **slot** **face**. The **default** **slot** **(current** **scope**) **only** **accepts** **`swc-menu-item`**. The **group** is **not** a **`menuitem`**; it does **not** own keyboard handling or roving `tabindex`; the parent host applies [`FocusgroupNavigationController`](https://github.com/adobe/spectrum-web-components/pull/6129) to **default-slot** **`swc-menu-item`** **rows** (see [Keyboard and focus](#keyboard-and-focus)).
- **Nesting (authoring):** do not place one `swc-menu-group` **inside** **another** **`swc-menu-group`’s** **default** **slot** (or otherwise **nest** **groups**). Sibling `swc-menu-group` and `swc-menu-item` rows at the same **list** **level** are **valid**; a **submenu** (child `role="menu"` on `swc-menu-item`) is a **separate** **menu** **surface** and may **include** **its** **own** **`swc-menu-group`** **as** **direct** **children** of **that** **submenu** **list** only. For **`swc-menu-item` rows** **slotted** **under** **a** **group**’s **default** **slot**, **`label` / `submenu` slot** **authoring** on **items**, and the **`submenu` slot** **allow**-**list** on **`swc-menu-item`**, see [Menu item — What `swc-menu-item` is (2nd-gen)](../menu-item/accessibility-migration-analysis.md#what-swc-menu-item-is-2nd-gen) (verify in 2nd-gen source and stories).
- **Slots (2nd-gen):** a **`label` slot** (section title / face) and a **default** slot that **(current** **migration**)** is **intended** **for** **only** **`swc-menu-item`**. Verify slot names and any future API expansion in 2nd-gen source.
...
Bottom line: `swc-menu-group` supplies structure and section labeling (shadow `role="group"`, `role="presentation"` label slot wrapper, default slot for rows) and does not own in-menu keyboard or focus—the **parent** **`swc-menu`**, **`swc-action-menu`**, or **`swc-menu-item`** (submenu) list and [`FocusgroupNavigationController`](https://github.com/adobe/spectrum-web-components/pull/6129) do. Submenu and command row detail stay on the menu, menu item, and action menu docs.
...
Scope: **`role="group"`** (or equivalent), **slotted** **only** **as** a **child** of **`swc-menu`**, **`swc-action-menu`**, or **`swc-menu-item`**, with **label** and **default** **slots** as in [What `swc-menu-group` is (2nd-gen)](#what-swc-menu-group-is-2nd-gen). **Default** **slot** **(current** **scope**)**: **only** **`swc-menu-item`**. **Optional** visible **label** **content**; **`menuitemcheckbox` / `menuitemradio`** **selection** **rows** are **out** of **scope** until [Menu migration scope](../menu/accessibility-migration-analysis.md#migration-scope-current) closes.

### ARIA roles, states, and properties

| Topic | What to do |
| --- | --- |
| Host / surface | **Slotted** **only** **as** a **direct** **list** **child** of **`swc-menu`**, **`swc-action-menu`**, or **`swc-menu-item`** ( **submenu** list ). **Not** a **child** of **another** **`swc-menu-group`**. `role="group"` on the shadow **container** that wraps the **label** region and the **default** (rows) **slot** (or equivalent wiring—verify in 2nd-gen source). The containing `role="menu"` must be the internal menu node in that **parent** host’s shadow tree (not the CE host). |
| Slots | **Label** slot: projects into the `role="presentation"` **label** wrapper (next row). **Default** slot **(current** **migration**)**: **only** [`swc-menu-item`](../menu-item/accessibility-migration-analysis.md)—**not** **nested** **`swc-menu-group`**, **not** [`swc-menu-separator`](../menu-separator/accessibility-migration-analysis.md) (separators are **siblings** in the **parent** list—see the separator doc), **other** **primitives**, or **ad-hoc** **content** (see [What `swc-menu-group` is (2nd-gen)](#what-swc-menu-group-is-2nd-gen); verify in 2nd-gen source). |
| Label region (in shadow) | A `div` (or equivalent) with `role="presentation"`, a **stable** `id`, and the **label** **slot** as its content. It is **not** a `menuitem`. The `role="group"` **ancestor** in the same shadow **subtree** sets `aria-labelledby` to that `id` (IDREF is **local** to `swc-menu-group`’s **composition**—see [Shadow DOM](#shadow-dom-and-cross-root-aria-issues)). The label row is **not** a roving `tabindex` stop; roving `tabindex` **targets** **`swc-menu-item` rows in the default slot** (and `separator` **rows** in the **parent** **list** **outside** the **group**—[Menu — Keyboard and focus](../menu/accessibility-migration-analysis.md#keyboard-and-focus)). |
...
- [ ] **Slotted** **only** **as** a **direct** **list** **child** of **`swc-menu`**, **`swc-action-menu`**, or **`swc-menu-item`** ( **submenu** )—**not** **under** **another** **`swc-menu-group`**. The group is used only inside the internal `role="menu"` subtree of that **parent** (shadow surface, not the menu CE host). Shadow structure: `role="group"`, `role="presentation"` label wrapper (**label** slot) with stable `id`, **default** slot **(current** **scope**)**: **only** **`swc-menu-item`**; `aria-labelledby` on the group node IDREFs that `id` (see [What `swc-menu-group` is (2nd-gen)](#what-swc-menu-group-is-2nd-gen)); `aria-label` when used matches visible title. **Submenus** ( **separate** **`role="menu"`** ) may **include** **their** **own** **`swc-menu-group`** **as** **direct** **children** of **that** **list** per [Nesting (authoring)](#what-swc-menu-group-is-2nd-gen). **[`swc-menu-separator`](../menu-separator/accessibility-migration-analysis.md) is not a default-slot child of the group.**
