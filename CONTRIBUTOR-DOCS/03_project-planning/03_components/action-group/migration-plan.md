...
| `@spectrum-web-components/reactive-controllers/src/RovingTabindex.js` | workspace | `RovingTabindexController` — replaced in gen2 by `FocusgroupNavigationController` |
...
| `swc-segmented-control` | Alternative for exclusive-selection UX | Not migrated yet in gen2; consumers needing exclusive-choice UX should use this when it ships |
| `swc-toggle-button-group` | Alternative for toggle-selection UX | Not migrated yet in gen2; consumers needing toggle/multi-select UX should use this when it ships |
...
| **B2** | `--mod-*` CSS custom properties dropped | Nine `--mod-actiongroup-*` modifier properties | No `--mod-*` properties; additive `--swc-*` set introduced | Remove all `--mod-actiongroup-*` overrides; wait for announced `--swc-*` properties. Standard gen2 change. |
...
| # | What is added | Notes |
| --- | ------------- | ----- |
| **A1** | `emphasized` group-level propagation | `emphasized` is deprecated in 1st-gen `sp-action-button` with a `window.__swc.warn()` runtime warning and has no announced timeline for gen2. If `swc-action-button` adds it in a future release, action-group propagation follows as an additive change. |
| **A2** | Consumer migration guidance for `swc-segmented-control` / `swc-toggle-button-group` | `selects` and `selected` are dropped. When those components ship in gen2, the consumer migration guide can document the upgrade path. No code change to action-group. |
...
- **Dropped**: explicitly removed; not implemented in gen2
...
| `disabled` | `boolean` | `false` | `disabled` | **Confirmed** (a11y analysis). New in gen2. Uses `aria-disabled="true"` on host and propagates to children; children remain keyboard-reachable per APG guidance (SWC-621). |
| `justified` | `boolean` | `false` | `justified` | **Inferred** (SWC convention). Children fill available width equally. React S2 uses `isJustified`; gen2 SWC convention drops the `is` prefix on booleans; `justified` follows that pattern. |
| `accessibleLabel` | `string` | `''` | `accessible-label` | **Confirmed** (implemented, shipped as `accessible-label`, not `label`). Reflects to `aria-label` on host. Named `accessible-label` rather than `label` to keep the attribute unambiguous as accessible-name-only. It produces no visible rendering, unlike properties elsewhere in gen2 that use "label" to mean visible text. Matches the same rename already shipped for `swc-action-button` (see its migration plan's B7). Recommended whenever the strip has a distinct purpose. |
...
| `emphasized` | `boolean` | `false` | `emphasized` | **Dropped**. `emphasized` is deprecated in 1st-gen `sp-action-button` with `window.__swc.warn()` and has no announced gen2 timeline. Figma shows no group-level emphasized style. Not implemented in gen2 action-group. |
...
`swc-action-group` owns composite keyboard navigation: one Tab stop into the strip, arrow keys move among enabled `swc-action-button` and `swc-action-menu` children. In gen2, `RovingTabindexController` is replaced by `FocusgroupNavigationController`, with `direction` set to match `orientation`.
...
`delegatesFocus: true` is preserved from 1st-gen. The explicit `focus()` override in `ActionGroup.ts` handles programmatic focus calls by delegating to `FocusgroupNavigationController.getActiveItem()`.

Mouse click updates the roving `tabindex="0"` to the clicked item (SWC-250 fix required; the 1st-gen mouse test is `it.skip`).

#### Child propagation

On each slot change and on changes to `quiet`, `size`, `staticColor`, `disabled`, and `compact`, the group propagates the relevant state to each managed child. The managed child selector in gen2 is `swc-action-button, swc-action-menu`.
...
- **`FormFieldMixin`:** Must not be applied. `swc-action-group` is a composite keyboard widget, not a form field. SWC-1612 is a ticket that uses `FormFieldMixin` for `sp-action-group`; that must not carry into the gen2 migration. The `label` property could be mistaken for a field-label association, but it is used only to provide `aria-label` on the group — not for form association.
...
| Layer | Path | Contains |
| -------- | ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| **Core** | `gen2/packages/core/components/action-group/` | `ActionGroup.base.ts`, `ActionGroup.types.ts`. Child collection logic, `label` → `aria-label`, `disabled` propagation contract, dev warnings. No rendering. |
| **SWC** | `gen2/packages/swc/components/action-group/` | `ActionGroup.ts`, `action-group.css`, element registration (`swc-action-group`), stories, tests. Renders `<slot role="presentation">`. Wires `FocusgroupNavigationController`. Propagates `compact` position mechanism, `quiet`, `size`, `staticColor` to slotted children. |
...
- [x] Create `gen2/packages/core/components/action-group/`
- [x] Create `gen2/packages/swc/components/action-group/`
- [x] Wire exports in both `package.json` files
- [x] Check out `spectrum-css` at `spectrum-two` branch as sibling directory (required for Phase 5 styling)
...
- [x] `emphasized` deprecated in 1st-gen `sp-action-button` with `window.__swc.warn()`; not in gen2 `swc-action-button`; no group-level propagation
...
- [x] Add `.swc-ActionGroup` to the internal semantic element in `render()`; keep layout styling off `:host` where possible
- [x] Copy S2 source from `spectrum-css` `spectrum-two` branch `components/action-group/index.css` (not `/dist`) into `action-group.css` as baseline
- [x] Translate `.spectrum-ActionGroup-item` child selectors to `::slotted(*)` equivalents for gap, margin, and border-between adjustments
- [x] Implement compact mode via CSS custom property cascade: set the four logical corner-radius properties (`--swc-action-button-border-start-start-radius`, `-start-end-radius`, `-end-start-radius`, `-end-end-radius`) directly per position — `::slotted(:first-child:not(:last-child))` zeroes only its two interior corners, `::slotted(:last-child:not(:first-child))` zeroes its two interior corners, and `::slotted(:not(:first-child, :last-child))` zeroes all four for middle children. A lone child (both first and last) matches none of these selectors and correctly keeps its natural full radius.
...
- [x] Verify compact mode disables correctly when `quiet` is also set (same as 1st-gen: compact has no visual effect in quiet mode)
- [x] Verify `justified` layout (children fill available width)
- [x] Verify `orientation="vertical"` layout and compact + vertical combined mode
- [x] Add `@cssprop` JSDoc tags for each exposed `--swc-*` property
...
- [x] Initial focus target is first enabled child (no selection concept in gen2)
- [x] Mouse click updates `tabindex="0"` to clicked item (SWC-250 fix; port `it.skip` test)
- [x] Focus ring visible on focused child; no z-index stacking hides indicator (SWC-1342 fix) — Phase 5
...
- [ ] `swc-action-menu` in group: keyboard opens menu; Escape closes; focus returns to trigger; roving sequence intact — blocked: `swc-action-menu` does not exist yet in gen2 (only `action-button` and `action-group` are built); revisit once it ships
...
- [x] Consumer migration guide: `selects` and `selected` removed. Consumers using selection UX should migrate to `swc-toggle-button-group` (toggle/multi) or `swc-segmented-control` (exclusive choice) when those ship in gen2
...
- SWC-2212: Action Group gen2 migration epic
- SWC-2213: Accessibility recommendations for gen2 action-group migration (complete)
...
- SWC-1612: Migrate to `FormFieldMixin` (must NOT apply to gen2 action-group)
