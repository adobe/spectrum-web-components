...
- [gen2 API decisions](#gen2-api-decisions)
    ...
    - [Pending accessible-name rule (gen2)](#pending-accessible-name-rule-gen2)
    - [Accessibility semantics notes (gen2)](#accessibility-semantics-notes-gen2)
    - [Deferred semantics note (gen2)](#deferred-semantics-note-gen2)
...
- `sp-button` is planned to become button-only in gen2; deprecated link mode moves to native `<a>` with global button styling
- gen2 `sp-button` should render an internal semantic `<button>` and move semantics off the host
...
- Component Button styles and [`global-button.css`](../../../../gen2/packages/swc/stylesheets/global/global-button.css) should share source/imports if possible
...
| `noWrap` | `boolean` | `false` | `no-wrap` | 1st-gen off-spec escape hatch to disable wrapping; candidate for rename in gen2. |
...
| **A5** | Host `focus` / `blur` compatibility parity | Deferred to `SWC-2035`; initial gen2 Button will document `click` plus bubbling `focusin` / `focusout` as the supported host-listener contract. |
...
| **A7** | `justified` — full-width layout mode | Not present in 1st-gen. Added in gen2 based on a post-planning S2 Design update. Requires the container to allow stretching. |
...
| `type` | deferred beyond initial scope | `'button'` | maybe none / future | **Deferred.** Initial gen2 Button should behave as a regular button; `submit` / `reset` are future work. |
...
### Pending accessible-name rule (gen2)
...
Cross-root `aria-labelledby` / `aria-describedby` support and form-associated `submit` / `reset` behavior are deferred beyond the initial Button migration scope. They depend on a clearer ElementInternals/platform/tooling recommendation and should not block the first gen2 Button release.
...
| Layer | Path | Contains |
|---|---|---|
| **Core** | `gen2/packages/core/components/button/` | `Button.base.ts`, `Button.types.ts`, validation, state, accessible-name logic, attribute forwarding, pending-label behavior, and other reusable semantic rules. No rendering. |
| **SWC** | `gen2/packages/swc/components/button/` | `Button.ts`, `button.css`, element registration, stories, tests, and the specific S2 rendering/styling for `sp-button`. |
...
- If navigation is removed as planned, no anchor render path is needed in gen2 `sp-button`
- The component stylesheet and [`global-button.css`](../../../../gen2/packages/swc/stylesheets/global/global-button.css) should share source/imports if possible so button-styled global elements stay visually in sync with the component
- The current contents of [`global-button.css`](../../../../gen2/packages/swc/stylesheets/global/global-button.css) are temporary POC scaffolding, not a locked design contract; they may be fully replaced during the Button migration as long as the resulting global-element API stays aligned with the component
...
This keeps the gen2 `core` / `swc` split intact: `core` owns semantic behavior, while each downstream SWC component can render its own control and styling on top of that shared contract.
...
- [x] Create `gen2/packages/core/components/button/`
- [x] Create `gen2/packages/swc/components/button/`
- [x] Wire exports in both `package.json` files
- [x] Implement Button styles so the component stylesheet and [`global-button.css`](../../../../gen2/packages/swc/stylesheets/global/global-button.css) share source/imports if practical — resolved by the `vite-global-elements-css` plugin, which auto-generates `global-button.css` from `button.css` on every dev/build run
- [x] Treat the current [`global-button.css`](../../../../gen2/packages/swc/stylesheets/global/global-button.css) implementation as replaceable POC code, not as the canonical source of Button styling — `button.css` is the source of truth; `global-button.css` is a generated output and is marked `DO NOT EDIT`
...
- [x] Rename legacy `noWrap` to `truncate` in the gen2 API — gen2 `Button.ts` exposes `truncate`; `no-wrap` is deprecated in 1st-gen with `@deprecated` JSDoc and `window.__swc.warn()`
...
- [x] Remove `label` in favor of `accessible-label` / `accessibleLabel` — gen2 `ButtonBase` exposes `accessibleLabel` (attribute: `accessible-label`) forwarded as `aria-label` on the internal `<button>`; no `label` prop in gen2
- [x] Remove deprecated link API (`href`, `target`, `download`, `referrerpolicy`, `rel`) from the gen2 public surface — absent from gen2 `Button.ts`
- [x] Remove deprecated `variant` aliases (`cta`, `overBackground`, `white`, `black`) from the gen2 public surface — already absent in gen2 `Button.ts`
- [x] Do not carry forward `quiet` as a gen2 visual API — `quiet` is absent from gen2; deprecated in 1st-gen with `@deprecated` JSDoc and `window.__swc.warn()`
...
- [x] Reconcile the component stylesheet with existing [`global-button.css`](../../../../gen2/packages/swc/stylesheets/global/global-button.css) — handled by the `vite-global-elements-css` plugin; `global-button.css` is now auto-generated from `button.css` on every dev/build run and is marked `DO NOT EDIT`
...
- [x] Remove or replace tests that depend on deprecated `href` mode — gen2 tests were written fresh; no href-dependent tests exist
...
- [x] Document that `quiet` is not part of the gen2 visual API — **deferred to `consumer-migration-guide`** (migration note, not relevant to gen2-only consumers)
...
- [x] Document that gen2 Button semantics and focus land on a real internal native `<button>`, not the custom-element host — documented in Accessibility story (Features #1 and #2)
...
|---|---|---|---|
| SWC-2033 | Cross-root ARIA support | Depends on a clearer cross-root semantics approach and should not block the initial Button migration. | [Additive](#additive--ships-when-ready-zero-breakage-for-consumers-already-on-gen2), [Deferred semantics note (gen2)](#deferred-semantics-note-gen2) |
| SWC-2034 | Form-associated `submit` / `reset` support | Depends on the ElementInternals/tooling recommendation and is intentionally out of the initial Button scope. | [Must ship — breaking or a11y-required](#must-ship--breaking-or-a11y-required), [Deferred semantics note (gen2)](#deferred-semantics-note-gen2) |
| SWC-2035 | Host `focus` / `blur` compatibility parity | Initial gen2 Button will support `click` plus bubbling `focusin` / `focusout`; extra compatibility events can be evaluated later. | [Internal semantic button implications](#internal-semantic-button-implications), [Documentation](#documentation) |
| SWC-2036 | React Spectrum-only `genai` and `premium` variants | Not part of the approved baseline Button scope in this migration plan. | [Additive](#additive--ships-when-ready-zero-breakage-for-consumers-already-on-gen2), [Public API](#public-api) |
...
- [gen2 global button stylesheet](../../../../gen2/packages/swc/stylesheets/global/global-button.css)
- [gen2 global elements stylesheet](../../../../gen2/packages/swc/stylesheets/global/global-elements.css)
- [React Spectrum S2 Button](https://react-spectrum.adobe.com/Button)
- [Global element styling guide](../../../../gen2/packages/swc/.storybook/guides/customization/global-elements.mdx)
