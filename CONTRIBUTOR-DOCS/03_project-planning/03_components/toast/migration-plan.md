...
- Toast is the individual notification element: one host, one message slot, one optional action, and a close button. It is designed to be used with `swc-toast-container`. Standalone use is not an intended supported configuration (Q15). The container and its queue manage placement, stacking, focus behavior, and timing.
- Toast ships alongside a first-party container/queue in this migration (Q6); proposed name `swc-toast-container` (Q13, open for feedback). Resolving Q5 (queue construct location/ownership) and Q8 (repositioning animation) is a near-term prerequisite, not a deferred tail-end item: most of the [API](#api) checklist's container-and-queue work, and nearly all of [Accessibility](#accessibility), depend on that architecture existing. Only a narrow single-toast skeleton can be implemented before that architecture exists. That sequencing does not make standalone Toast a supported configuration (Q15). See [Migration sequencing and prerequisites](#migration-sequencing-and-prerequisites).
...
- Q1, Q2, Q3, Q7, Q10, Q12, Q14, and Q15 are resolved; see the [Decision log](#decision-log). Q5, Q8, Q9, Q11, Q13, and Q16 remain open; Q5, Q8, Q11, and Q13 have a documented leaning, see below.
...
- **Q13** in [Architecture and behavior](#architecture-and-behavior): container naming and API shape. Proposed `swc-toast-container` with a decoupled queue export for showing toasts, open for feedback. The action is a constrained API, not a caller-provided element (Q3); exact names stay open here.
- **Q16** in [Architecture and behavior](#architecture-and-behavior): test the expanded toast alongside an open popover, tooltip, and modal dialog before settling how native dialog inertness and `Escape` dismissal interact.
...
| `swc-button` | Dependency, already migrated | Toast renders the optional action as `swc-button`, not `swc-action-button` (Q3). Callers do not slot it in |
...
**Container/queue ticket (placeholder):** a follow-up ticket tracks resolving Q5, Q8, and Q13, and then building the container/queue construct, its styling (peek-stack depth, expand/collapse layout), and the multi-toast portions of Accessibility that depend on it. It is sequenced ahead of the remaining API and Accessibility work, not after it: only the narrow single-toast skeleton described above proceeds before the container exists. That does not make standalone Toast a supported configuration (Q15). This does not reopen Q6; the container still ships as part of this migration, the ticket just tracks the order the work happens in.
...
| B3 | Action API | Light-DOM `action` slot, caller-provided button | No caller-provided `action` slot. One optional action, configured through a constrained API (exact names still open, Q3). Toast renders it as `swc-button`, not `swc-action-button` | Stop providing an `action` slot. Configure the optional action through the constrained API once its names are settled (Q13) |
...
| A6 | Action + auto-dismiss | Unguarded | Hard-disabled: if a toast action is configured when the toast opens, the timeout has no effect, paired with a `warnIf()` dev warning (Q2) | None |
...
| `timeout` | `number \| null` | `null` | `timeout` | **Confirmed.** Floors to 6000ms; `null`/`0`/negative disables auto-dismiss, matching 1st-gen (Q1). Has no effect when a toast action is configured (Q2). The container's queue runs it only while that toast is the collapsed front toast, and it does not run in the expanded view (Q12). Standalone use without `swc-toast-container` is not an intended supported configuration (Q15) |
| `icon-label` | `string \| undefined` | `undefined` | `icon-label` | **Confirmed.** Carried forward as-is |

**Action (Q3).** One optional action, configured through a constrained, prop-style API. Conceptually this is an action label and an action callback. Exact property names, method names, types, and callback signatures are still open (Q13). There is no caller-provided `action` slot. Toast renders the action itself; see [Visual matrix (gen2)](#visual-matrix-gen2).
...
Action button (when present): Toast renders one `swc-button`, not `swc-action-button`, styled as secondary, outline, `static-color="white"`. Callers do not slot this button in (Q3). Icons on the action are disallowed so they do not compete with the Toast UI. Confirmed by Figma playground and RSP S2 (`variant="secondary" fillStyle="outline" staticColor="white"`).
...
No `action` slot. The optional action is configured through the constrained API in [Properties / attributes (gen2)](#properties--attributes-gen2), not provided as slotted content (Q3).
...
| Queue export | For example a module export with `add()`/`close()`/`clear()`, or per-variant convenience methods (`.info()`, `.positive()`, etc.) | Decoupled from the container element; exact shape still open as part of Q13, including how the constrained action configuration (an action label and an action callback; names still open) and an `onClose` callback are supplied (see below). Not a caller-provided action element |
...
**How content flows from the queue export to a rendered toast:** calling the queue export (`add()`, or a per-variant convenience method) constructs a `<swc-toast>` element, sets its `variant`/`icon-label`/etc., applies the call's action configuration when one is present, and sets the call's message argument as its slotted content, then hands it to `swc-toast-container` to render; see [Slots (gen2)](#slots-gen2) for the message slot (Q9). `swc-toast` is the individual notification element and is meant to be used with `swc-toast-container` and its queue. The container and queue manage placement, stacking, focus behavior, and timing. Standalone use, including a hand-authored `<swc-toast open>` with no container, is not an intended supported configuration (Q15).

**Still open as part of Q13:** the exact shape of the constrained action API (Q3). Conceptually this is an action label and an action callback, without final property names, method names, types, or callback signatures. The queue export supplies that configuration. It does not accept a caller-provided action element or DOM node. Toast renders one optional action as `swc-button` (not `swc-action-button`), with the fixed treatment in [Visual matrix (gen2)](#visual-matrix-gen2). Icons on the action are not part of the API. Also open: whether the queue export accepts an `onClose` callback per call, matching RSP S2's `addToast()`, which fires whenever that specific toast closes, for any reason.
...
| **SWC (`swc-toast`)** | `gen2/packages/swc/components/toast/` | `Toast.ts`, `toast.css`: renders host role/state attributes, variant icon, inner `role="alert"` wrapper, the default message slot, the optional action as `swc-button` from the constrained action API (Q3), and `swc-close-button`. No caller-provided `action` slot. Element registration, stories, tests. |
...
- Core (`swc-toast`) owns ARIA attribute wiring and variant validation; SWC (`swc-toast`) renders variant icon, inner live-region wrapper, message slot content, the optional action, and the close button
...
- [ ] Render the optional action from the constrained action API (Q3): one `swc-button` (not `swc-action-button`), secondary, outline, `static-color="white"`. Exact property names and callback signatures stay open (Q13). Do not accept a caller-provided `action` slot
...
> Blocked on resolving Q5 (queue construct location) and Q8 (repositioning animation) first. Q13 (container naming/API) has a documented leaning but is still open for feedback. This work, and most of [Accessibility](#accessibility), is sequenced ahead of the rest of the implementation, not deferred behind it; only the single-toast bullets above can proceed before the container exists (Q15: standalone use is not an intended supported configuration).
...
- [ ] Decoupled queue export for adding, closing, and clearing toasts (exact shape pending Q13, including how the constrained action configuration and an `onClose` callback are supplied; not a caller-provided action element)
...
- [ ] `timeout` hard-disabled when a toast action is configured, checked from that configuration when the toast opens; paired with a `warnIf()` dev warning, matching the "mutually exclusive / no-effect combination" pattern used elsewhere (e.g. Badge's `outline` + non-semantic `variant`) (Q2)
...
| Q13 | Container naming and API shape. Proposed tag name `swc-toast-container`, open for feedback. Proposed shape: the element renders the region/list/peek-stack/expanded-view (see [Architecture](#architecture-core-vs-swc-split)), but adding a toast goes through a separate, decoupled queue export (methods like `add()`/`close()`/`clear()`, or per-variant convenience methods), not a method on the element itself, matching RSP S2's `ToastQueue`/`ToastContainer` split; see [Container API](#container-api-gen2). Also covers `placement` (proposed default `bottom`), the one-container-per-view dev warning, and the still-open exact shape of the constrained action API and an `onClose` callback (see [Container API](#container-api-gen2)). The action is not a caller-provided element or `action` slot (Q3). | Yes | Open ❓ (leaning documented) | Architecture reviewer + Design |
| Q16 | Expanded-view interaction with other open surfaces: test a background popover and tooltip when the toast calls `<dialog>.showModal()`, including native inertness, focus, and which surface responds to <kbd>Escape</kbd>. Determine whether the toast still needs `dismissibleStack` registration or should close those surfaces before expanding. Also test a toast triggered inside an already-open modal dialog and agree with Design on whether and how its expanded view can open. Resolve these scenarios before finalizing the expanded-view dismissal and focus behavior described in [Dependencies](#dependencies) and [Architecture](#architecture-core-vs-swc-split). | Yes | Open (scenario testing and design decision pending) | Architecture reviewer + Design |
...
| Q2 | `timeout` is hard-disabled (not just a dev warning) when a toast action is configured through the constrained action API (Q3), matching RSP S2, which forces `timeout` to `undefined` when an action is set. Presence comes from that configuration when the toast opens. | Team decision going into the API phase, restated with Q3. Paired with a `warnIf()` dev warning, matching the codebase's existing "mutually exclusive / no-effect combination" pattern (e.g. Badge's `outline` + non-semantic `variant`, Progress-circle's `indeterminate` + `value`) rather than staying silent. Exact action API names remain open (Q3, Q13). |
| Q3 | Do not keep a caller-provided light-DOM `action` slot. Use a constrained, prop-style action API instead. Exact property names, method names, types, and callback signatures stay open. | The action is a single optional control with a fixed Spectrum treatment, so callers do not need broad markup control. Toast renders it as `swc-button`, not `swc-action-button`: secondary, outline, `static-color="white"`. Icons on the action are disallowed so they do not compete with the Toast UI. With `swc-toast-container` and its queue in scope, that constrained API fits better than a caller-provided slot. This replaces the earlier decision to keep the 1st-gen `action` slot. `SlotAttributePropagationController` is not used, and `SlotPresenceController` is not used to detect action content. |
...
| Q6 | `swc-toast` ships alongside a first-party container/queue in this migration, rather than shipping Toast now and deferring the container. | Settled via team sync. Only "whether to build it in this cycle" is resolved; the container's shape and API (peek stack, expand/collapse, focus management, `clear()`, and related accessibility behavior) remain open and are being worked out incrementally across this plan. `placement` (RSP precedent: `top`/`bottom`/`top end`/`bottom end`) belongs on the container, not on `swc-toast` itself. Standalone use is not an intended supported configuration (Q15). |
...
| Q14 | Peek-stack layers (decorative toasts behind the front one, while collapsed) never contain real interactive content. They render as a wholly separate, empty element, no close button, no action button, nothing focusable, not the real toast's markup hidden or dimmed behind `role="presentation"`. | Matches RSP S2's `Toast.tsx` exactly: `SpectrumToast` substitutes a plain, empty `<div role="presentation">` for any non-main toast while collapsed. This also settles the tabindex concern raised during review: `role="presentation"` alone doesn't reliably remove descendants from the tab order across every AT/browser combination, but an element with zero focusable descendants doesn't need that guarantee at all. It stays compatible with either outcome of Q8: RSP's animated morph works by matching `viewTransitionName` across the real toast and the empty decorative div, not by keeping the same interactive markup mounted across the position change. During Q8 View Transitions prototyping, check whether keeping peek layers populated with managed focusability would preserve the same keyboard and assistive-technology behavior with a simpler transition before committing to the empty-layer swap. |
| Q15 | Standalone `swc-toast` is not an intended supported configuration. `swc-toast` is the individual notification element and is designed to be used with `swc-toast-container`. The container and its queue manage placement, stacking, focus behavior, and timing, including auto-dismiss. | Supporting a hand-authored `<swc-toast open>` with no container would put placement and accessibility on the consumer, which is why the container exists. The earlier question of a standalone auto-dismiss timer was carryover from an earlier version of the plan. There is no supported standalone path for that timer to serve. |
...
- [React Spectrum S2 Toast source](https://github.com/adobe/react-spectrum/blob/main/packages/@react-spectrum/s2/src/Toast.tsx): `variant`, RSP's own `actionLabel` / `onAction` / `shouldCloseOnAction` (RSP names; the SWC action API names are still open), 5s timeout floor, hard-block on actionable auto-dismiss
