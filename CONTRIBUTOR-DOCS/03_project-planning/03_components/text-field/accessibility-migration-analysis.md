...
| [Content on hover or focus (1.4.13)](https://www.w3.org/WAI/WCAG22/Understanding/content-on-hover-or-focus.html) | Does not apply the way it did in 1st-gen. gen2 drops the custom hover/focus tooltip for truncated values (see **Truncated value** in [ARIA roles, states, and properties](#aria-roles-states-and-properties)), matching [React Spectrum's explicit guidance against tooltips on text fields](https://github.com/adobe/react-spectrum/wiki/Frequently-Asked-Questions-(FAQs)). The full value is made accessible through the native input itself (caret movement, selection, horizontal scroll), not through a hover/focus affordance; an optional native `title` is only a pointer-only convenience whose presentation is user-agent-controlled and out of this criterion's scope. |
...
| **Error message via `aria-describedby`** | When `invalid` is `true`, fold the slotted `error-text` into the input's `ariaDescribedByElements` (after the description), so the error is both visible and programmatically associated for [error identification (3.3.1)](https://www.w3.org/WAI/WCAG22/Understanding/error-identification.html). `aria-errormessage` is intentionally **not** used, following [React Spectrum's TextField](https://react-spectrum.adobe.com/TextField): its AT support is still inconsistent while `aria-describedby` is universally read, and a single description association avoids duplicate-announcement and ordering ambiguity across AT. The error element is added to the description set only while `invalid` is `true`. **This revises an earlier recommendation to set both `aria-errormessage` and `aria-describedby`; see the migration plan [Decision log](./migration-plan.md#decision-log) (B9).** |
...
| **Character count (deferred)** | Deferred: not part of this migration. 1st-gen never implemented the CSS spec's character-count feature, and gen2 does not add it here. If it is added later, associate it via `aria-describedby` alongside help text and error text rather than leaving it a visual-only `<span>`, otherwise screen reader users get no remaining-character information at all. |
...
**None — resolved by design, not by omission.** This is worth explaining because 1st-gen genuinely has a cross-root problem here, and the point of replacing `sp-field-label`/`sp-help-text` with the `LabellingController` is that gen2 doesn't inherit it.
...
- **Labeled, placeholder only, no value:** name still comes from the label, never from the placeholder. The placeholder is exposed as the native `placeholder` accessible-name-fallback *only* in the true edge case of a completely unlabeled field — which gen2 should dev-warn against rather than quietly rely on.
- **Invalid:** `aria-invalid="true"`; error message text is both visible and reachable via `aria-describedby` (the error folds into the description set while invalid; `aria-errormessage` is not used); the alert-triangle icon stays `aria-hidden="true"` and adds no separate node to the tree.
