...
| [Content on hover or focus (1.4.13)](https://www.w3.org/WAI/WCAG22/Understanding/content-on-hover-or-focus.html) | Does not apply the way it did in 1st-gen. 2nd-gen drops the custom hover/focus tooltip for truncated values (see **Truncated value** in [ARIA roles, states, and properties](#aria-roles-states-and-properties)), matching [React Spectrum's explicit guidance against tooltips on text fields](https://github.com/adobe/react-spectrum/wiki/Frequently-Asked-Questions-(FAQs)). The full value is made accessible through the native input itself (caret movement, selection, horizontal scroll), not through a hover/focus affordance; an optional native `title` is only a pointer-only convenience whose presentation is user-agent-controlled and out of this criterion's scope. |
...
| **Character count (deferred)** | Deferred: not part of this migration. 1st-gen never implemented the CSS spec's character-count feature, and 2nd-gen does not add it here. If it is added later, associate it via `aria-describedby` alongside help text and error text rather than leaving it a visual-only `<span>`, otherwise screen reader users get no remaining-character information at all. |
