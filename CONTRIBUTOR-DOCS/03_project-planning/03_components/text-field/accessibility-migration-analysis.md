...
| **Accessible name — two sources, one writer** | Two ways to name the field, both rendered by the **`LabellingController`** ([SWC-2466](https://jira.corp.adobe.com/browse/SWC-2466)) inside `swc-text-field`'s own shadow root, not as an externally associated `swc-field-label`: (1) a **visible label**, provided as slotted content and rendered by the shared directive as a real, same-root `<label>` — the default, recommended path whenever a visible label exists; or (2) the **`accessible-label`** property (this project's established `accessible-label` → `aria-label` pattern, e.g. `swc-popover`'s), which sets `aria-label` directly on the `<input>` — for the case where no visible label is slotted. Because `aria-label` outranks a native `<label>` association in accessible-name computation, don't set `accessible-label` *and* slot a visible label with different text at the same time — that would silently override the visible text in the accessible name and risk a [WCAG 2.5.3 Label in Name](https://www.w3.org/WAI/WCAG22/Understanding/label-in-name.html) mismatch. Whichever source is used, there is exactly one writer by construction (one Lit render pass, one shadow root), which removes 1st-gen's two-writer hazard where `Textfield.ts`'s own render and `FieldLabel.ts`'s imperative cross-root `setAttribute('aria-label', …)` raced on the same attribute (see [Shadow DOM and cross-root ARIA Issues](#shadow-dom-and-cross-root-aria-issues)). |
...
**If an external labelling need ever arises** (for example, labelling a field from a page heading or table header it does not own — the "light DOM siblings via properties" half of the hybrid model in the [web component form strategy demos](https://github.com/nikkimk/web-component-form-strategy-demos/tree/main)), the right tool would still be the ARIAMixin element-reference APIs (`ariaLabelledByElements`/`ariaDescribedByElements`, Baseline 2025: Chrome 135+, Safari 16.4+, Firefox 136+) rather than an IDREF string or a direct cross-root `setAttribute`. This is not part of the default `swc-text-field` design and should not be built speculatively — add it only if a concrete consumer need appears.

### Accessibility tree expectations

- **Labeled, with a value:** name comes from the slotted visible label (rendered as a same-root `<label>`) or, when no visible label is slotted, the `accessible-label` property (rendered as `aria-label`); role `textbox`; value is the current text. Description, if help text or an error message is slotted, comes from the same-root `aria-describedby` (and, when invalid, `aria-errormessage`) target.
...
| **aXe + Storybook** | Dev-warning story for a field with no accessible name at all (no slotted label, no `accessible-label`, placeholder only) — this should warn, not silently pass. A story with both a slotted label and a differently-worded `accessible-label` set, to catch the [Label in Name (2.5.3)](https://www.w3.org/WAI/WCAG22/Understanding/label-in-name.html) mismatch risk. Error/invalid state story. Disabled and readonly stories. |
...
- [ ] `accessible-label` and a slotted visible label are never set with conflicting text at the same time (risk of a silent [WCAG 2.5.3](https://www.w3.org/WAI/WCAG22/Understanding/label-in-name.html) mismatch, since `aria-label` outranks the native `<label>` association).
