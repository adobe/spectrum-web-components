...
| **Description and error text** | Compose with `LabellingMixin` and `HelpTextMixin`. Associate a description and error message with the role-bearing inner input through the repository's supported `aria-describedby` path. Preserve a visible error when invalid. Define whether a persistent description remains associated while the error is shown, because the current `HelpTextMixin` swaps help text for error text while invalid. Do not prescribe `aria-errormessage` unless the receiving role and the implemented mixin explicitly support it. |
