...
- [Overview](#overview)
    - [Also read](#also-read)
    - [Related components and grouping (gen2)](#related-components-and-grouping-gen2)
...
- **Not a link:** **No** **`href`**, **no** **`LikeAnchor`** proxy anchor, **no** host **`role="link"`**. 1st-gen deprecated the link API on **`sp-action-button`** (dev warning); gen2 **drops** the API surface entirely. Prior art: [SWC-227](https://jira.corp.adobe.com/browse/SWC-227).
...
**Applies when `swc-action-button` ships hold / longpress after the initial deferral** (**Overview**). Until then, **`longpress-enabled`** and **`longpress-help-text`** are **spec-only** for documentation and future implementation—**do not** imply **`swc-action-button`** exposes **hold** on **initial** gen2.

gen2 should avoid **unconditional** supplementary descriptions that mention **longpress** or **Alt** + **Down arrow** when the instance does **not** implement that behavior.
...
- **`longpress-enabled`** — When **true**, the control **actually** wires **hold** / **longpress** (pointer timer and the **keyboard** path that reaches the same secondary action, aligned with [React Aria: Menu — long press](https://react-aria.adobe.com/Menu#long-press) / [React Spectrum `MenuTrigger`](https://react-spectrum.adobe.com/react-spectrum/MenuTrigger.html) as the reference). When **false** (the **default** until **hold** ships on **`swc-action-button`** after the **deferral**—**Overview**), **omit** **`aria-describedby`** for **longpress** instructions. **Future migration:** can mirror 1st-gen **`hold-affordance`** / **`longpress`** event wiring as **opt-in** when that enhancement is scheduled.
...
1st-gen **`sp-action-button`** does **not** ship a **pending** API; **Spectrum 2** still shows **pending** in the [Action button Figma](https://www.figma.com/design/Mngz9H7WZLbrCvGQf3GnsY/S2---Web--Desktop-scale-?node-id=702-2877). When gen2 adds **async** / **busy** feedback, treat it **identically** to **`swc-button`** pending guidance:
...
**gen2 consumer docs:** State clearly that **hold affordance** / **longpress** on **`swc-action-button`** is **deferred** until a **later** date and repeat the **consumer options** from **Overview** (1st-gen **`sp-action-button`**, **extend `swc-button`**, or **separate** **`swc-action-button`** controls in **`swc-action-group`**).

**Initial gen2 ship:** **`swc-action-button`** **does not** implement **`hold-affordance`** / **`longpress`**. The numbered guidance below defines how to treat the pattern **when** product schedules it **after** the deferral.
...
- [ ] **Group** / **menu** triggers: **`swc-button-group`** naming and **`aria-expanded`** / **`aria-haspopup`** for overlays; **no** **`aria-pressed`** / **`toggles`** on **`swc-action-button`**—**toggle** UX on **`swc-toggle-button`** / **`swc-toggle-button-group`** ([React Spectrum: ToggleButton](https://react-spectrum.adobe.com/ToggleButton), [ToggleButtonGroup](https://react-spectrum.adobe.com/ToggleButtonGroup)).
...
- [ ] **Hold** / **secondary** actions: **gen2 docs** state **hold** is **deferred** on **`swc-action-button`**; list **consumer options** (**1st-gen `sp-action-button`**, **extend `swc-button`**, **action-group** layout—**Overview**). When **hold** ships **later**, use **`longpress-enabled`** / **`longpress-help-text`** and full **testing** ([Longpress help text](#longpress-help-text-and-aria-describedby-proposed-properties)). **Storybook** consumer docs **must** call out **action group** (**separate** **`swc-action-button`** in **`swc-action-group`**) vs **longpress**-only; **`aria-describedby`** for **longpress** / **Alt** + **Down arrow** **only** when **`longpress-enabled`** is **true**, with **`longpress-help-text`** for **localization** ([React Aria: Menu — long press](https://react-aria.adobe.com/Menu#long-press) / [React Spectrum `MenuTrigger`](https://react-spectrum.adobe.com/react-spectrum/MenuTrigger.html)); align **press** / **key** events with [React Spectrum `ActionButton`](https://react-spectrum.adobe.com/ActionButton) where applicable.
