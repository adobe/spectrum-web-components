...
- [gen1 implementation notes (avoid in 2nd-gen)](#gen1-implementation-notes-avoid-in-2nd-gen)
...
- **Not a link:** **No** **`href`**, **no** **`LikeAnchor`** proxy anchor, **no** host **`role="link"`**. gen1 deprecated the link API on **`sp-action-button`** (dev warning); 2nd-gen **drops** the API surface entirely. Prior art: [SWC-227](https://jira.corp.adobe.com/browse/SWC-227).
- **Not `role="radio"` or `role="checkbox"`:** **`swc-action-button`** is always a **button** in the accessibility tree. **Radio-group** UX ships on **`swc-segmented-control`** / **`swc-segmented-control-button`**. **`swc-action-group`** + **`swc-button-group`** use **`role="group"`** on the wrapper and plain **buttons** inside—**not** gen1 **`role="radio"`** / **`role="checkbox"`** on children (see **gen1 implementation notes**).
...
## gen1 implementation notes (avoid in 2nd-gen)
...
| **`aria-expanded` / `aria-haspopup` (menus)** | **`swc-action-button`** does **not** use **`aria-pressed`** or gen1 **`toggles`** / **`selected`**. **Toggle** groups use **`swc-toggle-button`** / **`swc-toggle-button-group`**. For **menu** or overlay triggers, set **`aria-haspopup`** and **`aria-expanded`** in sync with the open surface. |
...
gen1 **`sp-action-button`** does **not** ship a **pending** API; **Spectrum 2** still shows **pending** in the [Action button Figma](https://www.figma.com/design/Mngz9H7WZLbrCvGQf3GnsY/S2---Web--Desktop-scale-?node-id=702-2877). When 2nd-gen adds **async** / **busy** feedback, treat it **identically** to **`swc-button`** pending guidance:
...
**Terminology:** **Hold affordance** is how **design** typically refers to the timed press-and-hold interaction; **`longpress`** (event) and **`hold-affordance`** (attribute) are the **gen1** engineering names for the same idea.

**2nd-gen consumer docs:** State clearly that **hold affordance** / **longpress** on **`swc-action-button`** is **deferred** until a **later** date and repeat the **consumer options** from **Overview** (gen1 **`sp-action-button`**, **extend `swc-button`**, or **separate** **`swc-action-button`** controls in **`swc-action-group`**).
