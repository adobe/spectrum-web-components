...
| B7  | `tip` attribute renamed to `hide-arrow` (inverted semantics); rendering moves to internal element | `<sp-popover tip>` (opt-in: set `tip` to show the arrow) renders an SVG tip directly inside the host's shadow root with `id="tip"` | Arrow is shown by default (React Spectrum S2 alignment). Set `hide-arrow` to hide it. The tip renders inside the internal `.swc-Popover` semantic container with `class="swc-Popover-tip"`. Class-based orientation; along-edge positioning via the `PlacementController`'s `arrow` middleware and `tipElement`. | **Breaking:** consumers setting `tip` must remove it (arrow now shows by default). Consumers that did not set `tip` (no arrow) must add `hide-arrow` to preserve the no-arrow behavior. |
