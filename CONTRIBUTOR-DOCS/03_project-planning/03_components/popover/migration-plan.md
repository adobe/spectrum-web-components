...
| `manual` | `boolean` | `false` | `manual` (reflect) | **Confirmed.** Suppresses the automatic trigger click-to-toggle wiring; the consumer drives `open` themselves. Used by first-party components (Picker, Menu, etc.) that own their open/close logic. ARIA relationship wiring still applies. Mirrors the tooltip plan's `manual`. See [Invocation pattern](#invocation-pattern). |
...
**`manual` opt-out.** First-party components that own their open/close logic (Picker, Menu, Action Menu, etc.) set the `manual` attribute to suppress the automatic click wiring and drive `open` themselves; ARIA relationship wiring still applies. Consumers can also drive `open` programmatically or via Lit binding at any time.
