...
| B8  | Computed placement not exposed as a public property | `actual-placement` was an Overlay-internal attribute set on the popover element | The host's `placement` attribute reflects the input (consumer request). The computed placement after `flip` middleware reorients is **not** exposed as a public, writable property. It is internal component state that drives `.swc-Popover--<placement>` modifier classes on the internal element (shadow-DOM scoped), which the component sets reactively. Tooltip reaches the same "no writable property" outcome by reflecting an internal `actual-placement` host attribute and styling via `:host([actual-placement])`; the popover styles its internal surface element with modifier classes instead. CSS that wants to react to placement targets the internal modifier class. | No consumer migration needed. Existing CSS that targeted 1st-gen `actual-placement` (if any) was already operating on Overlay-internal markup; that markup is gone. |
