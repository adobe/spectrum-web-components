...
- **API removals (B-series):** `[dialog]` host attribute removed (consumers wanting dialog padding compose `<swc-dialog>` and adopt `.swc-Popover` chrome). 1st-gen's `placement="undefined"` default changes — new default is `'bottom'` (React Spectrum-aligned).
...
- **Four deliverables in one PR:**
  ...
  3. **`dismissibleStack` module** in `2nd-gen/packages/core/utils/` — ~20 lines, generic LIFO stack for Escape coordination across all dismissible 2nd-gen top-layer UI. Modeled on WebAwesome's `dismissible-stack`. First consumer is `<swc-popover>` in this PR; tooltip + future components adopt subsequently.
...
- Background scroll lock policy for modal mode on iOS (Q7)
...
| `<swc-coachmark>` | Yes | TBD — depends on coachmark migration's authoring decision | Yes | Inheritance vs composition decision deferred to coachmark migration (Q5). If coachmark needs strict-modal semantics, it uses `<dialog>.showModal()` directly inside its shadow template. |
...
| A7  | Coachmark `extends Popover` decision | 1st-gen `Coachmark extends Popover`. 2nd-gen coachmark may extend `<swc-popover>` (inheritance) or implement its own lifecycle alongside `PlacementController` (composition). Decision belongs to the coachmark migration; tracked as Q5. |
...
- SWC's `popover.css` is authored on the internal element and consumed by the component as a constructed stylesheet. No `global-popover.css` is emitted in v1; the `vite-global-elements-css` plugin is not invoked for popover. Internal style sharing across first-party consumers (Q11) is deferred to the second-consumer migration.
...
**Implementation-phase smoke test required:** verify that `<dialog popover="auto">` + `showPopover()` works equivalently to `<div popover="auto">` + `showPopover()` across Chrome / Firefox / Safari. Specifically: `:popover-open` pseudo-class applies, `beforetoggle` and `toggle` events fire, light-dismiss (Escape + click outside) works, auto-stack participation works. If any browser has bugs, fall back to two separate render shapes (`<div popover="auto">` for default mode; `<dialog>` for modal mode); plan's API surface is unchanged.
...
- [ ] `dismissible-stack.ts`: exports three functions over a module-level `const dismissibleStack: object[] = []` — `registerDismissible(key)` pushes the key onto the stack, `unregisterDismissible(key)` removes the most-recent entry matching the key, `isTopDismissible(key)` returns whether the key is the topmost entry. Pattern modeled on WebAwesome's `dismissible-stack`.
...
- [ ] **Smoke test before implementation (Q3):** verify `<dialog popover="auto">` + `showPopover()` works equivalently to `<div popover="auto">` + `showPopover()` across Chrome / Firefox / Safari. If it does, proceed with the single-render architecture. If a browser has bugs, fall back to rendering separate elements per mode (`<div popover="auto">` default, `<dialog>` modal); the rest of the plan is unchanged.
...
| #   | Item | Blocking? | Status | Owner |
| --- | ---- | --------- | ------ | ----- |
| Q3  | **`<dialog popover="auto">` + `showPopover()` browser interop smoke test.** Spec-correct, but the combination is less battle-tested than either mechanism in isolation. Verify on Chrome / Firefox / Safari: `:popover-open` pseudo-class applies, `beforetoggle` and `toggle` events fire, light-dismiss (Escape + click outside) works, auto-stack participation works, and `dialog.matches(':popover-open')` returns true while open. **First implementation-phase gate.** If any browser has bugs, fall back to rendering separate elements per mode (`<div popover="auto">` default, `<dialog>` modal) — plan's API surface is unchanged. | Implementation gate | Pending | Implementation owner |
