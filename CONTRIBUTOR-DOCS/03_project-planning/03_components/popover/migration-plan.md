...
- **Two supported behavior modes — the `modal` attribute selects the internal element and open method:**
  ...
  - **API additions (B-series):** new properties — `modal`, `offset`, `cross-offset`, `container-padding`, `should-flip`, `for`, `trigger-element` (setter); new ARIA wiring on the trigger — durable `ariaControlsElements` and `aria-expanded` in both modes, plus `aria-haspopup="dialog"` when `modal` is set; new event contract — `swc-open`, `swc-after-open`, `swc-close`, `swc-after-close` with a `source` detail.
...
| B7  | `tip` attribute kept, rendering moves to internal element | `<sp-popover tip>` renders an SVG tip directly inside the host's shadow root with `id="tip"` | `<swc-popover tip>` renders the tip inside the internal `.swc-Popover` semantic container with class `swc-Popover-tip`. Class-based positioning replaces the JS-driven `style.translate` writeback. | No consumer API change beyond the rendering location — `tip` boolean attribute behaves the same. Precise tip positioning per trigger geometry is in scope for v1: the `PlacementController` already implements the `arrow` middleware, and the popover wires its tip element in via `tipElement`. |
...
| `tip` | `boolean` | `false` | `tip` (reflect) | **Confirmed.** Renders the SVG tip element with `class="swc-Popover-tip"`. Positioned along the placement edge by the `PlacementController`'s `arrow` middleware (the popover passes its tip element as `tipElement`); orientation comes from the `.swc-Popover--<placement>` modifier class. |
...
| `should-flip` | `boolean` | `true` | `should-flip` (reflect) | **Internal (`@internal`). Confirmed.** When `false`, disables `flip` middleware — the popover stays in the requested placement even when constrained. React Spectrum default is `true`. Set by first-party components; excluded from the public API table. |
...
> **Phase 3 scope note:** Phase 3 (API) delivered the typed **contract** — types, the property surface, runtime placement validation, `@internal` markers, and the `@fires` event documentation. The runtime **lifecycle** items below (`showPopover()` / `showModal()`, `swc-*` dispatch, ARIA wiring, backdrop-click, `dismissibleStack` / `PlacementController` integration) are deferred to Phases 4 (a11y) and 5 (styling/render), since they depend on the rendered internal element and ARIA semantics. They remain unchecked here intentionally.
...
- [x] `Popover.base.ts`: declares all properties (`open`, `modal`, `placement`, `actualPlacement`, `tip`, `offset`, `cross-offset`, `container-padding`, `should-flip`, `tip-padding`, `for`, `triggerElement`) on `SpectrumElement`; `container-padding` / `should-flip` / `tip-padding` marked `@internal`; runtime validation of `placement` against the `VALID_PLACEMENTS` static in `update()` (warns in dev mode via `window.__swc.warn`, falls through to controller)
...
- [ ] When `tip` is set, pass the rendered `.swc-Popover-tip` element to the `PlacementController` as `tipElement` (and `tipPadding`) so the controller's `arrow` middleware aligns the tip to the trigger. The controller already implements `arrow`; this is the popover-side wiring.
