...
- **No blocking open questions.** Remaining items are implementation-detail and follow-up: backdrop-click-to-close policy (Q6 — recommendation: strict-modal in v1), `<dialog>.showModal()` benchmarking on representative consumer DOMs (Q3), an amendment to the existing accessibility analysis to reflect modal-mode host semantics (Q4).
...
- Q6 (backdrop-click-to-close policy) needs explicit resolution before implementation begins — the recommendation is strict-modal (Escape and explicit close only), but the decision determines whether downstream consumers (Picker, Menu, etc.) need to wire their own backdrop dismiss handlers.
...
| --- | ------------ | ---------------- | ---------------- | ----------------------- |
| B1  | Host gains modal-dialog behavior — **major UX shift** | `HTMLElement`; no behavior; open/close driven externally by `<sp-overlay>`. Non-modal by default: page behind remained interactive, scrollable, and Tab-able. Click outside closed the popover via 1st-gen `OverlayStack` light-dismiss. | An internal `<dialog>` element is rendered in the shadow root and opens via `showModal()` on `open = true`. Browser provides `role="dialog"`, focus trap, background inert, native Escape handling, and top-layer rendering. The page behind is **blocking** (cannot be clicked, focused, or scrolled). A native `::backdrop` pseudo-element renders behind the dialog. The component owns its own open/close. See [Differences from 1st-gen popovers](#differences-from-1st-gen-popovers) for the full comparison. | Consumers stop pairing `<sp-popover>` with `<sp-overlay>`. They either (a) use `<swc-popover>` with `open` property control, (b) use a first-party 2nd-gen component that has popover built-in (Picker, Action Menu, etc.), or (c) adopt the `.swc-Popover` class for pure styling without behavior. Existing UX assumptions about "click outside to dismiss" and "page stays scrollable" must be updated — see Q6 for backdrop-click-to-close policy. |
...
| S2  | Move styling from `:host` to `.swc-Popover` internal element | All rules on `:host` and `:host([dialog])` | All rules on `.swc-Popover` (with `.swc-Popover--<placement>` modifiers, `.swc-Popover-tip` for the tip element). Host has only the structural rules needed for `<dialog>` / popover top-layer semantics. | No consumer action. Enables the auto-generated `global-popover.css` class distribution. |
...
| `swc-close` | On `<dialog>` `cancel` (Escape pressed) OR `close` (programmatic close) — de-duplicated so only one fires per close cycle. | none |
| `swc-after-close` | On `transitionend` after the close transition completes. Same 0-duration guard as `swc-after-open`. | none |

**Cancellable** (additive, not in v1): allowing `event.preventDefault()` on `swc-open` to abort the open, or on `swc-close` to abort the close. The `<dialog>` `cancel` event can be `preventDefault`-ed to keep the dialog open on Escape, which gives us a hook for this — but the semantics get subtle and are deferred until a consumer needs them.
...
| **Click outside the popover** | Closed the popover (light-dismiss via 1st-gen `OverlayStack`) | **Does not close by default.** Native `<dialog>.showModal()` does not implement backdrop-click-to-close; only Escape and explicit close actions dismiss. See Q6 for whether the component opts in to wiring backdrop-click-to-close. |
...
**Most significant practical consequence:** consumer UX changes from "click anywhere outside to dismiss" to "Escape or explicit close to dismiss" (unless Q6 resolves toward backdrop-click). This affects Picker, Menu, Action Menu, Contextual Help, and Coachmark — every first-party migration that follows.
...
- SWC renders an internal `<div class="swc-Popover">` (or `<dialog class="swc-Popover">` depending on the host-vs-internal-element decision) wrapping `<slot>` and, when `tip` is set, a `<span class="swc-Popover-tip"></span>` element. Placement-modifier classes (`.swc-Popover--<actual-placement>`) are applied reactively.
...
- [ ] `Popover.ts` (SWC): renders the internal `<dialog class="swc-Popover">` element; slot for content; conditional `<span class="swc-Popover-tip">` when `tip` is set; reactive class binding for `.swc-Popover--<actual-placement>` modifiers; element registration
...
- [ ] Q6 resolution: implement backdrop-click-to-close OR explicitly verify the "strict modal" behavior is wired correctly per the chosen recommendation
...
- [ ] Add `.swc-Popover` to the internal semantic container in `render()`; keep styling off `:host` except for structural rules required by the host's element type
...
- [ ] Modal mode: Escape closes; `cancel` event observed and `swc-close` dispatched
- [ ] Non-modal mode opens via `showPopover()` on `open = true`
- [ ] Non-modal mode closes via `hidePopover()` on `open = false`
- [ ] Non-modal mode: light-dismiss closes; `beforetoggle` observed and `swc-close` dispatched
...
- [ ] Modal-by-default UX differences (B1, see [Differences from 1st-gen popovers](#differences-from-1st-gen-popovers)) verified manually on a representative consumer-like setup: page-behind is inert, scroll is blocked, Escape closes, focus returns to trigger, click outside behavior matches Q6 resolution
...
- [ ] Consumer migration guide: **prominently call out the user-visible behavior differences** from [Differences from 1st-gen popovers](#differences-from-1st-gen-popovers) — page-behind is inert, page scroll is blocked, click-outside-to-close is gone (subject to Q6), backdrop pseudo-element renders, stacking semantics changed, initial focus and focus restoration are now native. Each is a deliberate consequence of modal-by-default, not a regression.
...
| Q6  | **Backdrop-click-to-close policy.** Native `<dialog>.showModal()` does NOT dismiss when the backdrop is clicked — only Escape or explicit close does. 1st-gen popovers dismissed on click outside via `OverlayStack` light-dismiss. Two options for v1: **(a)** stay strict — only Escape and explicit close dismiss; matches native `<dialog>` semantics and aligns with the user's "blocking by default" framing. Click-outside-to-close becomes an additive opt-in (alongside non-modal mode in A1). **(b)** wire a backdrop click listener that closes on click outside; matches React Aria's modal popover UX and 1st-gen consumer expectations for Picker / Menu / Action Menu. **Recommendation (a)** for v1 — match the native modal contract, document the change in the migration guide as part of B1, and revisit with consumer feedback. Each first-party component can override per pattern if needed (see [Relationship to other component migrations](#relationship-to-other-component-migrations)). | No (recommendation captured; decision belongs to architecture review) | To resolve before implementation | Architecture reviewer |
