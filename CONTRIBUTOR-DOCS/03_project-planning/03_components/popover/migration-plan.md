...
| B7  | Arrow shown by default; toggled via `hide-arrow` (replaces 1st-gen `tip`) | `<sp-popover tip>` opts *in* to an SVG tip inside the host's shadow root with `id="tip"` (off by default) | The arrow is shown by default inside the internal `.swc-Popover` element as `<span class="swc-Popover-tip">`; `hide-arrow` removes it. This inverts the 1st-gen `tip` opt-in to match React Spectrum's S2 `hideArrow`. Positioned along the placement edge by the `PlacementController`'s `arrow` middleware (the popover passes it as `tipElement`); orientation from the `.swc-Popover--<placement>` modifier class. | Consumers that relied on `tip` to show the arrow now get it by default; those that omitted `tip` set `hide-arrow` to keep no arrow. |
...
The public surface mirrors React Spectrum's S2 `Popover` — `placement`, `size`, `offset`, `cross-offset`, `should-flip`, `hide-arrow` — plus the web-component-specific `open`, `modal`, `for`, `trigger-element`, `manual`, and the readonly `actualPlacement`. Properties marked **Internal (`@internal`)** below (`container-padding`, `tip-padding`) are positioning implementation details: still functional and settable, but tagged `@internal` so the CEM analyzer excludes them from the generated public API table and component docs.
...
| `size` | `'s' \| 'm' \| 'l' \| undefined` | `undefined` | `size` (reflect) | **Confirmed.** Optional fixed size. When set, the popover uses a fixed inline size (`s` → 336px, `m` → 416px, `l` → 576px); when unset, it fits its contents. |
...
| `hide-arrow` | `boolean` | `false` | `hide-arrow` (reflect) | **Confirmed.** The arrow (tip) is shown by default; set `hide-arrow` to remove it. The arrow element (`class="swc-Popover-tip"`) is positioned along the placement edge by the `PlacementController`'s `arrow` middleware (the popover passes it as `tipElement`); orientation comes from the `.swc-Popover--<placement>` modifier class. When the arrow is shown, the main-axis `offset` is increased by the arrow allowance. |
| `offset` | `number` | `8` | `offset` | **Confirmed.** Main-axis offset in pixels from the trigger. Passed to `PlacementController` (plus an arrow allowance when the arrow is shown). |
| `cross-offset` | `number` | `0` | `cross-offset` | **Confirmed.** Cross-axis offset in pixels. Passed to `PlacementController`. |
| `should-flip` | `boolean` | `true` | `should-flip` (reflect) | **Confirmed.** When `false`, disables `flip` middleware — the popover stays in the requested placement even when constrained. |
| `container-padding` | `number` | `8` | `container-padding` | **Internal (`@internal`). Confirmed.** Distance from viewport edge for `flip` and `shift` middleware. Matches 1st-gen's `REQUIRED_DISTANCE_TO_EDGE`. Excluded from the public API table. |
