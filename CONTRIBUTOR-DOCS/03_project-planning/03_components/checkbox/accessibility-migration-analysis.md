...
- **In a group:** checkboxes are **each** a Tab stop and are reached with <kbd>Tab</kbd> and <kbd>Shift</kbd> + <kbd>Tab</kbd>; a checkbox group does **not** use roving tabindex or arrow-key navigation (the [APG checkbox pattern](https://www.w3.org/WAI/ARIA/apg/patterns/checkbox/) has neither). Each is toggled independently with <kbd>Space</kbd>. This is the multi-select counterpart to radio's single-roving-Tab-stop model.
