...
1st-gen bundles Spectrum-derived rules plus overrides; public `--mod-*` surface is **not** a supported consumer contract for gen2 (see [Rendering analysis — Modifiers](./rendering-and-styling-migration-analysis.md)). gen2 will not re-expose `--mod-*`; any new surface follows [`--swc-*` guidelines](../../../../CONTRIBUTOR-DOCS/02_style-guide/01_css/02_custom-properties.md#component-custom-property-exposure).
...
- **Ship link styling with typography / prose consumption paths** in gen2 so AI Toolkit and document UIs that already use `swc-Typography--prose` ([examples](../../../../gen2/packages/swc/patterns/ai-toolkit/system-message/stories/system-message.stories.ts)) get correct `<a href>` appearance without requiring `sp-link` per link.
...
- **Typography (gen2):** Primary integration point for prose link appearance and documentation ([`typography` component](../../../../gen2/packages/swc/components/typography/), [migration guide](../../../../gen2/packages/swc/components/typography/migration-guide.mdx)).
...
**Q2 is resolved** (CSS + native `<a>` only — no compatibility CE). Treat Link like **Typography** for repo shape: **no `gen2/packages/core/components/link/`** (Typography today has **no** core `.ts` layer — only [`gen2/packages/swc/components/typography/`](../../../../gen2/packages/swc/components/typography/) for stories/tests/docs and [`gen2/packages/swc/stylesheets/typography.css`](../../../../gen2/packages/swc/stylesheets/typography.css) for generated rules).

- **NPM package:** still **`@adobe/spectrum-wc`** (`gen2/packages/swc`); there is no separate link-only package.
- **Primary link stylesheet:** **`gen2/packages/swc/stylesheets/link.css`** — all **variant / modifier** rules live here; authors apply **BEM-style classes** on `<a>` for presentation. **Do not** encode presentation as custom attributes (beyond normal HTML anchor attributes such as `href`, `rel`, etc.).
...
**Link-specific deviation:** the [Badge migration reference](../../02_workstreams/02_gen2-component-migration/02_step-by-step/01_washing-machine-workflow.md#reference-badge-migration) describes the default **core + `sp-*` SWC** split. For Link, the **primary** gen2 deliverable is **shared CSS + documentation** with **native anchors** (**Q2** resolved). Packaging defaults are **set** in [Recommended packaging (default)](#recommended-packaging-default) (including **PR 6304** review).

| Layer | Path | Contains |
| ----- | ---- | -------- |
| **Core** | `gen2/packages/core/components/link/` | **N/A** — **Q2** resolved: CSS + native `<a>` only (same as Typography; no core) |
| **SWC** | `gen2/packages/swc/components/link/` | **N/A** — no `sp-link` CE; rules under [`stylesheets/`](../../../../gen2/packages/swc/stylesheets/) and [`stylesheets/global/`](../../../../gen2/packages/swc/stylesheets/) per [Recommended packaging (default)](#recommended-packaging-default); stories/tests extend **Typography** |
...
- [x] Copy S2 source from `spectrum-css` `spectrum-two` branch [`components/link/index.css`](https://github.com/adobe/spectrum-css/tree/spectrum-two/components/link) (not `/dist`) into the chosen gen2 stylesheet entry point
...
- [ ] `yarn lint:gen2` passes (ESLint, Stylelint, Prettier) — link CSS and docs pass stylelint/prettier; repo-wide ESLint has pre-existing failures unrelated to Link
...
- [gen2 Typography migration guide](../../../../gen2/packages/swc/components/typography/migration-guide.mdx)
- [gen2 — Semantic HTML and ARIA](../../../../gen2/packages/swc/.storybook/guides/accessibility-guides/semantic_html_aria.mdx)
