...
[CONTRIBUTOR-DOCS](../../README.md) / [Project planning](../README.md) / Research / Research: SWC-1420 — TypeScript conventions guide for 2nd-gen
...
- **Styles declaration:** Use `public static override get styles(): CSSResultArray` consistently for styles declaration, not static property assignment.
...
- **@property()** — Public reactive properties; attribute reflection; options (type, reflect, attribute, etc.). Always specify `type` in `@property()` options (e.g., `type: String`, `type: Boolean`, `type: Number`). Do not omit it.
...
- **Validation lifecycle:** Use `updated()` for post-render validation (debug warnings). Note: Badge currently uses `update()` but should migrate to `updated()` for consistency.
...
- **Type prefix naming:** Use underscore separators for multi-word component names in type prefixes: `STATUS_LIGHT_`, `PROGRESS_CIRCLE_`, not `STATUSLIGHT_`.
...
**Focus management:** For focus management, use a mixin when focus delegation requires method overrides (`focus()`, `blur()`, `click()`); use a controller for complex focus patterns like focus trapping or roving tabindex.

**Note:** `FocusVisiblePolyfillMixin` is a 1st-gen pattern that may be unnecessary in 2nd-gen given modern browser support for `:focus-visible`. Evaluate browser support matrix before porting.
...
- **Depth limit:** Target maximum mixin depth of 2 mixins (e.g., `SizedMixin(FocusableMixin(SpectrumElement))`). Move additional cross-cutting concerns to reactive controllers.
