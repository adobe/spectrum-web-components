...
[CONTRIBUTOR-DOCS](../../README.md) / [Project planning](../README.md) / Research / 2nd-gen component review: patterns and findings
...
10. **Role assignment**: Divider uses unconditional role assignment (correct pattern); Progress Circle uses conditional assignment (unnecessary complexity that could be simplified).
...
4. **Conditional role assignment**: Progress Circle checks for an existing role before setting, which adds unnecessary complexity. Components should be opinionated about their semantic role. If consumers need a different role, they should use a different component or submit a feature request.
...
2. **Accessible name validation** for Badge (icon-only variants). Note: Status Light does not require accessible name validation — consumers are responsible for including text alongside the component.
