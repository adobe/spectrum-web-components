...
[CONTRIBUTOR-DOCS](../README.md) / [Accessibility guides](README.md) / Semantic HTML, ARIA, and cross-root accessibility
...
- [Semantics and HTML](#semantics-and-html)
    - [What semantic HTML means](#what-semantic-html-means)
    - [Why prefer semantic elements](#why-prefer-semantic-elements)
    - [Semantic vs non-semantic examples](#semantic-vs-non-semantic-examples)
    - [How semantics help assistive technologies](#how-semantics-help-assistive-technologies)
    - [Examples in this repository](#examples-in-this-repository)
- [ARIA as a secondary method](#aria-as-a-secondary-method)
    - [What ARIA is](#what-aria-is)
    - [When to use ARIA](#when-to-use-aria)
    - [The first rule of ARIA use](#the-first-rule-of-aria-use)
    - [Appropriate ARIA examples](#appropriate-aria-examples)
    - [Patterns to avoid](#patterns-to-avoid)
    - [Custom elements and the host node](#custom-elements-and-the-host-node)
- [Cross-root ARIA challenges](#cross-root-aria-challenges)
    - [The problem](#the-problem)
    - [Cross-root ARIA delegation (emerging)](#cross-root-aria-delegation-emerging)
    - [Reference target (cross-root ARIA)](#reference-target-cross-root-aria)
    - [Impact on web component accessibility](#impact-on-web-component-accessibility)
...
Use ARIA **only when native HTML cannot express the needed role, state, or relationship**. If a native element exists with the right semantics, use it instead of repurposing a `div` or `span` with `role` and ARIA attributes.

### The first rule of ARIA use

The [W3C note “using ARIA”](https://www.w3.org/TR/using-aria/) states the first rule: **if you can use a native HTML element or attribute with the semantics and behavior you need, do so** rather than repurposing another element and adding ARIA. See [rule 1](https://www.w3.org/TR/using-aria/#rule1) in that document.
