...
- [gen2 layout](#gen2-layout)
- [Abstraction targets (gen2)](#abstraction-targets-gen2)
...
**Scope: 1st-gen → gen2.** This guide applies when migrating or designing for gen2 (where we can rethink placement). 1st-gen stays as-is: packages/ for components, tools/ for tools.
...
When deciding where something lives in gen2, ask: **does it depend on the rendering layer?**
...
For placement within `core/`, see [packages/core/MIGRATION.md](../../gen2/packages/core/MIGRATION.md).
...
## Abstraction targets (gen2)

When reclassifying 1st-gen tools or tool-like components for gen2, use the [decision process](#decision-process) to determine the category, then target the appropriate abstraction:

| 1st-gen package / tool | gen2 abstraction | Location / form |
| ---------------------- | ------------------- | ---------------- |
...
When a 1st-gen **component** or **tool** is reclassified in gen2 as a utility (e.g. CSS + helpers instead of a full package):

1. **Do not** migrate 1:1 as a new gen2 component package. Implement the gen2 side as the chosen abstraction (utils, CSS utilities, etc.) as in [Abstraction targets](#abstraction-targets-gen2).
...
2. **1st-gen:** Keep the existing package supported until a deprecation path is agreed. When deprecating:
   ...
   - Document the gen2 replacement. Note that not all utilities belong in `core/`—if the utility depends on the rendering layer or carries Spectrum 2 styles (e.g. a reclassified `swc-asset`), it may live in `swc/utils/` instead of `core/utils/`.
...
- **Code** — Place code according to [gen2 layout](#gen2-layout). Do not add new 1st-gen-style top-level "tools" packages in gen2.
...
- **Contributor docs** — This document is the contributor-facing process for evaluating tools vs packages in gen2.
