...
vrts.push([
    ...
    `https://${getHash(`${branch}-hcm`)}--spectrum.edgecompute.app/review/`,
]);
themes.map((theme) =>
    colors.map((color) => {
        if (
        ...
        scales.map((scale) =>
            directions.map((direction) => {
                ...
                vrts.push([
                    ...
                    `https://${getHash(context)}--spectrum.edgecompute.app/review/`,
                ]);
            })
        );
    })
...
async function main() {
    ...
    const data = JSON.stringify({
        meta: {
            branch,
            preview: `https://${slugify(branch)}--spectrum.edgecompute.app`,
            commit,
            system,
            vrts,
        },
