...
`sp-theme` applies a Spectrum theme by using CSS custom properties to set default sizes & colors for all of the components in its scope. The Spectrum design system provides four color themes (`light` and `dark`) and two different scales (`medium` and `large`) to support desktop & mobile UI.
