...
describe("Theme", () => {
  testForLitDevWarnings(
    async () => await fixture<Theme>(html` <sp-theme></sp-theme> `),
  );
  ...
  describe("Dev Mode", () => {
    ...
    it("warns in Dev Mode when no attributes or fragments", async () => {
      const el = await fixture<Theme>(html` <sp-theme></sp-theme> `);
    ...
    it("warns in Dev Mode when you use Spectrum Two theme ", async () => {
      const el = await fixture<Theme>(html`
        <sp-theme system="spectrum-two" color="dark" scale="medium"></sp-theme>
      `);
