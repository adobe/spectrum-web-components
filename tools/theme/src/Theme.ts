...
export type { Color, ProvideLang, Scale, SystemVariant, ThemeFragmentMap };
/**
 * @element sp-theme
...
export class Theme extends HTMLElement implements ThemeKindProvider {
	...
	set color(newValue: Color | "") {
		...
		const color =
			!!newValue && COLOR_VALUES.includes(newValue) ? newValue : this.color;
	...
	set scale(newValue: Scale | "") {
		...
		const scale =
			!!newValue && SCALE_VALUES.includes(newValue) ? newValue : this.scale;
	...
	private get styles(): CSSResultGroup[] {
		const themeKinds: FragmentType[] = [...Theme.themeFragmentsByKind.keys()];
		const getStyle = (
		...
		const styles = themeKinds.reduce((acc, kind) => {
			const kindFragments = Theme.themeFragmentsByKind.get(kind) as FragmentMap;
	...
	if (window.__swc.DEBUG) {
		...
		const checkForAttribute = (
			...
			if (!resolvedValue) {
				...
				!themeFragmentsByKind
					.get(name)
					?.get(resolvedValue + (name === "system" ? "" : systemModifier))
			) {
				issues.push(
